...
**Date:** 2026-04-17  
**Status:** 🟡 Node compatibility is now threaded through the CLI/runtime command surface, package host-fit validation, and a widening runtime-linker projection layer for common Node built-ins; the helper surface now also exposes the remaining process/util/assert helpers plus Buffer base64/hex round-tripping, the runtime linker now covers path, crypto, os, child-process, and process stdout/stderr projection alongside the existing fs/stream/http/process wiring, and the package corpus now includes Node-assuming coverage, but broader Node execution coverage is still incomplete
