...
- embeddability through a Phase-1 base library artifact that is useful immediately for **exact-version consumers**, with a Phase-2 public embedding surface: stable Rust embedding API plus the stable public **WIT-first** `--lib` contract, with `--capi` and `--component` as explicit projections/packaging flows over that same export surface.
...
| Registry audit | `kali package-audit` is a separate context-free registry-analysis/security-audit workflow and opens in Phase 4 as the envelope-only JSON command |
...
| **Phase 1 promises** | what the first dependable release may actually claim | first-class TS/JS compilation under the shared **bounded inference contract**; Kali-hosted execution in the shared **Default standalone context (schema v1)** plus non-browser builds in the shared **Deno-oriented build context (schema v1)**; the shared **Phase-1 browser-targeted command set**; runtime sandbox enforcement plus the shared **Phase-1 static policy-validation surface**; deterministic install/lock behavior; the Phase-1 **base library artifact** for **exact-version consumers**; proof-ready repository state |
...
Guardrail:
- attaching `--sandbox` never rescues an otherwise-invalid command shape or phase-gated API/artifact combination; it only adds the owning sandbox workflow step once the underlying command/context pair is itself valid.
...
3. **Kali-hosted execution foundation** — one AOT pipeline to one linked WASM payload, `run`/`test` in the shared **Default standalone context (schema v1)** on the Deno-oriented standalone surface, and the Phase-1 runtime/resource sandbox contract.
...
5. **Developer workflow foundation** — the minimal `init` scaffold contract, `check`, `fmt`, `lint`, the canonical `W2xxx` lint registry, AI-friendly diagnostics, and stable schema-v1 JSON envelopes/artifact metadata.
...
- **“Support Node, Deno, and browser APIs”** → Phase 1 centers Kali-hosted execution on the shared **Default standalone context (schema v1)** / Deno-oriented standalone surface plus the shared **Phase-1 browser-targeted command set**; broad Node compatibility is Phase 3.
...
- **“Latest ECMA-262”** → latest **published** ECMA-262 grammar is Phase 1; draft/Stage-3+ proposal support stays explicitly gated rather than implied.
...
- **“Constraint solving is on the table”** → keep one bounded solver story: Phase 1 allows only budgeted local/intra-module constraint solving inside the shared **bounded inference contract**, exported/public API inference still respects the shared **annotation-required inference boundary**, and broader higher-cost solver work is Phase 3+ depth rather than an implicit Phase-1 whole-program promise.
...
- **“supports browser APIs”** should be read through four separate questions: browser ambient typing, browser-targeted bundle/deploy path, standalone browser execution, and the smaller browser-applicable portion of Kali's mediated sandbox/effect vocabulary. Phase 1 ships the first two inside the shared **Phase-1 browser-targeted command set** and keeps policy/effect reasoning scoped to the documented browser-applicable mediated subset rather than inventing one policy key per ambient browser/DOM API.
- **“supports npm packages”** should be read through the shared **package-support decision order** plus the **package-support ladder**: package shape first, then host/API fit, then command maturity, all against the published artifact Kali actually installs, and only then the exact rung being claimed (`installable/materializable`, `checkable`, `buildable`, `executable`, or `deployable-through-host`). In schema v1, mutating dependency reconciliation remains owned by `kali install` alone, and npm lifecycle hooks stay an explicit opt-in install-time path (`kali install --allow-scripts`) rather than evidence of early `--api node` support.
- **“supports non node-gyp packages from npm”** should be read as a package-shape shortcut only, not as the full compatibility test: absence of `node-gyp` is not enough if the published package still depends on N-API/native bindings, prebuilt binaries, postinstall-downloaded executables, unsupported Node-only host assumptions, or another path outside the shared **pure JS/TS package contract**.
...
| Deno, Node, and browser support | Phase-gated breadth target | Phase 1 for the shared **Default standalone context (schema v1)** plus the exact **Phase-1 browser-targeted command set**; Phase 3 for broader Node compatibility | Phase 1 centers Kali-hosted execution on the Deno-oriented standalone surface through the shared **Default standalone context (schema v1)**, with the shared **Phase-1 browser-targeted command set** alongside it; Node is phase-gated until Phase 3 | [`specs/11-standard-apis.md`](./specs/11-standard-apis.md), [`specs/19-feature-maturity.md`](./specs/19-feature-maturity.md) |
...
| AI-friendly CLI and diagnostics | Hard invariant | Phase 1 MVP | Human output stays concise; JSON contracts, stable codes, and the canonical `W2xxx` lint registry are explicit product requirements | [`specs/12-cli.md`](./specs/12-cli.md), [`specs/15-errors.md`](./specs/15-errors.md), [`specs/18-schemas.md`](./specs/18-schemas.md) |
...
- **host breadth**: Phase 1 centers Kali-hosted execution on the shared **Default standalone context (schema v1)** / Deno-oriented standalone surface plus the shared **Phase-1 browser-targeted command set**; broad Node support is later.
...
- `README.md` should stay overview-first: keep headline assumptions, repository-state notes, and navigation there, but prefer linking to `SPEC.md` / `19-feature-maturity.md` instead of reproducing large normative command/context matrices that are likely to drift
- current verification-state wording in `README.md`, release notes, and other summaries should link to [`proofs/BOUNDARY.md`](./proofs/BOUNDARY.md) instead of restating whether the repository is still on the placeholder proof boundary
- if a PR changes a spec/implementation path already named inside the published proof boundary, it should update the corresponding Lean model/proof boundary in the same change or explicitly shrink the claimed boundary first; proof coverage must not silently drift behind the published manifest
...
- single-package registry-analysis wording should reuse the **single-package registry-analysis command**, the shared **registry-analysis target contract (schema v1)**, **registry-analysis availability boundary**, **registry-analysis context split**, and **registry-analysis command split** instead of re-listing command shape, version selection, and project-independence details ad hoc
...
| `kali build --lib --api browser lib.ts` | contradictory browser-library build shape (`E5008`); the browser-targeted context in Phase 1 is `check` + `build --bundle`, not a library artifact mode |
...
- `package-audit` is the Phase 4 context-free security-audit command: it follows **context-free registry analysis (schema v1)** and is an **envelope-only JSON command** in schema v1.
...
- **base library artifact** — the Phase-1 `kali build --lib` output shape: export-oriented and useful immediately for **exact-version consumers**, but still the pre-stable Phase-1 half of the public embedding surface
- **exact-version consumers** — hosts, internal tooling, or repository-local integrations that pin the exact Kali toolchain/runtime build that produced the artifact and upgrade in lockstep with it; this is the allowed Phase-1 consumption story for the **base library artifact**, and it is intentionally narrower than a stable public cross-version embedding contract
...
- **standalone execution** uses the shared **Default standalone context (schema v1)** on the Deno-oriented standalone surface,
