...
| Real-package e2e validation (for example `semver` and `@mariozechner/pi-coding-agent`) | preserved as phase-correct package-corpus evidence: representative package probes must assert the right rung and expected outcome for the current host/API maturity, and bootstrap-named runnable npm binaries should be tracked as explicit binary-entrypoint probes rather than assumed to be Phase-1 executable |
