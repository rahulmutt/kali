...
- **“supports npm packages”** should be read through the shared **package-support decision order** plus the **package-support ladder**: package shape first, then host/API fit, then command maturity, then the exact rung being claimed (`installable/materializable`, `checkable`, `buildable`, `executable`, or `deployable-through-host`). In schema v1, mutating dependency reconciliation remains owned by `kali install` alone, and npm lifecycle hooks stay an explicit opt-in install-time path (`kali install --allow-scripts`) rather than evidence of early `--api node` support.
