...
- Continue validating JSON payloads, artifact manifests, diagnostics, source spans, and schema-v1 envelopes at emission boundaries; the CLI envelope validator now also has direct rejection coverage for non-string command/stdout/stderr fields, with dedicated regression coverage pinning the command/stdout/stderr field-shape failures, the validator now also accepts schema-optional stdout/stderr/exitCode omission on externally sourced envelopes, and the nested diagnostic/label/related source-span path now also rejects reversed ranges explicitly, while the build/config `maxSpecializations` path now has explicit zero-value acceptance coverage on manifest parsing plus artifact-metadata validation, with direct negative-value rejection coverage now spanning both manifest parsing and the artifact-metadata validator path, while build-result provenance coverage now also rejects whitespace-padded `profileDataHash` on the schema-validation path.
...
- Keep docs/schema drift tests aligned with README and CLI examples; the README command examples are now parse-checked against the live CLI parser, the browser runtime contract docs now also spell out trim-on-compare handling for the summary and scope note fields alongside the existing host label/description fields, and the `doctor` smoke now exact-matches the canonical browser diagnostic hint string in JSON output instead of only checking for a substring, including a pretty-JSON integration check for the same contract. Top-level doctor payload coverage now also rejects whitespace-only `summaryNote` and `contractScopeNote` values on that same browser runtime contract path, and the browser runtime contract validator now also pins duplicate-rejection coverage for trimmed `supportedCommands` and `diagnosticNotes` items, including trimmed-duplicate regression coverage for both arrays. The artifact-metadata schema/validator path now also treats `maxSpecializations` as a non-negative integer so generated manifests, bindings, and JSON snapshots stay in step, and build-result provenance coverage now also rejects whitespace-padded `profileDataHash` on the schema-validation path.
