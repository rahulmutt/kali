...
- `kali package-audit --pretty lodash` now has a regression guard for the required `E5508` invalid-usage path when JSON mode is absent, and the `--preview` shim is unit-tested to fail before target validation or registry lookup.
