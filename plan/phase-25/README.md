...
- `kali package-effects --pretty browserpkg` now also keeps the pretty-native-JSON path covered under inherited browser resolution without requiring `--output json`, matching the native JSON contract, and the README command reference / CLI-spec drift test now also cover `kali package-effects --pretty lodash` so the native-JSON package-effects example stays aligned with the documented contract. The README command reference now also includes the `kali package-effects --pretty --output json lodash` envelope example so the package-analysis examples stay symmetric with `package-audit`.
