...
- Package-analysis-specific `--api` / `--compat` / `--wasm-threads` / `--sandbox` rejections now carry canonical CLI flag context in JSON output so the single-package registry-analysis contract stays machine-auditable; the broad `runtime_smoke` coverage now also asserts that context on the `package-effects` and `package-audit` JSON rejection paths.
