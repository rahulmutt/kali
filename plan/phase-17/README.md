...
- Current progress: browser late-compat TSX smoke now also mirrors the full `Proxy.revocable` alias family, including the bracketed `globalThis["Proxy"].revocable` and `globalThis.Proxy["revocable"]` spellings, alongside the existing gated object-model aliases so the TSX late-object gate stays aligned with the JS/browser coverage; sandbox effect analysis now also classifies the bracketed `Proxy.revocable` family as `proxy-traps` so the `effects` surface matches the same late-object alias set; the same TSX lane now also rejects `Intl.NumberFormat` through the mixed `globalThis["Intl"].NumberFormat` / `globalThis.Intl["NumberFormat"]` / `globalThis["Intl"]["NumberFormat"]` spellings so the broader Intl gate is no longer JS-only in browser late-compat smoke; browser-requested run/test broader-Intl smoke now also carries the same mixed-bracket `NumberFormat` spellings in the shared runtime smoke helper, keeping the run/test rejection matrix aligned with the late-compat coverage.
## Exit gate
