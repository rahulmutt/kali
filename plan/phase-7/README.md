...
- Progress: the standalone browser-requested run/test path is wired behind `KALI_BROWSER_BUNDLE_HARNESS_COMMAND` and now has regression coverage for the opt-in success path, including inherited browser `apiSurface` configs for both `run` and `test`; the default rejection path remains when the harness command is absent.
