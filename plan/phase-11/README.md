...
- Progress note: `Math.floor` now has integer-only smoke coverage; `Math.round` now also has integer-only smoke coverage; `Math.sqrt` now has a statically-known perfect-square integer literal path; nullish coalescing `??` is now implemented and covered across direct and browser-targeted smoke paths; `Promise.allSettled` remains on the gap list.
