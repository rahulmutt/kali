...
- Progress note: the literal-string dynamic import path now also resolves raw backtick template-literal specifiers with statically evaluable interpolations, keeping `check` / `build` resolution and browser-bundle chunk discovery aligned for that narrower constant-expression slice; direct browser-bundle smoke now also covers the same raw backtick template-literal specifier slice in TS and `.js` input.
