...
- Package-analysis-specific flags (`--api`, `--compat`, `--wasm-threads`, and `--sandbox`) now reject on `kali package-effects` and `kali package-audit` through the canonical `E5508` invalid-usage path instead of falling through to raw parser errors, keeping the inherited-only registry-analysis contract explicit in both commands.
