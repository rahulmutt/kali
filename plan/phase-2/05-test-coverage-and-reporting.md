...
- Added a quiet-mode JSON regression for the coverage path so `kali test --coverage --output json --quiet` still emits the stable envelope instead of dropping the machine-readable payload.
- Added pretty-JSON regression coverage for `kali test --coverage` so the stable coverage payload stays pinned across the presentation-control branch as well as the plain JSON branch.
