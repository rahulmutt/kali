...
- the CLI smoke suite now also pins duplicate and whitespace-padded `compat.features` inputs for both `effects` and `package-effects`, plus whitespace-padded `runtimeProfiles` inputs on `effects`, so the canonical analysis-context normalization remains stable at the command boundary as well as in the reusable payload
