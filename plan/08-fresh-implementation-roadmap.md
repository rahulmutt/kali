...
- the shared diagnostics exit-code helper now treats `E5506` feature-unavailable diagnostics as usage-style failures too, so the command spine keeps phase-gated availability checks on the canonical exit-code path alongside the other E55xx machine-contract errors
...
- the browser entrypoint smoke lane now also covers the plain `opera`, `vivaldi`, and `mullvad browser` spellings alongside the earlier wrapper aliases, so the browser-launcher coverage stays aligned with the same alias table without changing the browser-runtime contract
...
- `kali install` on an empty workspace now has a CLI JSON-envelope regression that proves the command stays a clean no-op without scaffolding `kali.json` or `kali.lock`, so the F5 evidence packet keeps the configless install split honest at the operator boundary; that same empty-workspace JSON path now also stays identical across repeated invocations, so the no-op install contract is represented in the determinism lane as well
...
- computed-bracket `Deno.permissions.query(...)` access now keeps the same effect-free payload while still surfacing the shared `computed-host-access` dynamic reason, so bracketed permissions queries stay visible without inventing a new effect kind
- the permission-facade hardening now also pins the statically-known bracketed `Deno.permissions["request"]()` / `revoke()` spellings, so the query-only Deno facade remains honest even when source code uses string-literal property access.
...
- the browser entrypoint smoke lane now also covers the plain `opera`, `vivaldi`, and `mullvad browser` spellings alongside the broader browser family set, so the browser-launcher alias table stays pinned across the same alias set without changing the browser-runtime contract
- the browser entrypoint smoke lane now also covers the remaining `chromium-browser`, `chromium-headless-shell`, `google chrome`, `brave`, `brave-browser`, `brave browser`, `vivaldi snapshot`, `microsoft-edge`, and `microsoft edge` spellings, so the CLI smoke lane stays aligned with the runtime alias table without widening the browser contract
...
- `kali init` now scaffolds missing target directories before the empty-directory check and carries a nested-child current-directory regression, so the F4d workflow packet stays aligned with the documented current-directory-scoped init contract
...
- package-effects now also has repeated-invocation determinism coverage in both native JSON and envelope modes, keeping the inherited-context registry-analysis sibling pinned across back-to-back runs. The shared determinism lane now also exercises the repeated-invocation envelopes for `effects` and `package-audit`, and it now keeps repeated JSON-envelope output stable under inherited browser analysis context too, so the stable reporting surfaces stay pinned alongside the artifact outputs.
- package-effects now also preserves browser resolution and inherited eval compatibility together under top-level sandbox config in JSON output, keeping the fresh-start registry-analysis packet honest when the browser and compat axes overlap instead of only one at a time
...
- package-audit now also carries a quiet pretty JSON regression with a top-level sandbox config path, keeping the fresh-start registry-audit packet deterministic even when unrelated policy plumbing is present in the manifest.
- package-audit now also keeps inherited Node analysis context aligned with a top-level sandbox config in JSON output, so the fresh-start registry-audit packet stays orthogonal across the documented node-and-policy axes instead of only proving each one independently.
...
- the source-graph `effects` command now also pins repeated native-JSON output under inherited browser analysis context, so both output modes stay deterministic on the browser-context report path
- the determinism smoke lane now also includes the inherited-browser JSON-envelope regression for `effects`, so the browser-context source-graph report stays pinned alongside the default JSON and quiet pretty variants.
- the source-graph `effects` command now also ignores a top-level `sandbox` config path in JSON output for the explicit browser-analysis path, so the same source-graph lane stays decoupled from policy-attachment plumbing there as well
- the source-graph `effects` command now also keeps inherited browser analysis context aligned with a top-level sandbox path in JSON output, so the browser-resolution and sandbox-attachment axes stay pinned together in the same JSON envelope path
- the reusable effect-report payload now also deduplicates repeated logical roots in first-seen order before serialization, keeping future multi-root callers deterministic without changing the current single-root CLI demos; the sandbox unit suite now pins that duplicate-root behavior directly alongside entry-point dedupe
...
- the CLI smoke suite now also pins duplicate `--compat` inputs on `effects`, whitespace-padded inherited `compat.features` on `package-effects`, and the manifest/config duplicate-compat rejection path, so the canonical analysis-context normalization remains stable at the command boundary as well as in the reusable payload
...
- the inherited-browser browser-bundle smoke lane now also combines `--bundle`, `--validate-ir`, and `--sandbox`, so the fresh-start packet keeps the inherited config path pinned alongside the explicit browser-flag path
- the standalone browser runtime negative gates for `run --api browser` and `test --api browser` now also stay pinned under inherited browser config plus attached sandbox policies, so the F4/F5 browser rejection path remains aligned across explicit and inherited config even when policy plumbing is present
