...
- The browser-bundle harness override parser now fails closed on malformed command specs instead of quietly reusing the default `bun` / `node` harness, and the browser smoke tests pin that rejection path for whitespace-only, empty executable token, and unterminated quoted invocations.
