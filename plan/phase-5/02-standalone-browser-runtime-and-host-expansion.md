...
- The CLI-level browser-entrypoint smoke lane now also covers the Chrome-for-Testing family (`chrome-for-testing`, `chromium-for-testing`, and `google-chrome-for-testing`) plus the additional privacy/community aliases (`librewolf`, `waterfox`, `zen-browser`, `zen browser`, `thorium-browser`, and `thorium browser`), keeping the browser-launcher evidence aligned with the full alias table instead of only the smaller stable-channel subset.
...
- The browser launcher normalizer now also strips macOS `.command` wrapper scripts before alias matching, so shell-wrapper launches for browser app bundles classify the same way as the underlying executable name and stay covered by the browser-alias tests.
...
- The browser-runtime summary parser now also tolerates noisy summary-file content and still prefers the last JSON summary line when the side-channel file contains log chatter, keeping the file-backed path deterministic instead of only the stdout fallback carrying that resilience.
- Added display-name alias coverage for Zen Browser and Thorium Browser so the later browser-runtime harness recognizes those common space-separated launcher spellings in addition to the existing hyphenated names.
- Added Chrome-for-Testing alias coverage (`chrome-for-testing`, `chromium-for-testing`, and `google-chrome-for-testing` plus their space-separated display-name forms) so the browser executable classifier stays aligned with the common CI/browser-testing launcher spellings as well as the older stable/beta/canary shims.
- Expanded the browser-launcher alias coverage to iterate over the full canonical browser executable-name table exposed by `kali_runtime`, keeping the browser host chooser and HTML-entrypoint detection aligned with every currently recognized Chromium, Chrome, Edge, Brave, Firefox, Opera, Vivaldi, Librewolf, Waterfox, Zen, and Thorium spelling instead of only a hand-curated subset.
- Expanded browser-launcher alias coverage to include Edge/Edge-variant shims such as `msedge-beta`, `msedge-canary`, `msedge-dev`, and `edge-beta` / `edge-canary` / `edge-dev`, keeping the browser host chooser honest across more Chromium/Edge spellings without changing the later browser contract.
- Expanded browser-launcher alias coverage to include Opera release-channel spellings (`opera-beta`, `opera-developer`, `opera-unstable`, `opera beta`, `opera developer`, `opera unstable`) so the browser runtime path stays honest for another common family of real-world browser shims without changing the later browser contract.
- Expanded browser-launcher alias coverage to include additional common nightly/insider-style shims (`brave-browser-nightly`, `brave browser nightly`, `firefox-beta`, `firefox beta`, `microsoft-edge-insider`, `microsoft edge insider`, `vivaldi-snapshot`, `vivaldi snapshot`) so the browser runtime path stays honest for more real-world launcher spellings without changing the later browser contract.
- Expanded browser-launcher alias coverage to include dev-channel Chromium/Chrome variants (`chrome-dev`, `chromium-dev`, `google-chrome-dev`, `google chrome dev`, `brave-browser-dev`, `brave browser dev`) alongside the existing stable/beta/canary display-name set, keeping the HTML-entrypoint and headless-launch selection honest for a broader set of real-world browser shims as well as direct browser binaries.
- Expanded browser-launcher alias coverage to include canary-channel variants (`google-chrome-canary`, `google chrome canary`, `microsoft-edge-canary`, `microsoft edge canary`) alongside the existing stable/beta/dev display-name set, and added explicit Chrome headless-shell aliases (`chrome-headless-shell`, `google-chrome-headless-shell`) plus bare `chrome` support in the browser harness, keeping the HTML-entrypoint and headless-launch selection honest for real-world browser shims as well as direct browser binaries.
...
- Added regression coverage for stable launcher spellings such as `chromium-browser`, `google-chrome-stable`, `firefox-esr`, `opera-stable`, and `vivaldi-stable` so the browser alias list stays aligned with common distro shims as it continues to grow.
...
- The browser-runtime execution helper now threads the summary-reported `runtimeBackend` label through the returned outcome, so browser-harness provenance stays machine-readable end to end instead of being hardcoded at the call site.
...
- The browser-runtime rejection diagnostics now also consume the browser contract's canonical ordered note list, keeping the supported-command, summary, scope, and host-description notes centralized as one stable bundle instead of repeating the emission order manually at the call site.
...
- The browser-bundle HTML-entrypoint regression now uses a symlinked browser-like executable shim, keeping the browser-name detection path covered without depending on a writable shell-script wrapper in the temporary harness directory.
- Hardened the shared browser-runtime summary reader so a malformed summary side-channel falls back to the captured stdout JSON instead of dropping the browser harness outcome on the floor, keeping the later browser runtime path deterministic when a host wrapper misbehaves.
...
- Browser-harnessed browser-runtime executions now surface a distinct `browser-harness` runtime-backend provenance label in `RuntimeOutcome`, keeping the host-adapter story explicit when the configured browser harness path is used.
...
- The browser-bundle harness override parser now fails closed on malformed command specs instead of quietly reusing the default `bun` / `node` harness, and the browser smoke tests pin that rejection path for whitespace-only, empty executable token, flag-only, and unterminated quoted invocations.
...
- The browser-runtime helper now also emits an HTML browser-host wrapper for browser-like launchers, so the same self-contained module body can run under a real browser entrypoint instead of only the node/bun smoke harness.
- The browser-runtime harness now captures deterministic summary metadata through an internal summary-file side channel in addition to the stdout fallback, so later `run` / `test` plumbing can keep harness summaries separate from guest output while still remaining host-agnostic.
- The browser-runtime summary reader now prefers the last JSON summary line from stdout when the side-channel file is missing or malformed, keeping guest log noise from obscuring the deterministic harness summary.
- The browser-runtime summary reader now also falls back to stdout when the side-channel file contains only an incomplete JSON object without the required args/tests arrays, so stale or partial summary writes stay non-fatal instead of hijacking the final outcome.
...
- Added a second browser-like executable smoke regression for a `mullvad-browser`-named shim, so the CLI-level browser entrypoint path stays covered by more than one canonical browser alias as that host-selection logic grows.
