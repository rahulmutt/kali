...
- Browser launcher normalization now also strips Windows `.lnk` shortcut suffixes before alias matching, so shortcut-based browser launchers stay on the same browser-executable path as the other shell/bundle wrappers already covered by the harness tests.
