...
- Host-predicate registration now has an explicit availability gate in the embedding wrapper: callers can opt the façade out deterministically and receive the canonical `E5506` availability diagnostic instead of silently registering inert predicates when the feature is unavailable. The embedding context now also exposes a small availability probe so callers can check that gate without attempting a registration first.
