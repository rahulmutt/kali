...
- The browser/runtime compatibility crate now includes a deterministic `SharedArrayBuffer` / `Atomics` shared-memory baseline with clone-shared byte storage and byte-wise atomic helpers, giving the later threaded-profile work one concrete internal memory model to reuse when the runtime gate opens. It also now exposes deterministic worker and broadcast-channel shared-buffer queues so cross-runtime-instance coordination can preserve the shared backing store instead of collapsing shared-memory payloads back into isolated JSON messages.
