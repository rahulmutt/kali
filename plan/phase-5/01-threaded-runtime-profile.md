...
- The browser/runtime compatibility crate now includes a deterministic `SharedArrayBuffer` / `Atomics` shared-memory baseline with clone-shared byte storage and byte-wise atomic helpers, giving the later threaded-profile work one concrete internal memory model to reuse when the runtime gate opens.
