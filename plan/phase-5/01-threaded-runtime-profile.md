...
- `kali_runtime` now exposes a canonical runtime-profile helper for direct callers, so downstream code can observe the same deduplicated, trimmed runtime-profile vector without having to reconstruct the normalization logic itself.
