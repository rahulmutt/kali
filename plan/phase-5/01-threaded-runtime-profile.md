...
- The MIR ownership model now exposes canonical thread-boundary disposition helpers so later threaded-profile work can distinguish `shared heap` values that may cross runtime-instance boundaries from `stack`, `owned heap`, and `borrowed` values that must remain local, and the MIR regression suite now pins those ownership-to-boundary rules directly.
