...
- Build artifact metadata sidecars now carry the `runtimeProfiles` axis explicitly, and the build path now threads the effective runtime-profile vector through executable, library, C-ABI, component, and browser-bundle metadata emission so the emitted artifact contract stays aligned with the same semantic knob even though the current phase still rejects threaded requests before artifact production.
