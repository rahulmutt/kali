...
- Build artifact metadata sidecars now carry the `runtimeProfiles` axis explicitly, normalizing the current phase to the empty `[]` set while still threading the effective runtime-profile vector through executable, library, C-ABI, component, and browser-bundle metadata emission so the emitted artifact contract stays aligned with the same semantic knob even though the current phase still rejects threaded requests before artifact production.
