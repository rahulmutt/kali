...
- CLI and config parsing now recognize the documented `--wasm-threads` / `compilerOptions.runtimeProfiles` axis, normalize `wasm-threads`, and reject it explicitly with the canonical `E5506` gate until the profile actually opens.
- The deterministic shared-memory baseline now also exposes the full bytewise atomics trio (`and` / `or` / `xor`) on the shared-buffer helper and the `Atomics` facade, so the later threaded-profile support library stays aligned with the rest of the byte-level atomic helpers instead of stopping at arithmetic primitives only. The `Atomics::is_lock_free` query now routes through a shared `kali_common` capability helper so browser-facing compatibility layers can reuse one deterministic atomic predicate instead of repeating the same target check in each crate, and the regression suite now also pins that helper directly so the byte-atomic capability signal remains deterministic instead of only being exercised indirectly through the buffer-operation tests.
...
- The shared name-resolution layer now treats `SharedArrayBuffer` and `Atomics` as explicit later-compatibility globals and raises the canonical `E5506` threaded-profile diagnostic instead of a generic undefined-name error, including `globalThis.SharedArrayBuffer` / `globalThis.Atomics` member-access forms, so the first language-visible primitives for the later runtime profile are recognized consistently even though the threaded execution path is still gated off.
- The CLI now accepts the zero-cap thread-budget overrides documented for execution commands, with `--max-threads` / `--max-spawned-processes` normalizing through the shared `resources.*` vocabulary and rejecting positive values with the canonical `E5506` gate until their phase/profile contracts open.
...
- CLI smoke coverage now exercises the resolver-level threaded-global gates end to end for `globalThis.SharedArrayBuffer` and `globalThis.Atomics`, so the Stage 5.1 regression story includes both the lower-level name-resolution checks and a user-visible `kali check` smoke fixture for the language-visible primitives. The same CLI smoke suite now also pins the `run` / `test` command paths for those threaded globals in both text and JSON output, so the later-threaded gate stays honest across the full execution surface instead of only the checking path. The same CLI smoke suite also now pins the sandbox-attached policy path that sets positive `resources.maxThreads`, so the command-level evidence covers the canonical `E5506` rejection for policy-driven thread budgets on `check`, `run`, and `test` alike, now mirrored in both text and JSON envelope output.
- Browser-targeted build smoke coverage now pins the later-threaded gate too: `kali build --bundle --api browser` rejects inherited `runtimeProfiles = ["wasm-threads"]` with the canonical JSON `E5506` envelope instead of silently drifting into the browser bundle lane.
- Browser-targeted `check` / `build --bundle` smoke coverage now also exercises the inherited browser `apiSurface` path with `runtimeProfiles = ["wasm-threads"]`, so the browser analysis/build lane keeps honoring the same threaded-profile gate when the browser context comes from `kali.json` instead of an explicit CLI flag.
- The browser/runtime compatibility crate now includes a deterministic `SharedArrayBuffer` / `Atomics` shared-memory baseline with clone-shared byte storage and byte-wise atomic helpers, giving the later threaded-profile work one concrete internal memory model to reuse when the runtime gate opens. The atomics helper suite now also covers `exchange`, so the shared-memory baseline exercises the full byte-mutation path rather than just load/store/add/sub/compare-exchange, and the regression suite now pins the compare-exchange failure path so mismatched observations leave the buffer unchanged. It also now routes worker and broadcast-channel posts through deterministic shared-buffer queues so cross-runtime-instance coordination preserves the shared backing store instead of flattening the transport back to JSON-only payloads. The same compatibility layer now carries a deterministic threaded runtime-topology model that assigns one runtime instance per worker/thread and produces a stable shutdown/leak report for any instances that were still live at shutdown, and the report now distinguishes instances that were already terminated before shutdown from those still live when teardown begins; the regression suite pins the mixed live/terminated shutdown ordering, including the terminated-first teardown variant, so the snapshot list stays deterministic when teardown does not happen in spawn order. Worker and broadcast-channel shared-buffer posts are now ignored after termination/close, matching the existing post-message shutdown behavior and keeping the shared-buffer path symmetrical. The threaded-topology regression suite now also covers a multi-terminated shutdown case so the terminated-instance count remains deterministic when more than one worker has already exited before shutdown.
...
- canonical `E5506` gating before the profile is available or on unsupported targets
