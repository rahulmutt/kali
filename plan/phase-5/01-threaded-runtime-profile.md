...
- The deterministic shared-memory baseline now also exposes the full bytewise atomics trio (`and` / `or` / `xor`) on the shared-buffer helper and the `Atomics` facade, so the later threaded-profile support library stays aligned with the rest of the byte-level atomic helpers instead of stopping at arithmetic primitives only.
