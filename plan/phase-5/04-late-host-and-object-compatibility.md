...
- The sandbox predicate context now also carries deterministic process-identity and working-directory detail keys for the late host-control surface (`effects.process.pid`, `effects.process.cwd`, `effects.process.chdir`, and `effects.process.exit`), so host-registered narrowing predicates can inspect the same late-control metadata without inventing a separate vocabulary even though the runtime gate remains closed.
