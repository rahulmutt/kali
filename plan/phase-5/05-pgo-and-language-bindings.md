...
- The shared binding-package manifest generator now accepts explicit host/runtime provenance labels, and the CLI build paths now thread the same `kali-hosted` / `wasmtime` labels into the binding manifest that the metadata sidecars already record. That keeps the higher-level binding bundle aligned with the same build-context provenance tuple instead of hard-coding the labels inside one helper path.
...
- The maintained Python and Node binding wrappers now also surface the optional `profileDataHash` provenance from bundled `cabi-metadata` sidecars, so PGO context stays visible at the wrapper layer instead of only in the metadata summary helpers.
- Those same helper summaries now also preserve the optional `profileDataHash` field from the generated `cabi-metadata` sidecar, so PGO provenance stays visible in the maintained binding helpers when the CLI emits it.
- The maintained Node binding helper now also exposes explicit `...WithName()` discovery and load aliases for stem-specific binding-package bundles, and the high-level `KaliCAPI.fromBindingPackageWithName()` alias now mirrors that explicit-manifest convenience path for callers that want to target one generated bundle directly.
