...
- Progress note: the documented Node surface now exposes the read-only `process.pid` query across the explicit and inherited `check` / `build` / `run` / `test` paths and now also exposes the read-only `process.cwd` query there; the default standalone surface now also exposes the read-only `Deno.cwd` query, while `process.chdir` / `process.exit` remain gated. The Node helper layer also now offers a deterministic environment snapshot helper on the process projection, which keeps the later mutable-env materialization path concrete without changing the current CLI availability gate.
