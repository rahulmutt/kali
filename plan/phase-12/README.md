...
- Progress note: the documented Node surface now exposes the read-only `process.pid` query across the explicit and inherited `check` / `build` / `run` / `test` paths and now also exposes the read-only `process.cwd` query there; the default standalone surface now also exposes the read-only `Deno.cwd` query, while `process.chdir` / `process.exit` remain gated.
