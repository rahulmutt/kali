...
- Progress note: browser-requested and browser-bundle runtime smoke now also cover the missing-summary-file fallback path, keeping the stdout-authoritative rule pinned when the configured browser harness summary file is absent or whitespace-only; the browser-requested JSON smoke now also pins the whitespace-only summary-file variant in JS input, and the browser-requested JSON smoke now also covers an unreadable summary-file variant in JS input on Unix hosts.
