...
- Progress note: the documented Node surface now exposes the read-only `process.pid` query across the explicit and inherited `check` / `build` / `run` / `test` paths and now also exposes the read-only `process.cwd` query there; the documented Node surface now also allows `process.chdir` as the sandbox-policy-mediated cwd mutation API, and `process.exit` is now available on that Node surface as the process-control exit API, with direct CLI smoke now pinning that exit path in JS input across `check` / `build` / `run` / `test`; the Node surface now also supports the `process.env.KEY = ...` / `delete process.env.KEY` property-mutation slice in JS input on the supported `check` / `build` / `run` / `test` paths. The default standalone surface now also exposes the read-only `Deno.cwd` query and now also allows `Deno.chdir` and `Deno.exit` for sandbox-policy-mediated cwd mutation and exit control. The Node helper layer also now offers a deterministic environment snapshot helper on the process projection, and the full runtime projection now exposes that same snapshot accessor for host-context plumbing, which keeps the later mutable-env materialization path concrete without changing the current CLI availability gate. `Deno.env.toObject` remains gated until a dedicated object-return lowering path exists; do not widen browser or standalone support wording on the basis of the host snapshot helper alone.
