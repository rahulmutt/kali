...
- The object.is / Number.is* smoke corpus now also covers direct bracketed root spellings (`Object["is"]` and `Number["isFinite"]` / `Number["isInteger"]` / `Number["isSafeInteger"]` / `Number["isNaN"]`) across runtime, browser-harness, bundle, build, and type-resolution regression coverage.
