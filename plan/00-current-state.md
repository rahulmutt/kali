...
- Browser bundle chunk smoke coverage now also exercises literal dynamic imports from `.js` input, so the browser `build --bundle` lane keeps the linked-graph chunking path aligned with first-class JavaScript compilation.
