...
- Runtime assignment semantics: nullish assignment and logical `&&=` / `||=` now also accept wrapped mutable-local targets in direct JS/TS smoke and browser-harness smoke, and the supported arithmetic compound-assignment slice (`+=`, `-=`, `*=`, `/=`, `%=`, `**=`) now also accepts the same wrapped mutable-local targets on those paths.
