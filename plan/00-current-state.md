...
- runtime slices for supported object, iterator, BigInt, Math, Promise, console, dynamic-import, and reflection behavior, including the static ASCII `String.prototype.charCodeAt(index?)` fold for omitted and statically-known integer indexes and the static ASCII `String.prototype.codePointAt(index?)` fold for omitted and statically-known in-range integer indexes with canonical gates for dynamic/non-ASCII/out-of-range forms; the literal-array identity `values.map((value) => value)` iterator slice now also has direct `for...of` and `for await...of` regression coverage, and browser-targeted `check` / `build --bundle` smoke now also covers that identity-map slice in JS, TS, JSX, and TSX input; the truthy-literal identity `values.filter((value) => value)` iterator slice now also has direct `for...of` regression coverage, and its resolved array slice now also flows through `Array.from(...)` and spread consumers; the static identity `values.some((value) => value)` / `values.every((value) => value)` slices on literal arrays now also lower to deterministic boolean results in the type checker and codegen smoke lanes, and the literal-array `values.some((value) => value > 1)` / `values.every((value) => value > 1)` comparison slices now also lower to deterministic boolean results in the type checker and codegen smoke lanes; browser-requested run/test browser-harness coverage now also accepts the supported identity `map`/`filter`/`flatMap` literal-array slices and the `some`/`every` boolean and comparison slices, and standalone build/check smoke now also exercises the same supported `map`/`filter`/`some`/`every`/`flatMap` JS slices, while the remaining callback-bearing methods stay gated with the canonical E5506 path;
