...
- The threaded runtime profile is now accepted on supported `run`/`test` execution paths when explicitly opted in with `--wasm-threads`, and positive `--max-threads` values are honored only under that opt-in.
