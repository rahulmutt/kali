...
- runtime slices for supported object, iterator, BigInt, Math, Promise, console, dynamic-import, and reflection behavior; the literal-array identity `values.map((value) => value)` iterator slice now also has direct `for...of` and `for await...of` regression coverage, and browser-targeted `check` / `build --bundle` smoke now also covers that identity-map slice in JS, TS, JSX, and TSX input; the truthy-literal identity `values.filter((value) => value)` iterator slice now also has direct `for...of` regression coverage, and its resolved array slice now also flows through `Array.from(...)` and spread consumers;
