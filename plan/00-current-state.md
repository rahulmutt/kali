...
- Runtime built-in math evidence: `Math.max`, `Math.min`, `Math.abs`, `Math.sign`, `Math.imul`, `Math.round`, `Math.clz32`, `Math.trunc`, `Math.ceil`, `Math.floor`, `Math.sqrt`, `Math.cbrt`, `Math.pow`, and the new `Math.log2` exact power-of-two literal path are covered by checker/codegen/runtime smoke slices in the current repo, with browser bundle coverage on the documented subset; the statically-known `floor` path now also resolves through const numeric alias chains, and the statically-known `sqrt` / `cbrt` / `log2` / `log10` paths now also resolve through const numeric alias chains in the checker/runtime smoke path, with direct `build` / `run` smoke now also pinning the `Math.log2` / `Math.log10` const alias-chain cases in the Deno `.js` and `.ts` inputs. The limited literal-array `for...of` and `for await...of` slices now share the same lowering path on literal arrays with literal elements, const numeric alias elements, and simple variable bindings, including browser-harness smoke coverage in TS and `.js` input.
