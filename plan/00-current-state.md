...
- Runtime built-in math evidence: `Math.max`, `Math.min`, `Math.abs`, `Math.sign`, `Math.imul`, `Math.round`, `Math.clz32`, `Math.trunc`, `Math.ceil`, and `Math.floor` are covered by the current smoke corpus, and `Math.round` / `Math.floor` / `Math.trunc` / `Math.ceil` now also constant-fold statically-known numeric literals, including the documented alias-chain slices for the supported subcases.
