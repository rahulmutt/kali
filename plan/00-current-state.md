...
- Runtime/reporting lanes: `run`, `test`, `test --coverage`, source-graph effects, package effects, package audit, deterministic JSON envelopes, schema-v1 payload validation, bracketed `Object["keys"]` / `globalThis["Object"]["entries"]` object-enumeration lowering coverage, browser-harness sequence-wrapper coverage over the supported object-enumeration calls, direct `Reflect.ownKeys(...)` iterator lowering over static object-literal slices, and the current string-concatenation iterable slice for `for...of` / `for await...of` over statically known string operands.
