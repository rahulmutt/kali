...
- Browser bundle chunk smoke coverage now also exercises literal and const-bound dynamic imports from `.js` input, and the browser bundle runtime smoke now also exercises the dynamic-import loader for `.js` input including directory-index targets, plus `Math.abs()` semantics in both `.ts` and `.js` inputs, so the browser `build --bundle` lane keeps the linked-graph chunking and builtin-lowering paths aligned with first-class JavaScript compilation.
