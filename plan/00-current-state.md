...
- Host/API slices: Deno-oriented default surface, documented Node subsets (including the read-only `process.pid` query on the Node surface), browser-targeted `check` / `build --bundle`, and browser-harness-assisted `run` / `test` paths where configured.
