...
- Complete guest-facing threaded runtime semantics beyond profile acceptance, while preserving no-GC/no-JIT invariants and resource-budget honesty; browser-targeted check/build rejection coverage for `--wasm-threads` now also spans JS, TS, JSX, and TSX input alongside the browser-harness and browser-bundle work.
