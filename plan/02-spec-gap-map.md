...
- Complete guest-facing threaded runtime semantics beyond profile acceptance, while preserving no-GC/no-JIT invariants and resource-budget honesty; browser-targeted check/build rejection coverage for `--wasm-threads` now also spans JS, TS, JSX, and TSX input alongside the browser-harness and browser-bundle work, and the browser-harness zero-capable `--max-threads 0` budget path now also has direct coverage alongside the positive-override smoke, while browser-requested run/test JS input now also covers the zero-capable `--max-threads 0` / `--max-spawned-processes 0` path.
