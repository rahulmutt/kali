...
- `Object.values()` is now covered alongside the already-covered `Object.keys()` / `Object.entries()` pair, and the runtime-smoke regression set now also checks overwrite ordering for that trio while the array-mutation smoke path covers `pop`/`unshift`; keep both in the runtime-smoke regression set while other object-enumeration gaps are pursued.
