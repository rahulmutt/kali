...
- Expand ECMA-262 conformance for supported syntax and runtime semantics; async/await sequencing now has a dedicated runtime-smoke regression, array iteration now has a minimized gated `for...of` rejection fixture, and a minimized mixed CommonJS/ESM package interop fixture now covers the default standalone surface, but broader generator and CJS/ESM interop coverage still need more minimized fixtures.
...
- Generator lowering still needs a dedicated implementation packet; the parser now accepts async function declarations/expressions, async generator syntax, and generator syntax, including delegated `yield*`, but lowering remains gated at resolution/check time and current runtime smoke coverage is still limited to simpler regression cases such as arithmetic, exceptions (including try/finally sequencing), object enumeration, and built-ins.
