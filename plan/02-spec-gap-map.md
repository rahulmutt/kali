...
- Current progress: the same schema-document test now also asserts the core result payloads for `check`, `run`, `install`, `fmt`, `lint`, `effects`, `build`, and `test`, and now also pins the supporting diagnostic, manifest, lockfile, and sandbox-policy schema documents, widening the drift net across the main command/result/config envelopes without changing any public payload shape.
