...
- Triage late object/runtime APIs such as `Proxy`, weak references, finalization, broader `Intl`, `SharedArrayBuffer`, `Atomics`, and broader object helpers against memory, threading, and optimization constraints before promotion; the shared late-object-model inventory now also carries frozen bracket-root spellings for `WeakRef` and `FinalizationRegistry`, including the single-quoted variants, so the runtime/browser smoke remains aligned on those aliases across the browser JS/JSX/TSX fixture variants. The shared broader-Intl browser JS helper now also carries single-quoted bracketed member spellings for `DateTimeFormat`, `RelativeTimeFormat`, `PluralRules`, `Collator`, `DisplayNames`, `Segmenter`, and `Locale` on the same rejection path.
