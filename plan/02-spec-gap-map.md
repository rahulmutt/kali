...
- Implement or deliberately keep gated high-cost language semantics that are still unavailable despite parser/diagnostic coverage: generator lowering, `for...of` / iterator lowering, `for await...of` execution, `Promise.allSettled`, the remaining unsupported `Math` members (with `Math.floor`, the statically-known perfect-square integer literal path for `Math.sqrt`, and the statically-known perfect-cube integer literal path for `Math.cbrt` now covered on the current integer-only subset), and non-literal `import(expr)`.
