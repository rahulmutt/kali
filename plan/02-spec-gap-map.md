...
- Complete guest-facing threaded runtime semantics beyond profile acceptance, while preserving no-GC/no-JIT invariants and resource-budget honesty; browser-targeted check/build rejection coverage for `--wasm-threads` now also spans JS, TS, JSX, and TSX input alongside the browser-harness and browser-bundle work, and the browser-harness zero-capable `--max-threads 0` budget path now also has direct coverage alongside the positive-override smoke, while browser-requested run/test JS input now also covers the zero-capable `--max-threads 0` / `--max-spawned-processes 0` path, and inherited browser `runtimeProfiles=["wasm-threads"]` browser-harness max-thread smoke now also spans JS, TS, JSX, and TSX input, including the zero-capable `--max-threads 0` budget path on that same input matrix.
...
- Add late host APIs only with explicit effect keys, policy behavior, resource budgets, and command/API-surface gating: subprocesses, sockets/listeners, workers/threads, environment materialization, process control, and late Node/Deno modules. Browser late-compat process-control coverage now also rejects the alias-target zero-probe spelling `globalThis["process"]["kill"](+0)` and the bracketed `process["kill"](+0)` spelling across JS, TS, JSX, and TSX input, plus the receiver-freeze aliases `Object.freeze(process)["kill"](0)` / `Object.freeze(globalThis.process)["kill"](0)` / `Object.freeze(globalThis["process"])["kill"](0)`, and the current smoke set now also keeps the mixed frozen alias `Object.freeze(globalThis["process"].kill)(0)` aligned with the documented Node zero-probe aliases. Browser-targeted build smoke now also carries the bracketed `Object.freeze(globalThis["process"]["kill"])(+0)` alias on the same rejection slice.
