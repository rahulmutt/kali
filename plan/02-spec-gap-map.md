...
- `Math.sign()` now rounds out the existing `Math.max()` / `Math.min()` / `Math.abs()` built-in math coverage on the runtime-smoke path; keep the four-function built-in regression set in sync with the corresponding codegen import tests.
