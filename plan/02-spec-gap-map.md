...
- Continue widening expression/operator, object-model, BigInt, Math, Promise, dynamic-import, reflection, and built-in semantics where translation-safe; the current smoke now also covers the `Math.hypot()` perfect-square-sum slice in browser-requested run/test and browser-bundle JSX/TSX inputs.
