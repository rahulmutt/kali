...
- Add late host APIs only with explicit policy/effect/resource contracts: environment materialization/mutation, process chdir/exit, subprocess spawning, socket/listener APIs, and late Node built-ins. Progress note: bare `check` / `build` / `run` / `test` now explicitly gate the phase-three Deno subprocess and socket/listener APIs (`Deno.Command`, `Deno.connect`, `Deno.listen`, `Deno.serve`) with canonical `E5506` diagnostics until the explicit sandbox/process-budget/network contracts exist. Progress note: the browser late-compatibility env-mutation matrix now also rejects the fully bracketed `globalThis["Deno"]["env"]["set/delete"]` spellings, keeping that rejection lane aligned with the already-covered mixed and partially bracketed forms.
