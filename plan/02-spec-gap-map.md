...
- Progress note: standalone `run` / `test` smoke now also covers the `Math.exp2` zero-identity const-alias slice in JS input with JSON-output coverage, keeping the last host-runtime evidence gap closed for that supported builtin slice.
