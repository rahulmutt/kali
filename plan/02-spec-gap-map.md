...
- Continue applying transparent wrapper handling to cheap numeric-literal Math slices where it stays deterministic; the frozen numeric-literal wrapper slice now joins the existing supported `Math.round` path, including the optional-chain-wrapped `Object.freeze(globalThis?.Math.round)` alias, the parenthesized optional-chain alias `Object.freeze((globalThis?.Math.round))`, plus the bracketed frozen-callable `Object.freeze(globalThis.Math["round"])` / `Object.freeze(globalThis["Math"]["round"])` aliases, the direct frozen `Math.abs` / `Math.sign` aliases now also have browser bundle/harness coverage, and the direct `Object.freeze(Math.floor)` / `Object.freeze(Math.trunc)` / `Object.freeze(Math.ceil)` aliases now also stay on the supported floor/trunc/ceil slice in checker/codegen/runtime smoke coverage.
