...
- Continue applying transparent wrapper handling to cheap numeric-literal Math slices where it stays deterministic; the frozen numeric-literal wrapper slice now joins the existing supported `Math.round` path, including the bracketed frozen-callable `Object.freeze(globalThis.Math["round"])` / `Object.freeze(globalThis["Math"]["round"])` aliases, and the direct frozen `Math.abs` / `Math.sign` aliases now also have browser bundle/harness coverage.
