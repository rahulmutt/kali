...
- Progress note: browser-harness and browser-bundle smoke now also cover the `globalThis.Math` / bracketed `globalThis["Math"]` core math suite in JSX and TSX input, keeping the source-class matrix aligned with the supported TS and `.js` core-suite coverage.
...
- Progress note: standalone `run` / `test` smoke now also covers the `Math.exp2` zero-identity const-alias slice in JS input with JSON-output coverage, and direct build smoke now also covers that zero-identity slice in Deno and browser `.js` / `.ts` inputs, keeping the remaining build-path evidence gap closed for that supported builtin slice.
...
- Progress note: `Object.fromEntries` now also folds the `globalThis.Object.fromEntries` and bracketed `globalThis["Object"]["fromEntries"]` alias forms on the derived-shape object-enumeration path, and the runtime/browser smoke now pins those aliases alongside the existing bare-call coverage.
