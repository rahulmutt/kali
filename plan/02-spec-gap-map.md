...
- Implement full generator and async-generator execution semantics, including resumable state machines, `yield`, `yield*`, `return`, `throw`, async interaction, and finalization; keep unsupported forms behind canonical gates until then. Current browser harness smoke also mirrors the sequence-wrapped generator and async-generator rejection on the yield* slice across JS, TS, JSX, and TSX input, and build/check smoke now also pins sequence-wrapped generator and async-generator class-expression rejection on the same gate, with direct run/test smoke now also covering the sequence-wrapped yield* class-expression slice across JS, TS, JSX, and TSX input, and browser harness `check`/`run`/`test`/`build` now also rejecting anonymous default-export generator and async-generator declarations across JS, TS, JSX, and TSX input, plus sequence-wrapped anonymous default-export generator and async-generator declarations on the same matrix. The library-export surface now also rejects sequence-wrapped generator default-export expressions through the same canonical E5506 gate.
- Expand iterator and async-iterator protocol behavior beyond bounded static slices, including lookup, `next` result handling, abrupt completion, close/finalization, and async protocol behavior; current smoke also covers parenthesized direct constructor aliases for the supported `Set` / `Map` slices such as `new (globalThis["Set"])` and `new (globalThis['Map'])`, and the frozen-callable constructor inventories now also carry nullish/logical wrapper aliases around the root and `globalThis` constructor forms. The shared `Array.from` helper inventory now also carries the parenthesized frozen bracket-root aliases around `globalThis["Array"]` and `globalThis['Array']`, plus the sequence-wrapped transparent aliases around `Array.from` and `globalThis.Array.from`, and the shared object-enumeration helper inventory now also canonicalizes the parenthesized receiver-property `Object.keys` / `Object.values` / `Object.entries` alias forms for both double- and single-quoted `globalThis["Object"]` roots, including the single-quoted and double-quoted parenthesized receiver-bracketed spellings that now ride the same smoke corpus; the canonical helper inventory now also carries the matching parenthesized bracket-root `["keys"]` / `["values"]` / `["entries"]` aliases for both quote-root spellings, plus the direct mixed-quote `Object.freeze(globalThis["Object"]['keys'])` / `Object.freeze(globalThis['Object']["keys"])`-style frozen callable aliases already used in the smoke corpus, and now also includes the direct double-quoted bracket-root `Object.freeze(globalThis["Object"]["keys"])` alias alongside the existing values/entries forms, and the browser-requested `for await` helper path now also mirrors the parenthesized single-quoted receiver-property `Object.freeze((globalThis['Object']).keys|values|entries)` variants in JS-like input, with the browser-object-values smoke now also covering the mixed double/single-quoted `globalThis["Object"]['values']` alias family.
- Continue widening expression/operator, object-model, BigInt, Math, Promise, dynamic-import, reflection, and built-in semantics where translation-safe; the current smoke now also covers the `Math.hypot()` perfect-square-sum slice in browser-requested run/test and browser-bundle JSX/TSX inputs, and browser-targeted build smoke now also covers the same slice in JSX and TSX inputs, and browser-targeted build smoke now also covers the `Math.floor` / `Math.trunc` / `Math.ceil` alias-chain and bracketed literal slices in JSX and TSX input on the browser API-surface path, and the frozen `Math.hypot` inventory now also carries the parenthesized bracket-root and single-quoted bracket-root aliases in the browser-harness/browser-bundle smoke, plus the parenthesized root-object `Object.freeze((globalThis["Math"]))["hypot"]` / `Object.freeze((globalThis['Math']))["hypot"]` aliases and the parenthesized dot-root `Object.freeze((globalThis.Math))["hypot"]` / `Object.freeze((globalThis.Math))['hypot']` aliases, and the `Math.cbrt` frozen-callable inventory now also carries the parenthesized receiver-wrapped bracket-root spellings around `globalThis["Math"]` and `globalThis['Math']`, plus the corresponding parenthesized root-object `Object.freeze((globalThis["Math"]))["cbrt"]` / `Object.freeze((globalThis['Math']))["cbrt"]` aliases and the parenthesized dot-root `Object.freeze((globalThis.Math))["cbrt"]` / `Object.freeze((globalThis.Math))['cbrt']` aliases, and browser harness/bundle smoke now also covers frozen callable `Math.sqrt` / `Math.cbrt` aliases across `.js` / `.ts` / `.jsx` / `.tsx` input, and the Promise.any smoke now also includes the mixed-bracket frozen-callable alias family around `globalThis["Promise"]['any']` plus the parenthesized receiver-wrapped bracket-root aliases around `globalThis["Promise"]` / `globalThis['Promise']` and the dotted-root bracketed frozen-callable aliases around `globalThis.Promise["any"]` / `globalThis.Promise['any']`; Promise.allSettled smoke now also includes the parenthesized bracket-root frozen aliases around `globalThis["Promise"]` and the single-quoted sibling alias on the shared browser-harness/browser-bundle slice, and Promise.race smoke now also covers bracketed `globalThis["Promise"]` / `globalThis['Promise']` aliases plus frozen bracketed aliases on the shared browser body, and now also carries the corresponding fully bracketed `globalThis["Promise"]["race"]` / `globalThis['Promise']['race']` forms plus parenthesized frozen wrappers in the shared browser body. The floor/trunc/ceil helper inventory now also carries the parenthesized root-object aliases around `globalThis.Math`, `globalThis["Math"]`, and `globalThis['Math']` on that same supported slice, and the browser-harness/browser-bundle object-enumeration spread slice now also covers the parenthesized single-quoted receiver-property `Object.freeze((globalThis['Object']).keys)` variant alongside the existing bracketed forms, while the direct `Object.values(...)` iteration slice now also mirrors the single-quoted receiver-property frozen alias on that same supported helper path.
- Array callback methods such as `find`, `findIndex`, `findLast`, `findLastIndex`, `map`, `filter`, `some`, `every`, `reduce`, `reduceRight`, and `flatMap` remain on the canonical `E5506` gate path in the direct runtime until faithful callback lowering is implemented; current lowering and CLI smoke now also pin the full rejection matrix across build/check/run/test entrypoints, and browser-requested run/test harness coverage now also pins the same gate in JS input. The shared object-enumeration helper inventory now also tracks the direct dot-root bracketed `globalThis.Object["keys"]` / `globalThis.Object["values"]` / `globalThis.Object["entries"]` frozen-callable aliases on the wrapped-object slice, keeping the helper sources aligned with the browser harness regressions.
...
- Triage late object/runtime APIs such as `Proxy`, weak references, finalization, broader `Intl`, `SharedArrayBuffer`, `Atomics`, and broader object helpers against memory, threading, and optimization constraints before promotion; the shared late-object-model inventory now also carries frozen bracket-root spellings for `WeakRef` and `FinalizationRegistry`, including the single-quoted variants, so the runtime/browser smoke remains aligned on those aliases across the browser JS/JSX/TSX fixture variants, and the direct source-resolution regression now also covers single-quoted `Proxy.revocable` spellings plus the mixed-quote bracket-access `globalThis['Proxy']["revocable"]` form and parenthesized receiver-wrapped frozen wrappers on that same late-object-model path. The shared broader-Intl browser JS helper now also carries single-quoted bracketed member spellings for `DateTimeFormat`, `RelativeTimeFormat`, `PluralRules`, `Collator`, `DisplayNames`, `Segmenter`, and `Locale` on the same rejection path, now including the matching single-quoted-root `globalThis['Intl'].<member>` and `globalThis['Intl']["<member>"]` forms for the same member set, plus the fully bracketed double-quoted `globalThis["Intl"]["..."]` variants in `kali_common`, and that helper inventory is now canonicalized and unit-tested in `kali_common`; the shared late-threaded-runtime inventory now also carries direct dot-root `true &&` / `false ||` wrapper variants for `SharedArrayBuffer` and `Atomics`, and the browser JS/JSX/TSX source inventories now also mirror those wrapper forms, keeping the threaded rejection corpus symmetric with the rest of the canonical alias families.
