...
- Generator lowering still needs a dedicated implementation packet; the parser now accepts generator syntax, but lowering remains gated at resolution/check time and current runtime smoke coverage is still limited to simpler regression cases such as arithmetic, exceptions, object enumeration, and built-ins.
