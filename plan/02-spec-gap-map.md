...
- Current progress: the default standalone utility corpus now also mirrors its minimized mixed-format CommonJS/ESM interop fixture on `.js` input for both `run` and `test`, keeping first-class JavaScript package evidence aligned with the existing TypeScript lane instead of only covering the TS form.
