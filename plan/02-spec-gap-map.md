...
- Continue widening expression/operator and built-in semantics where translation-safe; keep unsupported `Math`, object-model, dynamic import, compound-assignment, and dynamic language forms explicitly gated. The current repository snapshot now also covers logical `&&=` / `||=` assignment and the wrapped mutable-local arithmetic compound-assignment slice on direct JS/TS, browser-harness, and browser-bundle smoke.
