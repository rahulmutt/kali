...
- Browser bundle runtime smoke now also exercises `Math.max()` / `Math.min()` / `Math.abs()` / `Math.sign()` semantics in both TS and `.js` input, extending the browser bundle evidence beyond the existing dynamic-import and crypto smoke cases.
