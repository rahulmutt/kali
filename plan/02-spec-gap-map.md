...
- Continue widening expression/operator, object-model, BigInt, Math, Promise, dynamic-import, reflection, and built-in semantics where translation-safe. The static literal-array search helper slice now also pins omitted-`fromIndex` `lastIndexOf` semantics to search from the array tail, and the static ASCII string search helper slice now covers statically-known `includes` / `indexOf` / `lastIndexOf` operands while keeping dynamic search/fromIndex and non-ASCII cases gated for statically-known string receivers.
