...
- Implement or deliberately keep gated high-cost language semantics that are still unavailable despite parser/diagnostic coverage: generator lowering, `for...of` / iterator lowering, `for await...of` execution, nullish coalescing runtime semantics, `Promise.allSettled`, unsupported `Math` members (with `Math.floor` now covered on the current integer-only subset), and non-literal `import(expr)`.
