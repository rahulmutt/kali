...
**Status:** Complete for the historical Phase-3 target, with the current repo now exposing the documented Node execution subset for `run` / `test` while keeping `check`, `build`, `effects`, and package-analysis gating explicit.

The current regression suite now pins `E5506` for explicit and inherited `--api node` requests across `check`, `build`, `run`, and `test`, while `effects` / `package-effects` now also honor the documented Node analysis context and representative package-corpus cases continue to exercise the Node execution subset. The package-effects smoke lane also now covers the inherited browser analysis context so the registry-analysis report keeps the browser-configured selection path exercised alongside the Node path. The semver-style package-bin coverage now includes a positive Node execution lane, and the semver probe now also exercises `process.argv.slice(2)` passthrough while the CLI strips the `--` separator before constructing Node runtime argv, so the Node subset keeps the common CLI argument pattern honest instead of only the raw length accessor. `package-audit` continues to ignore inherited Node context as a context-free registry-analysis command and still rejects an explicit `--api node` request before any registry lookup can occur. The browser-bundle smoke harness still keeps the shared `kali:rt` console shims (`console_log`, `console_error`, `console_warn`) in place so emitted browser bundles instantiate cleanly under the Node-based harness.
