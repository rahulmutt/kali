...
synchronous `util.promisify` bridge. Install-time package host-fit validation now respects
`compilerOptions.apiSurface = "node"`, so Node-targeted projects can accept Node-only builtins
while the default standalone context still rejects them. Runtime wiring and `--api node`
enablement are still pending.
