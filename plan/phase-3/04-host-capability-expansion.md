...
- CLI smoke coverage now exercises the spawned-process budget on both `run` and `test`,
  covering the zero-cap deny/tightening path and the positive budget acceptance lane so the
  host-capability milestone has symmetrical evidence instead of only the rejection cases.
