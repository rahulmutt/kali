...
- Progress note: the `Deno.permissions.query` descriptor-object smoke now also covers `.jsx` and `.tsx` inputs on the direct check/build/run/test paths, keeping the query-only facade aligned across the browser-capable source-class matrix and the canonical `E5506` gate for unsupported descriptors.
...
- Progress note: `Math.exp2` now also has the exact zero-identity fold on the supported codegen/runtime smoke path, and browser-harness/browser-bundle smoke now also covers the `globalThis.Math.exp2` zero-identity slice in JSX and TSX input alongside the existing TS and `.js` coverage; direct build smoke now also covers that same zero-identity slice in JSX and TSX input on the Deno and browser surfaces; browser-harness/browser-bundle JSON success paths for that `globalThis.Math.exp2` root slice now also assert empty `errors` / `warnings` arrays, keeping the deterministic JSON envelope explicit on the supported slice.
...
- Progress note: browser-harness and browser-bundle smoke now also cover the bracketed `globalThis["Math"]` root for the `Math.sinh` / `Math.cosh` / `Math.tanh` zero-identity slice in JSX and TSX input.
...
- Progress note: browser-requested run/test coverage on the browser API surface now also rejects generator and async-generator spellings in `.js`, `.jsx`, `.tsx`, and `.ts` input with canonical E5506 errors, keeping the runtime smoke aligned with the existing generator gate; browser-targeted build smoke now also rejects async-generator spellings in TS, `.js`, `.jsx`, and `.tsx` input on both the Deno and browser surfaces.
...
- Progress note: the browser-requested unreadable-summary fallback smoke now also covers `.ts` input on the supported JSON `test` and human `run` / `test` paths, and the dedicated JS-input regression file now also mirrors that unreadable fallback on the browser-requested run/test paths, keeping the stdout-authoritative rule aligned across source classes; that JS-input file now also covers numeric invalid `testsFailed` values alongside the existing string-shape fallback.
- Progress note: the browser-requested whitespace-only summary fallback smoke now also covers `.ts` and `.tsx` input on the supported JSON `test` path, keeping the blank-summary fallback aligned across the browser-capable source classes. The browser-requested human-output `run` path now also covers empty summary-file fallback across JS, TS, JSX, and TSX input, and the browser-requested JSON `run` path now also covers that empty-summary fallback across JS, TS, JSX, and TSX input.
