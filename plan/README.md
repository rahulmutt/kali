...
- Progress note: browser-requested run/test coverage on the browser API surface now also rejects generator and async-generator spellings in `.js`, `.jsx`, and `.tsx` input with canonical E5506 errors, keeping the runtime smoke aligned with the existing generator gate; browser-targeted build smoke now also rejects async-generator spellings in TS, `.js`, `.jsx`, and `.tsx` input on both the Deno and browser surfaces.
