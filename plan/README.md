...
- Progress note: the browser-requested whitespace-only summary fallback smoke now also covers `.ts` and `.tsx` input on the supported JSON `test` path, keeping the blank-summary fallback aligned across the browser-capable source classes. The browser-requested human-output `run` path now also covers empty summary-file fallback across JS, TS, JSX, and TSX input.
