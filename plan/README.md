...
- Progress note: the browser-requested unreadable-summary fallback smoke now also covers `.ts` input on the supported JSON `test` and human `run` / `test` paths, and the dedicated JS-input regression file now also mirrors that unreadable fallback on the browser-requested run/test paths, keeping the stdout-authoritative rule aligned across source classes.
