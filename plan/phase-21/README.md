...
- Implement callback-bearing array methods such as `find`, `findIndex`, `findLast`, `findLastIndex`, `map`, `filter`, `some`, `every`, `reduce`, `reduceRight`, and `flatMap` only with faithful callback, abrupt-completion, and finalization behavior. The supported literal-array identity `values.map((value) => value)` slice is now live, and the literal-array `values.flatMap((value) => [value])` slice now also lives; keep the remaining callback-bearing methods gated until their faithful lowering exists.
