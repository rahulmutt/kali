...
- The Node `process.kill(0)` liveness-probe slice now also keeps transparent parenthesized and unary-plus wrapper forms covered alongside the bracketed Node aliases across JS, TS, JSX, and TSX smoke input, the Node package-corpus evidence now also mirrors the bracketed and `globalThis` alias forms, and the dedicated Node API-surface support regression now mirrors that same wrapper/alias matrix in its `run` and `test` inputs, including TS/TSX satisfies-wrapped zero literals on the checker and CLI surfaces and bracketed `globalThis.process["kill"]` / `globalThis["process"]["kill"]` zero-probe spellings. The zero-probe wrapper regression now also covers `(0 as number)` assertions in TS and TSX input, and the type-resolution regression set now also mirrors that Node zero-probe matrix directly on the checker path.
