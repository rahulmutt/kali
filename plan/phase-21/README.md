...
- Widen object-model, Math, BigInt, dynamic import, reflection, and operator semantics only when observable JavaScript behavior is pinned; current coverage also includes standalone `run` / `test` smoke for the `Number.isFinite` / `Number.isNaN` / `Number.isInteger` / `Number.isSafeInteger` primitive-value slice across JS and TS input with JSON-output coverage, plus browser-bundle smoke for that same slice across JS, TS, JSX, and TSX input, including mixed bracket/dot spellings such as `globalThis["Number"].isNaN`, and the fully bracketed `globalThis["Math"]["sqrt"]` / `globalThis["Math"]["cbrt"]` math slice now also covers JS, TS, JSX, and TSX input on the browser bundle/harness paths. Promise.allSettled now also has standalone build/runtime and browser-requested browser-harness coverage across JS, TS, JSX, and TSX input. Nullish coalescing smoke now also covers `void 0 ??` and `undefined ??` fallback operands on the standalone run/test paths and the browser-requested browser-harness paths in JS input. The same object-identity slice now also has type-system regression coverage for the `Object.is` alias spellings used by the browser/runtime smoke paths, including parser-backed bracketed `Object.is` and `Number.is*` alias spellings in JS input, and browser-harness `Object.is` same-reference alias-chain smoke now also exercises the bracketed, mixed, root, and dot alias spellings alongside the frozen-object variant; dedicated browser-harness alias-chain smoke now also covers object/array same-reference, frozen-reference, and fresh-literal comparisons across JS, TS, JSX, and TSX input. The browser-requested `Math.atan2` smoke now also covers the single-quoted bracketed zero slice in JS, TS, JSX, and TSX input with JSON-output coverage, the unsupported-math browser-harness rejection smoke now also covers the bracketed `Math["sqrt"]` / `Math["atan2"]` and `globalThis["Math"].sqrt` / `globalThis["Math"].atan2` spellings in JSX and TSX input, and the browser `Math.round` smoke now also includes the fully bracketed `globalThis["Math"]["round"]` spelling on the bundle and harness paths. The browser-harness `Math.atan2` trailing-argument evaluation smoke now also covers TS, JSX, and TSX input with JSON-output coverage. Type-resolution smoke now also covers sequence-wrapped call targets for supported object-model and Math helpers in JS input, while the parser now also canonicalizes transparent wrapper forms around computed object-property keys when they still resolve to static string/number names, including unary numeric wrappers, and the type-resolution regression suite now also mirrors the `Number.isSafeInteger` bracketed alias family in JS input. Diagnostic wording now matches that supported primitive/reference slice across the checker and codegen paths.
