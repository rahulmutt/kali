...
- Widen object-model, Math, BigInt, Promise, dynamic import, reflection, and operator semantics only when observable JavaScript behavior is pinned. Current smoke now also covers the single-quoted receiver-wrapped `Math.floor` / `Math.trunc` / `Math.ceil` frozen-callable aliases on the supported helper slice.
