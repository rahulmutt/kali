...
- Added a dedicated determinism smoke lane in `scripts/check-determinism.sh` plus a matching `mise` task and CI job, so the repeated-build evidence is now exercised as an explicit repository workflow instead of living only inside the larger runtime smoke suite. The lane now also pins the `effects`, `package-effects`, and `package-audit` repeated-invocation envelopes, and `package-audit` now also has repeated pretty JSON envelope coverage under quiet mode, keeping the later public reporting surfaces deterministic alongside the build artifacts.
