...
- Added CLI integration regressions that normalize duplicate `--compat` inputs on `effects`, whitespace-padded inherited `compat.features` on `package-effects`, and the manifest/config duplicate-compat rejection path, so the semantic-analysis axes stay canonically trimmed and deduplicated at the command boundary instead of only in unit-level helpers.
