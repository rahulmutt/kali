...
- Added a `date-fns`-style package-corpus regression that exercises named exports plus a subpath export-map entry on the default standalone surface, so the curated pure JS/TS corpus now has another representative real-world package shape pinned across check/build/run.
...
- Added an `effects` JSON-output regression that keeps inherited browser analysis context aligned with a top-level sandbox path, so the browser-resolution and sandbox-attachment axes stay pinned together in the source-graph effect-report lane as well.
- Added a dedicated determinism smoke lane in `scripts/check-determinism.sh` plus a matching `mise` task and CI job, so the repeated-build evidence is now exercised as an explicit repository workflow instead of living only inside the larger runtime smoke suite. The lane now also pins the `effects`, `package-effects`, and `package-audit` repeated-invocation envelopes, including the quiet-mode pretty JSON envelope variants that keep the browser-context effects path and the package-analysis reporting lanes deterministic alongside the build artifacts. It now also checks the default `effects` JSON payload path directly so the native-JSON reporting mode stays byte-stable even without `--output json`. The lane now also pins the default `effects` output under inherited browser analysis context, so the browser-context native-JSON path stays deterministic alongside the existing envelope checks. The lane now also covers the quiet pretty JSON package-audit regressions under inherited browser context and with a top-level sandbox config path, so the registry-audit lane stays pinned across the last remaining presentation-control/browser-policy combinations. The lane now also covers repeated `effects` and `package-effects` JSON-envelope invocations under inherited browser context with a top-level sandbox config path, keeping the source-graph and registry-analysis report lanes deterministic across that last browser/policy presentation combination too.
...
- Added CLI integration regressions that normalize duplicate and whitespace-padded `compat.features` inputs in both `effects` and `package-effects` JSON output, plus whitespace-padded `runtimeProfiles` inputs in `effects`, so the semantic-analysis axes stay canonically trimmed and deduplicated at the command boundary instead of only in unit-level helpers.
...
- Added an inherited-browser browser-bundle regression that combines `--bundle`, `--validate-ir`, and `--sandbox`, so the browser-targeted evidence now pins the inherited config path with the same validation/policy combination already covered explicitly.
...
- Added package-audit quiet pretty-JSON determinism under inherited browser analysis so the browser-context registry-audit lane now stays pinned across repeated pretty envelope runs as well, not just the plain JSON envelope path.
- Added a companion package-audit quiet pretty-JSON regression that also carries a top-level sandbox config path, so the browser-context registry-audit lane stays deterministic even when the manifest includes unrelated policy-attachment plumbing.
- Added a package-audit JSON-output regression that combines inherited Node analysis context with a top-level sandbox config, so the registry-audit lane stays orthogonal across the documented node-and-policy axes instead of only checking them independently.
...
- Added a Node-path semver package-bin smoke that exercises `require('../package.json').version` and guest-argument counting on the documented Node subset, and the help-path regression now pins the exact `Usage: semver ...` output as well, so the semver probe covers the package-json loading slice, the argument passthrough slice, and the no-args help slice directly.
...
- Added browser entrypoint coverage for the base Chrome and Google Chrome wrapper families (`chrome`, `chrome-beta`, `chrome-canary`, `chrome-dev`, `chrome-unstable`, `google-chrome`, `google-chrome-beta`, `google-chrome-canary`, `google-chrome-dev`, and `google-chrome-unstable`) so the alias table stays pinned across the broader Chromium family without widening the browser-runtime contract.
...
- Added browser entrypoint coverage for additional Firefox wrapper spellings (`firefox`, `firefox-beta`, `firefox-nightly`, `firefox-developer-edition`, `firefox developer edition`, and `firefox beta`) so the browser-harness alias table stays aligned with the broader Firefox family without changing the browser-runtime contract.
