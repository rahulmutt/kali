...
|---|---|---|---|
| 21 | Semantic completeness and conformance | now | supported vs gated semantics are backed by conformance, minimized regressions, and canonical diagnostics; current continuation work now also includes the Node `process.kill(0)` liveness-probe slice, including wrapped `process` and wrapped `process.kill` call-target forms plus the mixed frozen `Object.freeze(globalThis["process"].kill)(0)` alias and its receiver-freeze `+0` siblings, alongside the existing host-control gating work, and the browser late-compat JS fixture now reuses the shared zero-probe helper inventory too |
