...
|---|---|---|---|
| 21 | Semantic completeness and conformance | now | supported vs gated semantics are backed by conformance, minimized regressions, and canonical diagnostics; current continuation work now also includes the Node `process.kill(0)` liveness-probe slice, including wrapped `process` and wrapped `process.kill` call-target forms plus the mixed frozen `Object.freeze(globalThis["process"].kill)(0)` alias, its receiver-freeze `+0` siblings, and the newer parenthesized receiver-freeze spellings around `globalThis.process["kill"]` / `globalThis["process"]["kill"]`, while the dedicated late-process-control dot-receiver helper stays spliced in separately for `Object.freeze((process).kill)(0/+0)`, alongside the existing host-control gating work, and the browser late-compat JS fixture now reuses the shared zero-probe helper inventory too, including the shared parenthesized receiver-freeze helper directly; the browser late-compat browser fixture regressions now explicitly pin the single-quoted parenthesized freeze-wrapper symmetry slice for `Object.freeze((globalThis['process'].kill))(0/+0)` and the matching `['kill']` wrapper forms; the browser harness/bundle `Math.pow` alias-chain slice also now includes parenthesized freeze-wrapper spellings around `globalThis.Math["pow"]` and `globalThis["Math"]["pow"]`, and the browser bundle/browser harness bracketed-root slice now also exercises the parenthesized freeze-wrapper aliases for `globalThis.Math["pow"]` and `globalThis["Math"]["pow"]` |
