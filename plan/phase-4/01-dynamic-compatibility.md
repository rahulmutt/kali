...
- Package-audit findings are now sorted into a deterministic emission order before the CLI splits them into errors and warnings, so mixed finding sets stay byte-stable even if the underlying audit helper evolves.
- The human-output path now preserves that same deterministic finding order, so the summary on stdout and the detailed diagnostics on stderr stay aligned with the JSON envelope behavior.
- Added a lifecycle-script ordering regression so package-audit keeps reporting `preinstall`, `install`, and `postinstall` in the canonical phase order when multiple install-time scripts are present.
