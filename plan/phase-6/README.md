...
- Follow-up: object-enumeration coverage should keep the overwrite-ordering regression pinned alongside a minimized numeric-key limitation fixture, including mirrored `.js` input coverage where practical, so the supported string-key semantics stay minimized while the integer-like ordering gap remains explicit.
