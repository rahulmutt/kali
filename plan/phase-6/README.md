...
- Current progress: parser now accepts generator function syntax and `yield` expressions as AST forms, while the checker and CLI smoke tests still gate generator lowering with the canonical `E5506` availability path instead of letting it reach codegen; that gate is now covered on both `check` and `build` smoke paths.
