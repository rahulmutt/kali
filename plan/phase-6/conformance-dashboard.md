...
| Shared web-baseline primitives (`structuredClone`, `AbortController`, `AbortSignal`, `EventTarget`, `CustomEvent`, `URL`, `URLSearchParams`, `TextEncoder`, `TextDecoder`) in `.ts` and `.js` input | Phase 1 MVP | `crates/kali_cli/tests/runtime_smoke.rs` (run/test, including JSON-output coverage) |
...
| Browser timing baseline / `performance.now()` monotonic ordering in `.ts` and `.js` input | Phase 1 MVP | `crates/kali_cli/tests/runtime_smoke.rs` (browser-requested `run` / `test` and browser bundle smoke coverage, including inherited-browser-api-surface coverage, JSON-output coverage for the browser-requested `.js` `run` lane, the inherited browser-api-surface `.js` run lane, the browser-requested `.js` `test` lane and the browser-requested `.ts` `test` lane, plus JSON-output coverage for the browser bundle `.js` lane) |
...
| Browser-requested `run` / `test` object-enumeration semantics in `.ts` and `.js` input, including string-primitive enumeration in `.js` input and JSON-output coverage | Phase 1 MVP | `crates/kali_cli/tests/runtime_smoke.rs` |
...
| Environment snapshot materialization (`Deno.env.toObject`) | Later compatibility | Canonical later object-materialization gate; `crates/kali_cli/tests/runtime_smoke.rs`, `crates/kali_cli/tests/late_compat_js_input.rs` |
