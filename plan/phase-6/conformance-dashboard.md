...
| Literal-string `import()` lowering over the linked graph, including directory-index targets | Phase 3 target | `crates/kali_types/src/tests.rs`, `crates/kali_cli/src/build_tests.rs`, `crates/kali_cli/tests/runtime_smoke.rs` (standalone run/test TS and `.js` input, including JSON-output coverage for the `test` lane, browser-requested `run` / `test` TS and `.js` input with JSON-output coverage on the browser-requested `test` lane, plus browser-targeted bundle smoke) |
