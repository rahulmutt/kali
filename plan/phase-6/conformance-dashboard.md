...
| Read-only `Deno.permissions.query(...)` over the shared descriptor subset (`read`, `write`, `env`, `net`), including bracketed `query` access forms and const-bound descriptor aliases in `.js` input | Phase 1 MVP | `crates/kali_types/src/tests.rs`, `crates/kali_sandbox/src/tests.rs`, `crates/kali_cli/tests/runtime_smoke.rs`, `crates/kali_cli/tests/permission_query_js_input.rs`, `crates/kali_api_deno/src/tests.rs` |
