...
| Type annotation resolution accepts known names in `.jsx` and `.tsx` input | Phase 1 MVP | `crates/kali_types/src/tests.rs` |
| TypeScript assertion / satisfies expressions (`as`, `satisfies`) | Phase 1 MVP | `crates/kali_parser/src/tests.rs`, `crates/kali_types/src/tests.rs`, `crates/kali_cli/tests/runtime_smoke.rs` |
...
| Read-only `Deno.permissions.query(...)` over the shared descriptor subset (`read`, `write`, `env`, `net`), including bracketed `query` access forms and const-bound descriptor aliases in `.ts` and `.js` input | Phase 1 MVP | `crates/kali_types/src/tests.rs`, `crates/kali_sandbox/src/tests.rs`, `crates/kali_cli/tests/runtime_smoke.rs`, `crates/kali_cli/tests/permission_query_js_input.rs`, `crates/kali_cli/tests/permission_query_ts_input.rs`, `crates/kali_api_deno/src/tests.rs` |
| Read-only `Deno.permissions.query(...)` const-bound descriptor aliases in `.ts` and `.js` input | Phase 1 MVP | `crates/kali_types/src/tests.rs`, `crates/kali_cli/tests/permission_query_js_input.rs`, `crates/kali_cli/tests/permission_query_ts_input.rs`, `crates/kali_cli/tests/runtime_smoke.rs` |
| Compile/check-time inferred-effect-vs-policy validation on `check` / `build --sandbox` | Phase 2 target | `crates/kali_cli/tests/runtime_smoke.rs` (TS and `.js` input, including JSON-output coverage on the `.js` lanes) |
...
| Basic async/await sequencing | Phase 1 MVP | `crates/kali_cli/tests/runtime_smoke.rs` (including mirrored `.ts` and `.js` input coverage on the `run` and `test` paths, plus JSON-output coverage in both source classes) |
| Browser-requested `run` / `test` basic unary prefix semantics (`!`, unary `-`, unary `+`, and `void`) in `.ts` and `.js` input, including JSON-output coverage | Phase 1 MVP | `crates/kali_cli/tests/runtime_smoke.rs` |
| Promise.all sequencing over already-resolved promises in `.ts` and `.js` input | Phase 1 MVP | `crates/kali_cli/tests/runtime_smoke.rs` (run/test) |
| Browser-requested `run` / `test` Promise.all sequencing over already-resolved promises in `.ts` and `.js` input, including inherited-browser-api-surface coverage and JSON-output coverage | Phase 1 MVP | `crates/kali_cli/tests/runtime_smoke.rs` |
| Shared web-baseline primitives (`structuredClone`, `AbortController`, `AbortSignal`, `Event`, `EventTarget`, `CustomEvent`, `URL`, `URLSearchParams`, `TextEncoder`, `TextDecoder`) in `.ts` and `.js` input | Phase 1 MVP | `crates/kali_types/src/tests.rs`, `crates/kali_cli/tests/runtime_smoke.rs` (run/test, including JSON-output coverage) |
| Browser ambient typing for baseline host globals (`fetch`, `Headers`, `Request`, `Response`, `Blob`, `File`, `performance`, `crypto`) in `.ts` and `.js` input | Phase 1 MVP | `crates/kali_types/src/tests.rs` |
| Browser-requested `run` / `test` shared web-baseline primitives (`structuredClone`, `AbortController`, `AbortSignal`, `Event`, `EventTarget`, `CustomEvent`, `URL`, `URLSearchParams`, `TextEncoder`, `TextDecoder`) in `.ts` and `.js` input, including inherited-browser-api-surface coverage and JSON-output coverage | Phase 1 MVP | `crates/kali_cli/tests/runtime_smoke.rs` |
| Browser-requested `run` / `test` shared web-baseline primitives (`structuredClone`, `AbortController`, `AbortSignal`, `Event`, `EventTarget`, `CustomEvent`, `URL`, `URLSearchParams`, `TextEncoder`, `TextDecoder`) when the browser API surface is inherited in `.ts` and `.js` input, including JSON-output coverage | Phase 1 MVP | `crates/kali_cli/tests/runtime_smoke.rs` |
| Browser bundle shared web-baseline primitives (`structuredClone`, `AbortController`, `AbortSignal`, `Event`, `EventTarget`, `CustomEvent`, `URL`, `URLSearchParams`, `TextEncoder`, `TextDecoder`) in `.ts` and `.js` input | Phase 1 MVP | `crates/kali_cli/tests/runtime_smoke.rs` (including JSON-output coverage) |
...
| Browser-requested `run` / `test` queueMicrotask ordering when the browser API surface is inherited in `.ts` and `.js` input, including JSON-output coverage | Phase 1 MVP | `crates/kali_cli/tests/runtime_smoke.rs` |
| Read-only `Deno.pid` / `Deno["pid"]` / `globalThis.Deno.pid` handling in `.js` input | Phase 1 MVP | `crates/kali_cli/tests/runtime_smoke.rs` (run/test, including JSON-output coverage on the `test` lane) |
...
| Browser bundle runtime object type / constructor semantics (`typeof`, `instanceof`) in `.ts` and `.js` input, including JSON-output coverage | Phase 1 MVP | `crates/kali_cli/tests/runtime_smoke.rs` |
...
| Basic unary prefix semantics (`!`, unary `-`, unary `+`, and `void`) in `.ts` and `.js` input, including JSON-output coverage on the standalone `run` / `test` lanes | Phase 1 MVP | `crates/kali_cli/tests/runtime_smoke.rs` |
...
| Browser bundle `Math.ceil()` semantics in `.ts` and `.js` input, including JSON-output coverage in both source classes | Phase 1 MVP | `crates/kali_cli/tests/runtime_smoke.rs` |
...
| Browser bundle `queueMicrotask` ordering in `.ts` and `.js` input, including JSON-output coverage | Phase 1 MVP | `crates/kali_cli/tests/runtime_smoke.rs` (including JSON-output coverage in `.ts` and `.js` input) |
...
| Browser-requested `run` / `test` object-enumeration semantics when the browser API surface is inherited in `.ts` and `.js` input, including overwrite/delete-reinsert ordering plus string-primitive enumeration in `.js` input and JSON-output coverage | Phase 1 MVP | `crates/kali_cli/tests/runtime_smoke.rs` |
...
| Browser-requested `run` / `test` integer-like key ordering semantics when the browser API surface is inherited in `.ts` and `.js` input, including JSON-output coverage | Phase 1 MVP | `crates/kali_cli/tests/runtime_smoke.rs` |
...
| Browser-requested `run` / `test` boolean conjunction / disjunction semantics when the browser API surface is inherited in `.js` input | Phase 1 MVP | `crates/kali_cli/tests/runtime_smoke.rs` (including JSON-output coverage) |
...
| Browser-requested `run` / `test` `Math.clz32()` semantics when the browser API surface is inherited in `.ts` input, including JSON-output coverage | Phase 1 MVP | `crates/kali_cli/tests/runtime_smoke.rs` |
...
| Browser-requested `run` / `test` `Math.clz32()` semantics when the browser API surface is inherited in `.js` input, including JSON-output coverage | Phase 1 MVP | `crates/kali_cli/tests/runtime_smoke.rs` |
...
| Basic `Math.ceil()` built-in semantics | Phase 1 MVP | `crates/kali_codegen/src/tests.rs`, `crates/kali_cli/tests/runtime_smoke.rs` (run/test, `.js` input, including JSON-output coverage) |
...
| Feature | Status | Evidence |
|---|---|---|
| Open-ended or unstable cross-module/public-API constraint solving | Phase 3 target | Canonical `E5506`/annotation-required boundary in `specs/04-type-system.md` and `specs/19-feature-maturity.md`; public export-boundary negative coverage in `crates/kali_types/src/tests.rs` and `crates/kali_cli/tests/runtime_smoke.rs` (including JSON-output `check` coverage for unresolved identifiers inside default-exported function bodies in `.js` input) |
...
| `for await...of` array iteration | Rejected by default | Canonical iterator-lowering gate (`E5506`); `crates/kali_types/src/tests.rs`, `crates/kali_cli/tests/runtime_smoke.rs` (TS and `.js` `check` / `build` / `run` / `test` coverage, plus browser-targeted `check` / `build --bundle` and browser-requested `run` / `test` `.js` coverage, including JSON-output coverage) |
...
| Mutable environment access (`Deno.env.set`, `Deno.env.delete`, `process.env = ...`-style host mutation) | Phase 3 target | Canonical host-mutation gate (`E5506`); `crates/kali_runtime/src/tests.rs`, `crates/kali_cli/tests/late_compat_js_input.rs`, `crates/kali_cli/tests/late_compat_browser_js_input.rs` |
...
| `Proxy` | Later compatibility | Canonical availability gate (`E5506`); `crates/kali_cli/tests/runtime_smoke.rs`, `crates/kali_cli/tests/late_compat_js_input.rs`, `crates/kali_cli/tests/late_compat_browser_js_input.rs`, `crates/kali_cli/src/build_tests.rs` (including `Proxy.revocable` / `globalThis.Proxy.revocable` / bracketed `globalThis["Proxy"]["revocable"]` coverage) |
...
| `WeakMap` / `WeakSet` / `WeakRef` | Later compatibility | Canonical availability gate (`E5506`); `crates/kali_cli/tests/runtime_smoke.rs`, `crates/kali_cli/tests/late_compat_js_input.rs`, `crates/kali_cli/tests/late_compat_browser_js_input.rs`, `crates/kali_cli/src/build_tests.rs` |
| `FinalizationRegistry` | Later compatibility | Canonical availability gate (`E5506`); `crates/kali_cli/tests/runtime_smoke.rs`, `crates/kali_cli/tests/late_compat_browser_js_input.rs`, `crates/kali_cli/src/build_tests.rs` |
| `SharedArrayBuffer` / `Atomics` | Later compatibility | Canonical availability gate (`E5506`); `crates/kali_cli/tests/runtime_smoke.rs`, `crates/kali_cli/tests/late_compat_js_input.rs`, `crates/kali_cli/tests/late_compat_browser_js_input.rs` |
| Broader `Intl` surface | Later compatibility | Canonical availability gate (`E5506`); `crates/kali_cli/tests/runtime_smoke.rs`, `crates/kali_cli/tests/runtime_smoke_broader_intl_js_input.rs`, `crates/kali_cli/tests/late_compat_js_input.rs`, `crates/kali_cli/tests/late_compat_browser_js_input.rs` (including bracketed `globalThis["Intl"]` / `globalThis["Intl"]["NumberFormat"]` / `globalThis["Intl"]["DateTimeFormat"]` / `globalThis["Intl"]["RelativeTimeFormat"]` / `globalThis["Intl"]["PluralRules"]` / `globalThis["Intl"]["Collator"]` / `globalThis["Intl"]["DisplayNames"]` / `globalThis["Intl"]["Locale"]` source coverage) |
...
| Interactive permission escalation APIs (`Deno.permissions.request()` / `revoke()` and similar prompt-style flows) | Rejected by default | Canonical Phase-1 Deno permission-facade gate (`E5506`); `crates/kali_parser/tests/parser_integration.rs`, `crates/kali_types/src/tests.rs`, `crates/kali_cli/tests/runtime_smoke.rs`, `crates/kali_cli/tests/late_compat_js_input.rs`, `crates/kali_cli/tests/late_compat_browser_js_input.rs` |
| Browser-targeted `for...of` array iteration | Rejected by default | Canonical iterator-lowering gate (`E5506`); `crates/kali_cli/tests/runtime_smoke.rs` (`check` / `build --bundle` on TS and `.js` input, including JSON-output coverage) |
| Browser-targeted `effects` / `check` / `build --bundle` rejection for `--wasm-threads` | Rejected by default | Canonical threaded-profile gate (`E5506`); `crates/kali_cli/tests/runtime_smoke.rs` |
...
| `Promise.allSettled` | Rejected by default | `crates/kali_cli/tests/runtime_smoke.rs` (standalone run/test JS input) |
