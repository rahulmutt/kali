...
| Generator lowering / `yield` / `yield*` syntax | Later compatibility | `crates/kali_parser/src/tests.rs`, `crates/kali_types/src/tests.rs`, `crates/kali_cli/tests/runtime_smoke.rs` (`check` and `build` gate coverage, including async generator syntax) |
