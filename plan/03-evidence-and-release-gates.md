...
- package-audit preview-shim regressions now also confirm the legacy `--preview` gate wins before malformed-target validation, package-analysis-specific flag validation, or registry lookup in JSON mode, including a valid single-target JSON-mode invocation so the registry never gets consulted on that path, keeping the command-shape boundary explicit. The same preview ordering is now also covered on the text-output browser-api path, and the current integration coverage now also exercises the malformed `npm:lodash` selector in both text and JSON modes so the shim still wins before target validation. The package-analysis-specific flag regressions now also cover both flag-before-target and target-before-flag spellings for `package-effects` and `package-audit`, keeping the rejection boundary order-independent.
