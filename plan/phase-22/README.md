...
- Triage `Proxy`, weak references, finalization, broader `Intl`, `SharedArrayBuffer`, `Atomics`, and wider object helpers against no-GC/no-JIT and optimization constraints. Frozen `Proxy.revocable` aliases now also reject through the same canonical `E5506` path in runtime, browser, and Node smoke, including the Node API-surface frozen-alias regression coverage. Browser-harness/browser-bundle `Math.pow` smoke now also covers parenthesized freeze-wrapper spellings around `globalThis.Math["pow"]` and `globalThis["Math"]["pow"]` on the supported alias-chain slice.
