...
- The thread-topology regression set now also pins monotonic instance-id assignment after termination/re-spawn, keeping worker accounting deterministic when a released slot is followed by a new worker. The runtime unit regression now also exercises that respawn path directly so the monotonic counter stays evidence-backed.
...
- Browser-requested runtime summary parsing now also rejects whitespace-only `threadTopology.liveInstances[].scriptUrl` values before merging fallback metadata, so malformed summary files still yield the stdout snapshot path; the same thread-topology fallback coverage now also spans JSX and TSX browser-harness summary fixtures, and the browser-requested `run` fallback fixtures now mirror that whitespace-padded `scriptUrl` check across JS, TS, JSX, and TSX input as well.
- The browser-requested runtime summary merge now also has an end-to-end regression for the stdout-threadTopology fallback path when the summary file omits `threadTopology`, keeping the merge helper and the browser harness outcome aligned. The browser bundle end-to-end regression now also mirrors the invalid-threadTopology fallback path, and the CLI browser-requested run/test wrapper now also preserves that stdout threadTopology snapshot when the summary file omits it, so both browser wrappers keep the stdout snapshot merge path pinned.
- CLI browser-harness JSON smoke now also exercises the invalid `threadTopology.liveInstances[].scriptUrl` summary-file path while still succeeding on the stdout summary line, keeping that malformed-browser-summary branch under integration coverage.
...
- Browser late-compat network coverage now also has the JSX and TSX `run` / `test` rejection fixtures on the shared `late_network_source()` helper, keeping the socket/listener slice aligned with the existing JS surface.
- The direct call-target binding helper now also centralizes the `process.kill` / `process["kill"]` / `globalThis.process["kill"]` / `globalThis["process"]["kill"]` aliases used by the Node API-surface smoke, so the callable-target aliases stay single-sourced alongside the existing sequence-callable-target helper.
