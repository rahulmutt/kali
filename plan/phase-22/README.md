...
- The shared late-object-model inventory now also carries the parenthesized single-quoted receiver-wrapped `Proxy.revocable` alias alongside the other late-object/runtime spellings, keeping the browser and runtime smoke aligned on that wrapper family too.
...
- Triage `Proxy`, weak references, finalization, broader `Intl`, `SharedArrayBuffer`, `Atomics`, and wider object helpers against no-GC/no-JIT and optimization constraints; the shared late-object-model inventory now also carries frozen bracket-root spellings for `WeakRef` and `FinalizationRegistry`, including the single-quoted variants, plus single-quoted bracketed `Proxy` constructor aliases, so the browser and runtime smoke stay aligned on those aliases across the browser JS/JSX/TSX fixture variants.
