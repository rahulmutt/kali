...
- Browser-requested runtime summary parsing now also rejects whitespace-only `threadTopology.liveInstances[].scriptUrl` values before merging fallback metadata, so malformed summary files still yield the stdout snapshot path.
