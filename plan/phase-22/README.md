...
- Browser-requested run/test JS, TS, JSX, and TSX smoke now also accepts the zero-capable `--max-threads 0` / `--max-spawned-processes 0` budget with matching human-output and JSON-output evidence, including the dedicated zero-spawned-process regression added in the current repository.
...
- The Node API-surface process-control regression now also covers the bracketed `process["cwd"]` / `process["chdir"]` / `process["exit"]` spellings and their `globalThis.process[...]` / `globalThis["process"][...]` aliases on the documented Node surface in JS, JSX, and TSX input, including the explicit `globalThis["process"]["cwd"]` / `globalThis["process"]["chdir"]` / `globalThis["process"]["exit"]` spellings in the current corpus.
