...
- Current smoke now also rejects single-quoted bracketed `SharedArrayBuffer` / `Atomics` aliases in the threaded-runtime gate, keeping the late-compat spellings honest alongside the positive budget path; the CLI build rejection lane now reuses the same canonical late-threaded-runtime source inventory.
...
- Triage `Proxy`, weak references, finalization, broader `Intl`, `SharedArrayBuffer`, `Atomics`, and wider object helpers against no-GC/no-JIT and optimization constraints; the shared late-object-model inventory now also carries frozen bracket-root spellings for `WeakRef` and `FinalizationRegistry`, including the single-quoted variants, so the browser and runtime smoke stay aligned on those aliases across the browser JS/JSX/TSX fixture variants.
