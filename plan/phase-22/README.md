...
- Browser-requested run/test JS smoke now also accepts the zero-capable `--max-threads 0` / `--max-spawned-processes 0` budget with matching human-output and JSON-output evidence, including the dedicated zero-spawned-process regression added in the current repository.
