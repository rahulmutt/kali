...
The benchmark lane currently uses the checked-in `math-benchmark-v1`, `call-inlining-benchmark-v1`, and `object-enumeration-benchmark-v1` fixture workloads under `crates/kali_cli/tests/fixtures/benchmarks/`, so the size/speed comparison evidence spans more than one pinned workload shape and now also covers object-enumeration folding.
