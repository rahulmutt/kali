...
- Current progress: the same release and release-advanced object-enumeration folding path now also covers const-bound literal aliases, so `Object.keys()` / `Object.entries()` / `Object.values()` stay folded when the literal object is referenced through a `const` binding, and the new alias-chain regression keeps that folding path honest when the object is rebound once more before enumeration.
