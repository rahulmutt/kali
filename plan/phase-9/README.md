...
- Progress: the optimization benchmark smoke now uses checked-in, hash-validated fixture workloads (`math-benchmark-v1.ts` / `math-benchmark-v1.json`, `call-inlining-benchmark-v1.ts` / `call-inlining-benchmark-v1.json`, `object-enumeration-benchmark-v1.ts` / `object-enumeration-benchmark-v1.json`, and `identity-chain-benchmark-v1.ts` / `identity-chain-benchmark-v1.json`) so the compile-time size/speed comparison lane is pinned to deterministic inputs instead of an ad-hoc inline source string.
