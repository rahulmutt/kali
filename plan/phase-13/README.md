...
- Progress note: the package corpus now also keeps the `spawn-tools` Deno-host package shape exercised on the standalone Deno surface, the browser surface's check/bundle lanes, the browser-harness run/test lanes, the inherited browser API-surface run/test lanes, and the browser runtime TS-input run/test lanes, with matching browser-runtime JSON-output coverage now also in JS input on the browser and inherited browser surfaces, and now also extends that browser-runtime matrix to JSX and TSX input on the browser and inherited browser surfaces, preserving the support-rung boundary across contexts while the standalone browser runtime gate remains separate.
