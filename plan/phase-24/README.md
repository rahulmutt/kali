...
- The benchmark fixture set now also tracks `math-pow-builtin-js` alongside `math-pow-builtin` so `Math.pow` evidence stays visible in both TS and JS workload forms, `math-abs-sign-builtin-js` now mirrors the `Math.abs` / `Math.sign` slice into the JS workload form too, the new `math-trunc-builtin-js` / `math-ceil-builtin-js` fixtures now mirror the `Math.trunc` / `Math.ceil` slices into JS as well, the new `math-floor-builtin` / `math-floor-builtin-js` pair now keeps the `Math.floor` workload visible in both source classes, the new `math-round-builtin` / `math-round-builtin-js` pair now keeps the `Math.round` workload visible in both source classes, and the generic `folded-arithmetic` benchmark now also has a JS workload form (`folded-arithmetic-js`) so the baseline arithmetic inventory stays paired across source classes.
