...
- Progress: the build-result JSON payload now validates its browser-bundle `bundleFormat` field before emission, now rejects unexpected top-level and artifact-entry keys before printing, and the schema-doc drift net now pins that `esm` / `cjs` variant explicitly, keeping the browser bundle contract aligned across code and schema. The build-result schema validation tests now also round-trip the `lib` and `capi` artifact kinds, keeping the remaining artifact-mode variants covered alongside the existing bundle/component cases. The schema-doc drift net now also records the current snapshot's implementation-specific build-result artifact-kind labels (`meta-json` plus browser-bundle `chunk-*` labels), keeping the checked-in payload vocabulary explicit without promoting those labels to new canonical kinds.
