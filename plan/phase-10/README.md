...
- Progress: schema-document assertions now cover the core result payloads for `check`, `run`, `install`, `fmt`, `lint`, `effects`, `build`, `test`, `package-effects`, and `package-audit`, and now also pin the supporting diagnostic, manifest, lockfile, and sandbox-policy schema documents, keeping the JSON-schema drift net aligned with the current command and config surface.
