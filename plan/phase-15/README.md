...
- Current regression coverage also asserts empty `errors` arrays on successful browser-targeted `check` / `build` and browser-requested `run` / `test` JSON paths for the promise-allSettled smoke slice, the browser for-of / for await iterator smoke slices, the browser Object.is / Object.hasOwn helper slices, the browser Object.fromEntries / Reflect.ownKeys helper slices, the browser Reflect.ownKeys helper slice now also spans JSX and TSX input across the browser-harness and browser-bundle success paths, the Deno env-set / env-delete executable build JSON smoke slices, the standalone object-enumeration / `Object.fromEntries` JSON smoke slices, the browser Math.round JSON smoke slice now also spans TS, JS, JSX, and TSX input and now also includes the `globalThis.Math.round` root slice in browser-harness/browser-bundle JSON success paths, the browser Math.log2 / Math.log10 browser-harness and browser-bundle JSON success paths, the browser Math.hypot empty-identity / perfect-square smoke slice now also carries the same empty-errors assertion on the browser-harness JSON path, and the generic emitted-envelope regression now also checks that successful text-output envelopes preserve empty `errors` / `warnings` arrays alongside stdout and stderr; the current run-path coverage now also pins that same envelope shape on successful text-output `run` and `test` envelopes, the `init` scaffold success paths now also assert empty `errors` / `warnings` arrays in JSON output, the browser runtime summary-fallback JSON success paths now also assert empty `errors` arrays, and the browser Math.exp2 browser-harness/browser-bundle JSON success paths now also assert empty `errors` / `warnings` arrays, keeping the success-envelope shape explicit without narrowing extension posture.
