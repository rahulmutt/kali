...
- Current regression coverage also asserts empty `errors` arrays on successful browser-targeted `check` / `build` and browser-requested `run` / `test` JSON paths for the promise-allSettled smoke slice, the browser for-of / for await iterator smoke slices, the browser Object.is / Object.hasOwn helper slices, the browser Object.fromEntries / Reflect.ownKeys helper slices, the Deno env-set / env-delete executable build JSON smoke slices, the standalone object-enumeration / `Object.fromEntries` JSON smoke slices, and the browser Math.round JSON smoke slice now also spans TS, JS, JSX, and TSX input, keeping the success-envelope shape explicit without narrowing extension posture.
