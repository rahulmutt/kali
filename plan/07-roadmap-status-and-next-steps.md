...
- the public `effects` lane now also ignores a top-level `sandbox` config path in JSON output, so source-graph analysis stays decoupled from policy-attachment plumbing just like the registry-analysis hardening already does
