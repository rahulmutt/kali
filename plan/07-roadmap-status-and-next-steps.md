...
- the shared diagnostics exit-code helper now treats `E5506` feature-unavailable diagnostics as usage-style failures too, so phase-gated command and availability checks keep the canonical exit-code split aligned with the other E55xx machine-contract errors
- a dedicated determinism smoke lane now runs through `scripts/check-determinism.sh` and a matching CI job, so the repeated-build evidence is exercised separately from the larger runtime smoke suite; the lane also pins the repeated-invocation envelopes for `effects`, `package-effects`, and `package-audit`, including the quiet-mode pretty JSON variants that keep the browser-context effects path and the package-analysis reporting lanes deterministic, not just the build artifacts. The same lane now also checks the default `effects` JSON payload path directly, so the native-JSON reporting mode stays byte-stable even without the envelope wrapper. The lane now also covers the quiet pretty JSON package-audit regressions under inherited browser context and with a top-level sandbox config path, so the registry-audit lane stays pinned across the last remaining presentation-control/browser-policy combinations. It now also covers repeated `effects` and `package-effects` JSON-envelope invocations under inherited browser context with a top-level sandbox config path, keeping the source-graph and registry-analysis report lanes deterministic across that last browser/policy presentation combination too.
...
- package-effects now also keeps repeated JSON-envelope output stable under inherited browser analysis context, so the browser-resolution registry-analysis lane stays deterministic even when policy plumbing is present in the workspace config
...
- the source-graph `effects` command now also pins repeated native-JSON output under inherited browser analysis context, so the browser-context report stays deterministic in both output modes instead of only the envelope path
...
- the reusable effect-report payload now also deduplicates repeated logical roots in first-seen order before serialization, keeping future multi-root callers deterministic without widening the current CLI shape; the library regression suite now pins duplicate-root inference and entry-point dedupe together so that contract stays explicit
...
- the CLI-level `effects` JSON lane now also pins duplicate `--compat` inputs, the `package-effects` JSON lane now also pins whitespace-padded inherited `compat.features`, and the manifest/config path still rejects duplicate `compat.features` entries with the canonical config diagnostic, so the trimmed-and-deduplicated analysis context remains honest in end-to-end smoke coverage instead of only in helper-level tests
...
- `kali install` on an empty workspace now has a CLI JSON-envelope regression that proves the command stays a clean no-op without scaffolding `kali.json` or `kali.lock`, keeping the configless install path honest at the operator boundary; that same empty-workspace JSON path now also stays identical across repeated invocations, so the no-op install contract is covered by the determinism lane too
...
- the PGO profile-data loader now rejects unknown JSON fields, keeping the build-only profile input strict and schema-backed instead of permissive toward ad hoc extensions
