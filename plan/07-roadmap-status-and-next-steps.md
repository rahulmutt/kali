...
- the CLI-level `effects` and `package-effects` JSON lanes now also pin duplicate and whitespace-padded `compat.features` inputs at the command boundary, plus whitespace-padded `runtimeProfiles` inputs on `effects`, so the trimmed-and-deduplicated analysis context remains honest in end-to-end smoke coverage instead of only in helper-level tests
