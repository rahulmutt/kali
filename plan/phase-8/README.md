...
- Progress: the Node package corpus now also exercises dedicated `node:buffer`, `node:path`, and `node:crypto` built-in package slices on `.js` input across `check` / `build` / `run` / `test`, so the Node built-in breadth probe stays separate from the node-assuming package fixtures while keeping first-class JavaScript compilation in view.
