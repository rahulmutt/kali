...
- Progress: the deterministic package-corpus matrix now lives in [`package-corpus-matrix.md`](./package-corpus-matrix.md) and groups the current corpus evidence by browser-targeted, default-standalone, Node, and Deno slices so the rung/context split stays explicit. The matrix drift test now also pins representative browser runtime corpus rows, the browser condition-preference TS and `.js` slices, the browser replacement-map, string-entry, string-export, browser-condition export, and browser dual-exports `.js` rows, the browser web-baseline `.js` runtime row, the browser host-heavy package-content `.js` runtime row, the browser typed export branch `.js` runtime row, the browser and default-standalone mixed CommonJS/ESM interop `.js` rows, the default standalone `semver` `.js` row, the default standalone package-content `.js` test row, the Node built-in packages (`node:buffer`, `node:assert`, `node:path`, `node:crypto`, `node:fs`, `node:url`) `.js` row, representative Node/Deno rows, and the Node-assuming `.js` surface row so the checked-in snapshot stays aligned with the current evidence set.
...
- Progress: the Node package corpus now also exercises dedicated `node:buffer`, `node:assert`, `node:path`, `node:crypto`, `node:fs`, and `node:url` built-in package slices on `.js` input across `check` / `build` / `run` / `test`, so the Node built-in breadth probe stays separate from the node-assuming package fixtures while keeping first-class JavaScript compilation in view.
...
- Progress: the browser package corpus now also exercises the canonical pure-JS `semver` probe on `.js` input, and the browser runtime corpus now also exercises the same probe on `.js` input for `run` and `test` when the browser harness command is configured; the package-corpus matrix now records both browser semver slices explicitly, keeping the support-rung evidence aligned with the browser-targeted first-class-JavaScript path instead of only the TypeScript lane.
