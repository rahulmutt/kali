...
| npm-style package corpus | Node built-in packages (`node:buffer`, `node:path`, `node:crypto`, `node:fs`) on `.js` input | Node | `check`, `build`, `run`, `test` | checkable / buildable / executable / testable | `crates/kali_cli/tests/package_corpus.rs` |
