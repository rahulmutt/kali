...
| npm-style package corpus | browser module-only packages with `.js` input and inherited browser `apiSurface` | browser-targeted | `check`, `build --bundle` | checkable / buildable / deployable-through-host | `crates/kali_cli/tests/package_corpus.rs` |
...
- The Node host-heavier package-content probe now also exercises the inherited Node `apiSurface` path in `.js` input, keeping package-content evidence explicit across both the direct and inherited Node surface forms.
