    ...
    header = hdr(
        ...
        P.matrix_declined(
            ...
            reason=[
                ...
                "literal and adds three spread re-checks, the non-js variant declares",
    ...
    header = hdr(
        ...
        P.matrix_declined(
            ...
            reason=[
                ...
                "explicitly: the `.ts`/`.tsx` cells get a five-line `as const` /",
                "`satisfies unknown` block spliced in and the `.js`/`.jsx` cells get nothing.",
        ...
        "string or a five-line block that supplies its own indentation. Hand-applying that is",
...
def gen_object_has_own_bundle():
    ...
    header = hdr(
        ...
        "by FOUR kali_common::object helpers, each of which joins a thirty-plus entry alias",
        "table. That is rule 8's `format!` trap and rule 9's \"one level removed inside a",
...
def gen_object_has_own_harness():
    ...
    header = hdr(
        ...
        "whose three arguments are computed by FOUR kali_common::object helpers, each of",
        "which joins a thirty-plus entry alias table. That is rule 8's `format!` trap and rule",
        "9's \"one level removed inside a library crate\" case at once. The two texts below are",
...
def gen_object_is_primitive_literals_harness():
    ...
    header = hdr(
        ...
        "    the identical eighteen lines, and the `ok 1` TAP line is written to the process's",
        "    own stdout rather than into the JSON leaf. That is not assumed -- the generator",
