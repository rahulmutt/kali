...
def gen_object_has_own_from_entries():
    ...
    header = hdr(
        ...
        wrap(f"The live-captured value is {pin!r}. The generator re-captures it from the real "
             ...
             "what the binary does without failing the generator -- CONDITIONAL ON THE BINARY "
             "BEING BUILT. `_pin` prints a loud skip and falls back to the embedded constant "
             "when the built `kali` is absent, so a generator run without it re-derives "
             "nothing here; the guarantee is only as strong as that precondition, and the "
             "sentence says so rather than overstating what the code does. NO CELL COUNT IS "
             "WRITTEN HERE: it would be a figure nothing re-resolves, and the capture set is a "
