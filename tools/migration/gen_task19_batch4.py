...
def check_reproduction_commands(spec, text):
    ...
    for printed in printed_commands(text):
        ...
        if cmd[1].endswith("audit-case-migration.py"):
            ...
            # rc=3 is AUDIT INAPPLICABLE: the audit ran to completion and
            # demanded zero claims, so it decided nothing. That is not a
            # reproduction failure -- the command reproduces exactly what the
            # header will say it does -- but it must not pass in silence
            # either. `gate_reds_for` sees the same rc=3 and REQUIRES a
            # declared reason, which is rendered into this file's header as an
            # EXPECTED-RED paragraph; so the outcome is stated in the shipped
            # prose, not swallowed here. Every other non-zero stays fatal.
            if p.returncode == 3:
                print(f"    reproduction command: {spec['stem']}: the audit is "
                      f"INAPPLICABLE to this pair (rc=3, 0 claims demanded); the "
                      f"header's EXPECTED-RED paragraph says so")
            elif p.returncode != 0:
                raise GenError(
...
def render(spec, gate_reds, cc_classes, u8_names, source_ref) -> str:
    ...
    L = []
    A = L.append
    A("# GENERATED by tools/migration/gen_task19_batch4.py -- do not edit this "
      "file by hand.")
    A("# Edit the generator and re-run it with --write; its default mode "
      "re-renders every")
    A("# file it owns and fails on any byte difference.")
    A("#")
...
GATE_RED_REASON = {
    ...
    ("audit-case-migration.py", 3): (
        "rc=3 is AUDIT INAPPLICABLE, not a dropped claim: the audit ran to "
        "completion over this pair and demanded ZERO claims of it, so it "
        "decided nothing either way. This source's assertions are exit-status "
        "claims (`assert!(!output.status.success(), ...)`) carrying no output "
        "literal, and everything else in it is fixture program text, which the "
        "audit excludes from its search by construction -- so there is nothing "
        "for a literal-coverage tool to find. Stated here because AUDIT OK over "
        "zero demanded claims and AUDIT OK over a covered pair are otherwise "
        "indistinguishable, and only one of them is evidence. What DOES cover "
        "this pair is the case runner itself: every case below pins the exit "
        "status the source asserted, and `check_fixtures.py` pins the argv and "
