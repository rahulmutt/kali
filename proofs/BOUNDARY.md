...
- The ownership slice currently models a small RC snapshot with live-reference and release tracking, plus live/released disjointness, release-update preservation, unrelated-heap preservation, last-ref zeroing on the decrement path, a local zero-count collection helper, and a local refcount-decrement update helper for well-formed snapshots; it is still narrower than the eventual full ownership / reference-counting story.
