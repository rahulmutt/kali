...
3. **Dispatch:** `t.dispatchEvent(new CustomEvent(<string literal>))` where the argument is an **inline** `new CustomEvent` with a literal type and no further args (`detail`/options → E5506). Semantics:
