...
- ✅ `crates/kali_cli/tests/package_corpus.rs` now also exercises `@stripe/react-stripe-js` in the browser web-baseline interop slice, so one more representative browser payment/UI package stays concrete through the browser command path without changing the documented support rungs.
- ✅ `plan/phase-3/03-ecosystem-breadth.md`, `PLAN.md`, and `TODO.md` now name the `@stripe/react-stripe-js` widening explicitly, keeping the corpus-breadth wording aligned with the current evidence set.
- ✅ Kept the update narrow: this is still a corpus / evidence widening within the existing package-support model, not a support-rung change.
