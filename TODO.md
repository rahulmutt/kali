...
- widen specialization depth beyond the current MIR-aware layout-specialized path; the optimizer
  ...
  `release_recursively_specializes_nested_mir_call_sites`, the literal-shaped
  concrete-argument fallback that can clone deterministic specializations even without MIR layout
  metadata, and the newer release-advanced MIR-specialized-clone path that still keeps generic
  instantiation available for large callees inside layout-specialized bodies, so the remaining work
  is the fuller
  generic-instantiation planner and cross-module specialization model while keeping the specialization
  budget and benchmark evidence honest,
- the release MIR-specialized clone path now also keeps generic specialization enabled inside
  layout-specialized clones, so large generic callees can still clone and fold after the MIR pass
  narrows the arguments even in `release` mode,
- widen the representative package corpus and browser/runtime interoperability without overclaiming
  ...
  `AbortController`, `EventTarget`, and `CustomEvent`, plus stub surfaces for `BroadcastChannel`, `WebSocket`, `Worker`, and `IndexedDB` in addition to the shared `Blob` / `File` / `FormData` / storage / `FileReader` helpers, plus a deterministic `navigator` metadata baseline and a `random_uuid` helper for `crypto.randomUUID()`-style calls plus a deterministic `Crypto` facade for the shared `crypto.getRandomValues()` / `crypto.randomUUID()` subset, and the browser web-baseline interop corpus now also exercises those surfaces alongside `fetch`, `Headers`, `Request`, `Response`, `date-fns`, `lodash`, `lodash-es`, `ramda`, `rambda`, `rxjs`, `uuid`, `recoil`, `mitt`, `swr`, `formik`, `nanostores`, `pinia`, `xstate`, `valtio`, `clsx`, `classnames`, `mobx`, `vue-router`, `react-router`, and now also carries `react-router-dom` through the browser exports-map and pattern-exports slices, `zod`, `svelte`, `lit`, `next`, `hono`, `react-helmet-async`, `@vueuse/core`, `@emotion/react`, `@emotion/styled`, `@stripe/react-stripe-js`, `@heroicons/react`, `lucide-react`, `react-dom`, `dayjs`, `jotai`, `framer-motion`, `@storybook/react`, `vite`, `@tanstack/react-form`, `@floating-ui/react`, `@headlessui/react`, `@chakra-ui/react`, `@mantine/core`, `@mui/material`, `@radix-ui/react-dialog`, `@tanstack/react-query`, `@tanstack/table-core`, `@tanstack/react-virtual`, `@testing-library/dom`, `@testing-library/user-event`, `@playwright/test`, `URLSearchParams`, `URL`, `TextEncoder`, `TextDecoder`, `localStorage` / `sessionStorage`, `atob` / `btoa`, `superjson`, `chart.js`, `recharts`, `msw`, `yaml`, `path-to-regexp`, and the existing browser representatives, plus the deterministic `performance.now()` and `queueMicrotask` helpers; the browser exports-map and pattern-exports corpus also now exercises `hono` explicitly, and the scoped browser corpus now also carries `@mantine/core` through the exports-map and browser-condition slices so one more representative UI package shape stays covered without changing the support-rung story, and `crates/kali_cli/tests/package_corpus.rs` now carries `@storybook/react` through the browser web-baseline, exports-map, and browser-condition slices, so the package-shape notes stay aligned with the narrower slice coverage, and now also carries `@tanstack/table-core` through the browser web-baseline interop and scoped browser exports-map/browser-condition slices so one more representative scoped table package name stays covered without changing the support-rung story, and now also carries `@tanstack/router` through the scoped browser exports-map and browser-condition slices so one more representative scoped routing package name stays covered without changing the support-rung story, and now also carries `@tanstack/react-router` through the browser web-baseline interop and scoped browser exports-map/browser-condition slices so one more representative scoped routing package name stays covered without changing the support-rung story, and the utility plain-package corpus now also carries `react` and `preact` on the default standalone surface so the representative React/Preact package breadth stays concrete without changing the support-rung story, and the utility plain-package corpus now also carries `axios` on the default standalone surface so one more common pure-JS package stays covered without changing the support-rung story, and the utility plain-package corpus now also carries `camelcase` on the default standalone surface so one more common pure-JS package stays covered without changing the support-rung story, and the utility plain-package corpus now also carries `msw` and `yaml` on the default standalone surface so one more browser/networking package name and one more data-format package name stay covered without changing the support-rung story, and the utility plain-package corpus now also carries `query-string` on the default standalone surface so one more query-string package name stays covered without changing the support-rung story, and the utility exports-map and pattern-exports corpus now also carries `reselect` so one more state-management package shape stays covered without changing the support-rung story, and the Node-runner corpus now also exercises `ava` in the exports-map and mixed-format slices so the test-runner breadth stays concrete without changing the support-rung story, and the utility plain-package corpus now also carries `chart.js`, `recharts`, `@emotion/react`, `@emotion/styled`, `ajv`, and `tailwindcss` on the default standalone surface so one more browser charting, validation, and build-tool package name stays concrete on the standalone surface without changing the support-rung story, and the browser web-baseline interop corpus now also exercises `@tanstack/query-core` alongside the utility plain-package slice, and the scoped browser corpus now also carries it through the exports-map and browser-condition slices so one more representative scoped query package shape stays concrete without changing the support-rung story, and the browser web-baseline interop corpus now also exercises `@babel/runtime` and `@npmcli/package-json` as another representative scoped utility package name pair without changing the support-rung story, and the utility web-baseline interop corpus now also exercises `@storybook/react` on the default standalone surface too without changing the support-rung story.
- the utility scoped-package corpus now also carries `@babel/runtime`, `@npmcli/package-json`, and `@reduxjs/toolkit` on the default standalone surface so the representative scoped utility package breadth stays concrete without changing the documented support-rung story,
...
### Stage 3.3 - cross-module object-return smoke widening
- ✅ `crates/kali_cli/tests/runtime_smoke.rs` now also exercises an inferred object-returning helper chain under an explicit `compilerOptions.maxSpecializations = 1` cap, so the public API inference story stays concrete even when the module flow crosses a re-export chain.
...
- ✅ `crates/kali_cli/tests/package_corpus.rs` now also exercises `tailwindcss` in the utility plain-package corpus on the default standalone surface, so the representative build-tool breadth stays concrete across the standalone path without changing the documented support rungs.
- ✅ `plan/phase-3/03-ecosystem-breadth.md`, `PLAN.md`, `plan/phase-1/sum.md`, and `TODO.md` now name the new `tailwindcss` coverage explicitly, keeping the tracker aligned with the current evidence set.
...
### Stage 3.3 - ajv browser/utility corpus widening
- ✅ `crates/kali_cli/tests/package_corpus.rs` now also exercises `ajv` in the browser web-baseline interop slice and the utility plain-package corpus, so the representative validation-package breadth stays concrete across both browser and default standalone surfaces without changing the documented support rungs.
- ✅ `plan/phase-3/03-ecosystem-breadth.md`, `PLAN.md`, `plan/phase-1/sum.md`, and `TODO.md` now name the new `ajv` coverage explicitly, keeping the tracker aligned with the current evidence set.
- ✅ Kept the update narrow: this widens the package corpus within the existing support-rung model; it does not change the documented support rungs.

### Stage 3.3 - dayjs browser exports-map/browser-condition widening
- ✅ `crates/kali_cli/tests/package_corpus.rs` now also exercises `dayjs` across the browser exports-map and browser-condition slices, so the representative date-time package breadth now stays concrete through the browser command path without changing the documented support rungs.
- ✅ `plan/phase-3/03-ecosystem-breadth.md`, `PLAN.md`, `plan/phase-1/sum.md`, and `TODO.md` now name the new `dayjs` coverage explicitly, keeping the tracker aligned with the current evidence set.
- ✅ Kept the update narrow: this widens the package corpus within the existing support-rung model; it does not change the documented support rungs.

### Stage 3.3 - lucide-react browser/utility corpus widening
- ✅ `crates/kali_cli/tests/package_corpus.rs` now also exercises `lucide-react` in the browser web-baseline interop slice and the utility plain-package corpus, so the representative browser UI package breadth now stays concrete across both browser and default standalone surfaces without changing the documented support rungs.
- ✅ `plan/phase-3/03-ecosystem-breadth.md`, `TODO.md`, and `PLAN.md` now name the new `lucide-react` coverage explicitly, keeping the tracker aligned with the current evidence set.
- ✅ Kept the update narrow: this widens the package corpus within the existing support-rung model; it does not change the documented support rungs.
...
### Stage 3.1 - Release-advanced cross-owner generic reuse
- ✅ `crates/kali_optimize/src/tests.rs` now also proves `release-advanced` reuses identical generic specializations across layout-specialized owners, so the advanced-mode cross-owner reuse path stays regression-tested alongside the existing release-mode coverage.
- ✅ `plan/phase-3/01-optimization-and-specialization.md`, `PLAN.md`, and `TODO.md` now name the new `release-advanced` reuse coverage explicitly, keeping the specialization-depth notes aligned with the current evidence set.
...
- ✅ `kali_api_web` now exposes a `random_uuid` helper for `crypto.randomUUID()`-style calls, plus a deterministic `Crypto` facade for the shared `crypto.getRandomValues()` / `crypto.randomUUID()` randomness subset, `kali_api_deno` reexports it, and `kali_runtime` now wires both `crypto_random_uuid` and `cryptoRandomUUID` host imports through the same helper, so the browser UUID slice stays covered without changing the documented support rungs.
- ✅ `crates/kali_api_web/src/tests.rs`, `crates/kali_api_deno/src/tests.rs`, `plan/phase-3/03-ecosystem-breadth.md`, `PLAN.md`, and `TODO.md` now name the UUID widening explicitly, keeping the tracker aligned with the current evidence set.
...
### Stage 3.3 - Web timing/randomness/microtask baseline widening
- ✅ `crates/kali_cli/tests/package_corpus.rs` now also exercises `performance.now()` and `queueMicrotask` in the shared web-baseline interop source, so the deterministic browser/runtime baseline keeps its timing and microtask helpers under test without changing the documented support rungs.
- ✅ `plan/phase-3/03-ecosystem-breadth.md` and `TODO.md` now name the same helper widening explicitly, keeping the tracker aligned with the current evidence set.
- ✅ Kept the update narrow: this widens browser/runtime interoperability within the existing web-baseline model; it does not change the documented support rungs.
...
- ✅ `crates/kali_cli/tests/package_corpus.rs` now also exercises `vite` in the utility plain-package and web-baseline interop slices, so the browser/runtime interoperability widening now carries one more modern build-tool package through both the default standalone and web-baseline command paths without changing the documented support rungs.
...
- ✅ The shared browser/runtime support library now also exposes an `IndexedDB` alias for the in-memory stub, and the Deno compatibility surface reexports that browser-aligned name too so Rust-facing code can mirror the docs while the lower-case `indexedDB` global stays the corpus source of truth.
...
- ✅ The shared browser/runtime support library now also dispatches a deterministic `abort` event from `AbortController`, giving the browser baseline a more realistic abort lifecycle without changing the documented support-rung story.
...
- ✅ `crates/kali_cli/tests/package_corpus.rs` now also exercises `@vueuse/core` across the scoped browser exports-map and browser-condition slices, so the scoped browser corpus now carries one more representative scoped browser utility package shape through the browser command path without changing the documented support rungs.
...
- ✅ `crates/kali_cli/tests/package_corpus.rs` now also exercises `graphql` in the browser web-baseline interop and utility plain-package slices, so the representative package corpus now carries one more common JS package name through both command paths without changing the documented support rungs.
- ✅ `crates/kali_cli/tests/package_corpus.rs` now also exercises `yup` across the browser web-baseline interop slice and the utility plain-package / utility web-baseline interop slices, so the representative package corpus now carries one more validation-library package name through the existing command paths without changing the documented support rungs.
...
### Stage 3.3 - atob/btoa web-baseline widening
- ✅ `crates/kali_cli/tests/package_corpus.rs` now also exercises `atob` / `btoa` in the shared web-baseline interop source, so the browser and utility corpus now carries deterministic binary-string round-tripping through the same helper path without changing the documented support rungs.
- ✅ `plan/phase-3/03-ecosystem-breadth.md`, `PLAN.md`, and `TODO.md` now name the `atob` / `btoa` browser/runtime widening explicitly, keeping the current progress notes aligned with the shared helper source.
- ✅ Kept the update narrow: this widens browser/runtime interoperability within the existing web-baseline model; it does not change the documented support rungs.

### Stage 3.3 - navigator metadata widening
- ✅ `crates/kali_cli/tests/package_corpus.rs` now also exercises `navigator.userAgent`, `navigator.language`, and `navigator.onLine` in the shared web-baseline interop source, and the shared browser/runtime support library now exposes a deterministic `navigator` metadata baseline so browser-style code can inspect ambient browser metadata without changing the documented support-rung story.
- ✅ `plan/phase-3/03-ecosystem-breadth.md`, `PLAN.md`, and `TODO.md` now name the deterministic `navigator` metadata widening explicitly, keeping the current progress notes aligned with the shared helper source.
- ✅ Kept the update narrow: this widens browser/runtime interoperability within the existing web-baseline model; it does not change the documented support rungs.

### Stage 3.3 - Storage web-baseline widening
- ✅ `crates/kali_cli/tests/package_corpus.rs` now also exercises `localStorage` / `sessionStorage` in the shared web-baseline interop source, so the browser and utility corpus now carries the in-memory browser-state helpers through the browser command path without changing the documented support rungs.
- ✅ `plan/phase-3/03-ecosystem-breadth.md`, `PLAN.md`, and `TODO.md` now name the storage widening explicitly, keeping the current progress notes aligned with the shared helper source.
- ✅ Kept the update narrow: this widens browser/runtime interoperability within the existing web-baseline model; it does not change the documented support rungs.

### Stage 3.3 - msw utility corpus widening
- ✅ `crates/kali_cli/tests/package_corpus.rs` now also exercises `msw` in the utility plain-package corpus on the default standalone surface, so the representative browser/networking package breadth now stays concrete on the standalone path without changing the documented support rungs.
- ✅ `plan/phase-3/03-ecosystem-breadth.md`, `PLAN.md`, and `TODO.md` now name the utility `msw` coverage explicitly, keeping the current progress notes aligned with the shared helper source.
- ✅ Kept the update narrow: this widens the package corpus within the existing support-rung model; it does not change the documented support rungs.
...
### Stage 3.3 - @npmcli/package-json browser web-baseline widening
- ✅ `crates/kali_cli/tests/package_corpus.rs` now also exercises `@npmcli/package-json` in the browser web-baseline interop slice, so the representative scoped utility package breadth stays concrete across the browser command path without changing the documented support rungs.
- ✅ `plan/phase-3/03-ecosystem-breadth.md`, `PLAN.md`, and `TODO.md` now name the browser widening explicitly, keeping the current progress notes aligned with the evidence set.
- ✅ Kept the update narrow: this widens the representative package corpus within the existing support-rung model; it does not change the documented support rungs.
...
- [x] Stage 3.2 Node API layer scaffold
  ...
  - The dedicated semver bin smoke now runs under `--api node`, keeping the Node CLI surface honest about the API gate instead of treating `process.argv` / CommonJS entrypoints as a standalone surface.
...
- ✅ Added browser, utility, and Node-runner corpus cases that resolve published exports maps and subpath entrypoints for `react`, `preact`, `vue`, `svelte`, `lit`, `ramda`, `rxjs`, `immer`, `uuid`, `typescript`, `esbuild`, `date-fns`, `lodash-es`, `vitest`, `jest`, `mocha`, and `ava`, broadening the package-support corpus beyond the original single-entrypoint stubs.
- ✅ Added `axios` to the utility plain-package corpus on the default standalone surface so the representative pure-JS package breadth now carries one more common package name through the standalone command path without changing the documented support rungs.
- ✅ Added `camelcase` to the utility plain-package corpus on the default standalone surface so the representative pure-JS package breadth now carries one more common package name through the standalone command path without changing the documented support rungs.
...
- ✅ Added `mocha` and `ava` to the Node-runner exports-map and mixed-format corpus slices so the representative test-runner coverage now exercises one more package-shape variant without changing the documented support rungs.
...
- ✅ Added `redux` to the browser web-baseline interop corpus so the representative package corpus now carries one more browser state-management package name through the browser command path without changing the documented support rungs.
...
- ✅ `crates/kali_cli/tests/package_corpus.rs` now also exercises `react-router-dom` in the browser exports-map and pattern-exports slices, so the representative router package breadth now carries one more browser package shape through the browser command path without changing the documented support rungs.
...
- ✅ Added `jotai` to the browser exports-map and browser-condition slices so the representative browser state-management package breadth now carries one more package shape through the browser command path without changing the documented support rungs.
...
- ✅ Added `@playwright/test` to the browser web-baseline interop slice so the representative browser test-runner package breadth now carries one more package name through the browser command path without changing the documented support rungs.

### Stage 3.3 - @storybook/react utility web-baseline widening
- ✅ `crates/kali_cli/tests/package_corpus.rs` now also exercises `@storybook/react` in the utility web-baseline interop slice, so the representative scoped browser package breadth now stays concrete on the default standalone surface too without changing the documented support rungs.
- ✅ `plan/phase-3/03-ecosystem-breadth.md` now names the utility web-baseline `@storybook/react` widening explicitly, keeping the stage progress note aligned with the code.
- ✅ Kept the update narrow: this widens the package corpus within the existing support-rung model; it does not change the documented support rungs.
- ✅ Added `vite` to the utility plain-package and web-baseline interop slices so the representative package corpus now carries one more modern build-tool package name through the existing support-rung checks without changing the documented support rungs.
- ✅ Added `react` and `preact` to the utility plain-package corpus on the default standalone surface so the representative React/Preact package breadth now stays concrete without changing the documented support rungs.
- ✅ Added `@tanstack/table-core` to the browser web-baseline interop slice and the scoped browser exports-map / browser-condition slices so the representative scoped table package breadth now carries one more package name through the browser command path without changing the documented support rungs.
- ✅ Added `@tanstack/router` to the scoped browser exports-map and browser-condition slices so the representative scoped routing package breadth now carries one more package name through the browser command path without changing the documented support rungs.
- ✅ `plan/phase-3/03-ecosystem-breadth.md`, `PLAN.md`, and `TODO.md` now name `@tanstack/table-core` explicitly in the Stage 3.3 progress notes, keeping the corpus-breadth wording aligned with the current evidence set.
...
### Stage 3.3 - @tanstack/react-router browser/scoped-browser corpus widening
- ✅ `crates/kali_cli/tests/package_corpus.rs` now also exercises `@tanstack/react-router` in the browser web-baseline interop slice and the scoped browser exports-map / browser-condition slices, so the representative scoped routing package breadth now carries one more package shape through the browser command path without changing the documented support rungs.
- ✅ `plan/phase-3/03-ecosystem-breadth.md`, `PLAN.md`, and `TODO.md` now name the `@tanstack/react-router` widening explicitly, keeping the corpus-breadth wording aligned with the current evidence set.
- ✅ Kept the update narrow: this is still a corpus / evidence widening within the existing package-support model, not a support-rung change.

### Stage 3.3 - @tanstack/query-core scoped-browser corpus widening
- ✅ `crates/kali_cli/tests/package_corpus.rs` now also exercises `@tanstack/query-core` in the scoped browser exports-map and browser-condition slices, so the representative scoped query package breadth now stays concrete across the browser shape coverage without changing the documented support rungs.
- ✅ `plan/phase-3/03-ecosystem-breadth.md`, `PLAN.md`, and `TODO.md` now name the scoped-browser `@tanstack/query-core` widening explicitly, keeping the corpus-breadth wording aligned with the current evidence set.
- ✅ Kept the update narrow: this is still a corpus / evidence widening within the existing package-support model, not a support-rung change.
...
### Stage 3.3 - reselect utility exports-map/pattern-exports widening
- ✅ `crates/kali_cli/tests/package_corpus.rs` now also exercises `reselect` in the utility exports-map and pattern-exports slices, so the representative state-management package breadth now carries one more package shape through the standalone command path without changing the documented support rungs.
- ✅ `plan/phase-3/03-ecosystem-breadth.md`, `PLAN.md`, and `TODO.md` now name the `reselect` utility exports-map/pattern-exports widening explicitly, keeping the corpus-breadth wording aligned with the current evidence set.
- ✅ Kept the update narrow: this is still a corpus / evidence widening within the existing package-support model, not a support-rung change.
...
### Stage 3.3 - rambda browser/utility corpus widening
- ✅ `crates/kali_cli/tests/package_corpus.rs` now also exercises `rambda` in the browser web-baseline interop, utility plain-package, and utility module-entry slices, so one more functional-utility package name stays concrete across the browser and standalone command paths without changing the documented support rungs.
- ✅ `plan/phase-3/03-ecosystem-breadth.md`, `PLAN.md`, and `TODO.md` now name the `rambda` browser/utility widening explicitly, keeping the corpus-breadth wording aligned with the current evidence set.

### Stage 3.3 - rxjs browser/utility corpus widening
- ✅ `crates/kali_cli/tests/package_corpus.rs` now also exercises `rxjs` in the browser corpus and browser web-baseline interop slice, while the utility plain-package coverage already existed, so one more observable/stream package name stays concrete across the browser and standalone command paths without changing the documented support rungs.
- ✅ `plan/phase-3/03-ecosystem-breadth.md`, `PLAN.md`, and `TODO.md` now name the `rxjs` browser/utility widening explicitly, keeping the corpus-breadth wording aligned with the current evidence set.
...
### Stage 3.3 - react-helmet-async browser web-baseline widening
- ✅ `crates/kali_cli/tests/package_corpus.rs` now also exercises `react-helmet-async` in the browser web-baseline interop slice, so one more representative browser head-management package stays concrete through the browser command path without changing the documented support rungs.
- ✅ `plan/phase-3/03-ecosystem-breadth.md`, `PLAN.md`, and `TODO.md` now name the `react-helmet-async` widening explicitly, keeping the corpus-breadth wording aligned with the current evidence set.
- ✅ Kept the update narrow: this is still a corpus / evidence widening within the existing package-support model, not a support-rung change.

### Stage 3.3 - @stripe/react-stripe-js browser web-baseline widening
- ✅ `crates/kali_cli/tests/package_corpus.rs` now also exercises `@stripe/react-stripe-js` in the browser web-baseline interop slice and the scoped browser exports-map/browser-condition slices, so one more representative browser payment/UI package stays concrete through the browser command path and the scoped browser shape coverage without changing the documented support rungs.
- ✅ `plan/phase-3/03-ecosystem-breadth.md`, `PLAN.md`, and `TODO.md` now name the `@stripe/react-stripe-js` widening explicitly, keeping the corpus-breadth wording aligned with the current evidence set and the scoped-browser slice coverage.
- ✅ Kept the update narrow: this is still a corpus / evidence widening within the existing package-support model, not a support-rung change.

### Stage 3.3 - deepmerge utility corpus widening
- ✅ `crates/kali_cli/tests/package_corpus.rs` now also exercises `deepmerge` in the utility plain-package corpus on the default standalone surface, so one more common pure-JS package stays concrete through the standalone command path without changing the documented support rungs.
...
### Stage 3.3 - @tanstack/react-virtual browser web-baseline widening
- ✅ `crates/kali_cli/tests/package_corpus.rs` now also exercises `@tanstack/react-virtual` in the browser web-baseline interop corpus, so one more representative scoped browser virtualization package stays concrete through the browser command path without changing the documented support rungs.
- ✅ `plan/phase-3/03-ecosystem-breadth.md`, `PLAN.md`, `plan/phase-1/sum.md`, and `TODO.md` now name the new `@tanstack/react-virtual` coverage explicitly, keeping the tracker aligned with the current evidence set.
- ✅ Kept the update narrow: this is still a corpus / evidence widening within the existing package-support model, not a support-rung change.
...
- ✅ The same helpers now also package wellformedness and ownership preservation together via `KaliCore.Safety.releaseRefPreservesWellFormedAndOwnership`, `KaliCore.Safety.releaseAndDecrementPreservesWellFormedAndOwnership`, and `KaliCore.Safety.releaseAndCollectPreservesWellFormedAndOwnership`.
