...
- ✅ `KaliCore.Safety.releaseAndCollectHeapCellOriginAndOwnership` now makes the surviving release-and-collect heap cells' original ownership tag explicit alongside their provenance and name preservation.
...
- ✅ `releaseAndDecrementKeepsOtherPositiveCountCells` now proves positive-count cells from the original heap survive on the decrement path when they are not the released target.
...
- ✅ `releaseAndCollectHeapCellOriginAndOwnership` now makes the surviving collected heap cells' original name and ownership tag explicit.
