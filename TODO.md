...
- the utility scoped-package corpus now also carries `@babel/runtime` and `@npmcli/package-json` on the default standalone surface so the representative scoped utility package breadth stays concrete without changing the documented support-rung story,
