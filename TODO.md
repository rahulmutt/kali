...
- [x] Stage 3.2 Node API layer scaffold
  ...
  - The helper layer now exposes `NodePath`, `NodeCrypto`, and `NodeRuntimeProjection` facades so future linker registration has a single Node-host surface to project through.
  - The runtime linker now consumes the Node projection facade for `fs/promises`, stream, and HTTP host imports when the effective API surface is `node`.
