...
- ✅ Added `@emotion/react` and `@emotion/styled` to the browser web-baseline interop slice so the browser/runtime interoperability widening now carries another scoped browser package name through the browser command path without changing the documented support rungs.
