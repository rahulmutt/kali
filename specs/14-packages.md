...
- to keep registry analysis deterministic and independent from **project dependency state** in schema v1, they use the shared [canonical stable-release selection rule](#canonical-stable-release-selection-rule-schema-v1)
...
- that inherited context is still validated against the normal maturity rules for package analysis; for example, config-selected `apiSurface = node`, `runtimeProfiles = ["wasm-threads"]`, or `compat.features = ["eval"]` should still produce `E5006` until those package-analysis modes are supported, rather than silently falling back to a smaller context
