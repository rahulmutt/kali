...
| `--max-threads N` | execution commands | Override the invocation thread cap for the threaded runtime profile; may only tighten the effective limit. Follows the shared **feature-gated zero-capable execution budgets** rule from [SPEC.md](../SPEC.md): `0` is a valid explicit deny/tightening value in Phase 1, while positive values are accepted only when the threaded profile is explicitly active and supported in the current availability context. |
| `--wasm-threads` | `check`, `effects`, `build`, `run`, `test` | Opt into the later threaded runtime profile required for `SharedArrayBuffer` / `Atomics`; supported execution contexts accept the opt-in on the same availability path as their corresponding `--max-threads` budget, while unsupported targets or command/profile combinations must still fail with `E5506` |
...
- practical support-reading shortcut: for `package-effects`, keep two questions separate — **does the command exist yet?** and **what inherited analysis context is active once it exists?** Explicit package-analysis flags such as `--api ...`, `--compat ...`, `--wasm-threads`, and `--sandbox` stay invalid usage in schema v1, so browser/Node/threaded/`eval` cases should be read only through inherited config/defaults rather than through a second per-command flag surface; those flag rejections are outer command-shape checks and win before package-target validation or registry lookup
...
In schema v1 JSON mode, `run` and `test` payloads may include optional `hostContract`, `runtimeBackend`, and `threadTopology` provenance snapshots; those fields are deterministic observability data, not support claims. When present, `threadTopology.liveInstances` is serialized in ascending `instanceId` order and remains duplicate-free so repeated emissions stay stable. Each `threadTopology.liveInstances[].scriptUrl` is a canonical absolute URL string, not a relative or whitespace-padded spelling.
...
- `--api node` is supported for `check`, `build`, `run`, and `test` on the documented Node-compatible subset, while `effects` and the package-analysis commands still gate that surface explicitly. The documented Node-compatible subset includes the bracketed `process["cwd"]` / `process["chdir"]` / `process["exit"]` forms and their `globalThis.process[...]` spellings.
...
- Graph-scope rule: on `check` and `build`, attaching `--sandbox` validates the same **resolved source graph** from [SPEC.md](../SPEC.md) that the command already analyzes/builds; it does not validate only the root file while ignoring transitive imports/dependencies.
- Full inferred-effect-vs-policy validation is a Phase 2 feature.
- Policy validation must also reject policies that try to enable capabilities unavailable in the selected command/profile/phase (for example `effects.eval: true` before the eval compatibility path exists, `effects.eval: true` without effective `--compat eval`, or positive `resources.maxThreads` / `resources.maxSpawnedProcesses` values before the threaded profile or subprocess support exists). `resources.maxSpawnedProcesses` remains subject to the selected command/context's normal sandbox checks, including the browser-targeted budget compatibility rule where applicable.
...
- `kali build --sandbox ...` never executes the program; in Phase 1 it validates policy/config over that command's **resolved source graph**, and starting in the Phase 2 target window it also performs effect-vs-policy validation
...
- `kali effects <file>` summarizes effects for that command's **resolved source graph** under the selected API surface/profile; it is not limited to syntax that appears textually in the one named file
...
- `effects` then summarizes that same **resolved source graph** from those recorded logical roots
...
Status: Phase 1 MVP. `kali doctor` is an environment/debugging command. In schema v1 it reports the effective browser bundle/runtime harness command selected from `KALI_BROWSER_BUNDLE_HARNESS_COMMAND` or from Kali's auto-detection fallback, plus a declarative browser runtime contract snapshot for diagnostic use, including the non-empty browser host description note and non-empty diagnostic hint in the machine-readable snapshot. The browser-harness `envVar` field is the canonical `KALI_BROWSER_BUNDLE_HARNESS_COMMAND` name, the `executable` field, the env-selected `override` string, and the browser runtime contract `hostDescription` are also kept as non-empty strings in that JSON snapshot. The browser runtime contract `hostLabel`, `hostDescription`, `hostDescriptionNote`, `summaryNote`, `contractScopeNote`, and `diagnosticHint` are compared against their canonical strings after trimming surrounding whitespace, and the `supportedCommands` / `diagnosticNotes` items are compared after trimming too, but whitespace-only values still fail validation and duplicates remain forbidden. It has no source-file arguments and no semantic/context flags; global presentation flags such as `--output json`, `--pretty`, `--quiet`, and `--verbose` retain their ordinary meanings.

Behavior:
- text output prints a compact browser-harness summary plus the browser runtime contract snapshot intended for humans, including the browser host description note and the canonical diagnostic notes
...
- in configless mode, that inherited context is just the **default inherited analysis context (schema v1)** from [SPEC.md](../SPEC.md), and schema-v1 package-effect payloads record that state explicitly with `report.analysisContext.apiSurface = "default"`
...
- the legacy `--preview` compatibility shim is rejected with `E5508` before the command reaches target validation, registry lookup, or JSON-envelope construction
...
- as the `package-audit` half of the shared **registry-analysis command split**, `package-audit` follows **context-free registry analysis (schema v1)** and therefore does **not** inherit the shared **inherited analysis context** or accept package-analysis-specific `--api` / runtime-profile / `--compat` flags or `--sandbox`; those flag rejections also stay ahead of target validation and registry lookup
...
- `compilerOptions.maxSpecializations` caps specialization fan-out for generic/layout-driven optimization in modes that actively specialize; schema v1 models it as a non-negative integer, and CLI `--max-specializations` overrides it for a single invocation
