...
- ESM entrypoints satisfy that rule directly; CommonJS entrypoints participate only when Kali's static CJS lowering can prove one fixed export set for the entry module, otherwise the library-oriented build fails with `E5006` instead of synthesizing reflection-based exports
