...
- Policy validation must also reject policies that try to enable capabilities unavailable in the selected command/profile/phase (for example `effects.eval: true` before the eval compatibility path exists, `effects.eval: true` without effective `--compat eval`, or positive `resources.maxThreads` values before the threaded profile exists). `resources.maxSpawnedProcesses` remains subject to the selected command/context's normal sandbox checks, including the browser-targeted budget compatibility rule where applicable.
