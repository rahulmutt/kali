...
In schema v1 JSON mode, `run` and `test` payloads may include optional `hostContract`, `runtimeBackend`, and `threadTopology` provenance snapshots; those fields are deterministic observability data, not support claims. When present, `threadTopology.liveInstances` is serialized in ascending `instanceId` order and remains duplicate-free so repeated emissions stay stable.
