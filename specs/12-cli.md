...
- practical support-reading shortcut: for `package-effects`, keep two questions separate — **does the command exist yet?** and **what inherited analysis context is active once it exists?** Explicit package-analysis flags such as `--api ...`, `--compat ...`, `--wasm-threads`, and `--sandbox` stay invalid usage in schema v1, so browser/Node/threaded/`eval` cases should be read only through inherited config/defaults rather than through a second per-command flag surface
