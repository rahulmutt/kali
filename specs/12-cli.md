...
- conflicting artifact-mode combinations such as `--bundle --lib`, `--bundle --capi`, `--bundle --component`, `--lib --capi`, or `--lib --component` are **CLI usage/config errors**, not feature-maturity rejections; the user asked for incompatible output modes rather than for a later-phase feature
