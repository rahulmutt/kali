...
- Environment access: `Deno.env.get` / `Deno.env.has` *(expose only the sandbox-permitted environment view rather than the raw host environment; bracketed `Deno["env"]["get"]` / `Deno["env"]["has"]` / `globalThis["Deno"]["env"]["get"]` / `globalThis["Deno"]["env"]["has"]` source forms stay on the same read-only env slice, including the mixed dot/bracket `globalThis.Deno["env"]["get"]` / `globalThis.Deno["env"]["has"]` forms; the current repository snapshot also allows `Deno.env.set` / `Deno.env.delete` plus the bracketed `Deno["env"]["set"]` / `Deno["env"]["delete"]` forms on the default standalone surface, while the Node and browser surfaces still reject this broader Deno mutable-env slice; browser-targeted contexts still reject mutable env access, including `process.env.KEY = ...` / `delete process.env.KEY` and the bracketed `process["env"]["KEY"]` / `delete process["env"]["KEY"]` style forms on the Node/browser compatibility boundary; `Deno.env.toObject` now lowers on the default standalone Deno surface, while browser and Node compatibility surfaces still gate it)*
...
- `Deno.env.get` stays under `effects.process.envRead`; `Deno.env.toObject` follows the same effect family on the default standalone Deno surface as well
...
- process identity query `Deno.pid` / `globalThis.Deno.pid` is already available on the default standalone surface in the current repository snapshot, the read-only `Deno.cwd` / `globalThis.Deno.cwd` working-directory query is now available there too, and `Deno.chdir` / `globalThis.Deno.chdir` is now also available there as a sandbox-policy-mediated working-directory mutation API, while `Deno.exit` now shares the same standalone exit import on that surface

Process identity `Deno.pid` / `globalThis.Deno.pid` is a read-only query on the default standalone surface in the current repository snapshot, while process termination (`Deno.exit`) is now also available there as the standalone exit API. `Deno.cwd` / `globalThis.Deno.cwd` is also available there as a read-only working-directory query, and `Deno.chdir` / `globalThis.Deno.chdir` is now available there as the working-directory mutation API. On the Node surface, `process.pid` is a read-only query, `process.cwd` is now also available as a read-only working-directory query in the current repository snapshot, `process.chdir` is now also available there as the working-directory mutation API, and `process.exit` is now also available there as the Node process-control exit API. The documented Node surface now also accepts the bracketed `process["cwd"]` / `process["chdir"]` / `process["exit"]` forms, their single-quoted `process['exit']` sibling used by the shared smoke corpus, their `globalThis.process[...]` / `globalThis['process'][...]` spellings, and optional-chain-wrapped `process?.kill(0)` / `globalThis.process?.kill(0)` / `globalThis["process"]?.kill(0)` zero-probe aliases, while the dedicated later-compatibility rejection fixtures still keep the unsupported Deno/browser process-control spellings explicit. Those latter APIs widen the embedding/sandbox contract but are not needed for the initial package-oriented baseline.
...
- `Deno.env.set` / `Deno.env.delete` *(the broader cross-surface mutable-env path is still a Phase 3 target; the current repository snapshot already exposes this slice on the default standalone surface, including the bracketed `Deno["env"]["set"]` / `Deno["env"]["delete"]` forms, while Node and browser-targeted contexts still reject it)*
...
- `Deno.exit` now shares the default standalone exit path in the current repository snapshot; the remaining Deno-side sandbox contracts are still encoded elsewhere in the matrix rather than widened implicitly by that host-control alias
...
- `url` — URL parsing
- `assert` — assertions
- `process` — process global subset needed by real packages first (`env`, `argv`, selected control/query helpers); `process.pid` is already available as a read-only query on the Node surface, `process.cwd` is also available as a read-only query in the current repository snapshot, `process.chdir` is now also available there as the working-directory mutation API, and `process.exit` is now also available there as the Node process-control exit API. The Node surface now also accepts the bracketed `process["cwd"]` / `process["chdir"]` / `process["exit"]` forms and their `globalThis.process[...]` spellings. The Node surface now also supports the `process.kill(0)` liveness-probe subset on the same `process.kill` member path, including the bracketed `process["kill"]` / `globalThis["process"]["kill"]` invocation spellings over the zero-literal probe and the receiver-freeze aliases `Object.freeze(process)["kill"](0)` / `Object.freeze(globalThis.process)["kill"](0)` / `Object.freeze(globalThis["process"])["kill"](0)`, plus the parenthesized receiver-freeze spellings `Object.freeze((globalThis.process)["kill"])(0)` / `Object.freeze((globalThis.process)["kill"])(+0)` / `Object.freeze((globalThis["process"])["kill"])(0)` / `Object.freeze((globalThis["process"])["kill"])(+0)` / `Object.freeze((process).kill)(0)` / `Object.freeze((process).kill)(+0)` / `Object.freeze((globalThis.process).kill)(0)` / `Object.freeze((globalThis.process).kill)(+0)` / `Object.freeze((globalThis["process"]).kill)(0)` / `Object.freeze((globalThis["process"]).kill)(+0)`, plus the transparent callable-wrapper alias `Object.freeze(globalThis["process"].kill)(0)` / `Object.freeze(globalThis["process"].kill)(+0)`, while broader kill/signal semantics remain later compatibility, and it now also supports the `process.env.KEY = ...` / `delete process.env.KEY` property-mutation slice (including the equivalent `globalThis.process.env...` and bracketed `process["env"]["KEY"]` / `delete process["env"]["KEY"]` forms), while the broader `Deno.env.set` / `Deno.env.delete` mutation path remains a Phase 3 target and stays unavailable on the Node surface.
