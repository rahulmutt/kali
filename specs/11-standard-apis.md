...
- Environment access: `Deno.env.get` *(exposes only the sandbox-permitted environment view rather than the raw host environment; bracketed `Deno["env"]["get"]` / `globalThis["Deno"]["env"]["get"]` source forms stay on the same read-only env slice; `Deno.env.toObject` remains a later object-materialization follow-up)*
