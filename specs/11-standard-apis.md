...
- `Deno.env.set` / `Deno.env.delete` *(the broader cross-surface mutable-env path is still a Phase 3 target; the current repository snapshot already exposes this slice on the default standalone surface, including the bracketed `Deno["env"]["set"]` / `Deno["env"]["delete"]` forms, while browser-targeted contexts still reject it)*
