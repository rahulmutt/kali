...
- process identity query `Deno.pid` / `globalThis.Deno.pid` is already available on the default standalone surface in the current repository snapshot, the read-only `Deno.cwd` / `globalThis.Deno.cwd` working-directory query is now available there too, and `Deno.chdir` / `globalThis.Deno.chdir` is now also available there as a sandbox-policy-mediated working-directory mutation API, while termination remains a later compatibility path until a future schema/policy revision gives it an auditable contract

Process identity `Deno.pid` / `globalThis.Deno.pid` is a read-only query on the default standalone surface in the current repository snapshot, while process termination (`Deno.exit`) and Node process-control exit (`process.exit`) remain later compatibility. `Deno.cwd` / `globalThis.Deno.cwd` is also available there as a read-only working-directory query, and `Deno.chdir` / `globalThis.Deno.chdir` is now available there as the working-directory mutation API. On the Node surface, `process.pid` is a read-only query and `process.cwd` is now also available as a read-only working-directory query in the current repository snapshot, and `process.chdir` is now also available there as the working-directory mutation API, while `process.exit` remains later compatibility. The dedicated later-compatibility rejection fixtures still include the bracketed `process["pid"]`, `process["exit"]`, and `globalThis.process["pid"]` / `globalThis.process["exit"]` spellings so the string-literal source-shape boundary stays explicit. Those latter APIs widen the embedding/sandbox contract but are not needed for the initial package-oriented baseline.
...
- therefore `Deno.exit` remains **Later compatibility** until a future schema/policy revision makes its sandbox contract explicit; `Deno.pid` / `globalThis.Deno.pid`, the read-only `Deno.cwd` / `globalThis.Deno.cwd` working-directory query, and `Deno.chdir` / `globalThis.Deno.chdir` stay noted as current-repository queries in the snapshot above
...
- `url` — URL parsing
- `assert` — assertions
- `process` — process global subset needed by real packages first (`env`, `argv`, selected control/query helpers); `process.pid` is already available as a read-only query on the Node surface, `process.cwd` is also available as a read-only query in the current repository snapshot, and `process.chdir` is now also available there as the working-directory mutation API, while `process.exit`-style process control stays on the later compatibility path until the policy and embedding contract for it is specified. The current repository snapshot also rejects `process.env = ...` assignment mutations on the Node surface, keeping the mutable-environment boundary explicit until the dedicated Phase 3 path lands.
