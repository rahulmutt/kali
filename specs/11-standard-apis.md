...
Process identity `Deno.pid` / `globalThis.Deno.pid` is a read-only query on the default standalone surface in the current repository snapshot, while process termination (`Deno.exit`) remains later compatibility. `Deno.cwd` / `globalThis.Deno.cwd` is also available there as a read-only working-directory query, and `Deno.chdir` / `globalThis.Deno.chdir` is now available there as the working-directory mutation API. On the Node surface, `process.pid` is a read-only query, `process.cwd` is now also available as a read-only working-directory query in the current repository snapshot, `process.chdir` is now also available there as the working-directory mutation API, and `process.exit` is now also available there as the Node process-control exit API. The dedicated later-compatibility rejection fixtures still include the bracketed `process["pid"]` and `globalThis.process["pid"]` spellings so the string-literal source-shape boundary stays explicit, while the Node-exit gate now tracks the supported Node surface directly. Those latter APIs widen the embedding/sandbox contract but are not needed for the initial package-oriented baseline.
