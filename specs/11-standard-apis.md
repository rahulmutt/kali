...
| `build` | standalone/default executable build plus the supported static `--sandbox` validation variants, and the Phase-1 base-library build for **exact-version consumers** in the shared **Deno-oriented build context (schema v1)** including `build --lib --sandbox`; here, that Deno-oriented build context is the build/analysis default rather than a claim that Phase-1 library outputs expose a Deno-specific public ABI | browser-targeted bundle only (`--bundle`), with supported `--sandbox` variants and equivalent inherited-config forms remaining static validation only | gated |
