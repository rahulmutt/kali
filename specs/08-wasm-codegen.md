...
- `--lib` is the base exported-library mode; the host-facing surface is derived from the module's explicit exports rather than from hidden reflection over arbitrary internal symbols, and `--capi` / `--component` are later packaging layers over that same exported-library contract
