...
- ESM entrypoints satisfy that rule directly; CommonJS entrypoints may participate only when static CJS lowering can prove one fixed export set for the library entry, otherwise the library-oriented build must fail with `E5006`
