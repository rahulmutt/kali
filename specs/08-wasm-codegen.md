...
- in early phases, `--lib`, `--capi`, and `--component` are **library-oriented artifact modes**: non-browser, export-oriented modes derived from a statically known export surface for the entry module
...
- `--lib` is the base exported-library mode; the host-facing surface is derived from the entry module's explicit exports after frontend lowering resolves that module to one fixed export set rather than from hidden reflection over arbitrary internal symbols, and `--capi` / `--component` are later packaging layers over that same exported-library contract
- ESM entry modules satisfy that rule directly; CommonJS entry modules may participate only when static CJS lowering can prove one fixed export set for the library entry module, otherwise the library-oriented build must fail with `E5006`
