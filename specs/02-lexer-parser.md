...
- Full grammar coverage for the latest published ECMA-262 edition, including:
  ...
  - Object literal expressions with identifier, string, and numeric property names, plus shorthand identifier properties and literal computed keys that canonicalize through transparent wrappers (for example parentheses, unary numeric wrappers, `await`, `as`, `satisfies`, `sequence` wrappers, and transparent `Object.freeze(...)` calls) to static property names
