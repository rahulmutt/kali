...
2. **Phase 2 — Ownership + effects**: MIR, ownership/escape analysis, deterministic memory management, the stable **public effect-report surface** (effect summaries/reporting), compile/check-time inferred-effect-vs-policy validation on top of the Phase-1 policy-schema/config validation path, and the stable **public embedding surface** (Rust API, public `--lib` + WIT contract, C ABI, and Component Model/C-embedding packaging).
