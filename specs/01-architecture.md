...
2. **Phase 2 — Ownership + effects**: MIR, ownership/escape analysis, deterministic memory management, the stable **public effect-report surface** (`kali effects`, `kali package-effects`, and compile/check-time inferred-effect-vs-policy validation), explicit `pure` / effect-annotation checking for the built-in capability model, and the stable **public embedding surface** (Rust API, public `--lib` + WIT contract, C ABI, and Component Model/C-embedding packaging).
