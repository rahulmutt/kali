...
2. **Phase 2 — Ownership + effects**: MIR, ownership/escape analysis, deterministic memory management, the stable **public effect-report surface** in both of its explicit halves (the reporting half: `kali effects`, `kali package-effects`; and the policy-comparison half: compile/check-time inferred-effect-vs-policy validation), explicit `pure` / effect-annotation checking for the built-in capability model, and the stable **public embedding surface** (Rust API plus the public plain `--lib` + WIT contract, with `--capi` and `--component` as explicit projections/packaging flows over that same exported interface rather than separate embedding semantics).
