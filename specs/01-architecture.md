...
2. **Phase 2 — Ownership + effects**: MIR, ownership/escape analysis, deterministic memory management, stable effect summaries/reporting, compile/check-time inferred-effect-vs-policy validation on top of the Phase-1 policy-schema/config validation path, and the first stable public embedding surfaces (Rust API, public `--lib` + WIT contract, C ABI, and component/C-embedding packaging).
