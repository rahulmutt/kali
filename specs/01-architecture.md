...
├── tests/                 — Target integration/conformance test tree
├── proofs/                — Target Lean proof tree; current repo baseline still starts with `proofs/BOUNDARY.md`
└── Cargo.toml             — Workspace manifest
```

Interpretation rule:
- follow the shared **current-repository-state vs target-contract reading** from [SPEC.md](../SPEC.md): a crate or top-level directory appearing in this workspace layout does **not** by itself imply that the current repository already contains that crate/directory, nor does it imply Phase 1 user-visible support or public-stability guarantees for that surface
- for example, `kali_api_node` may exist as an internal staging area before `--api node` is part of the supported command/profile matrix, `kali_embed` / `kali_capi` may exist as internal or pre-stable crates before the Phase-2 public embedding surface is frozen, and the verification side may still consist only of `proofs/BOUNDARY.md` before the target Lean tree lands.
...
1. **Phase 1 — Core compiler**: lexer, parser, AST, name resolution, TypeScript-compatible checking, first-class JavaScript compilation under the shared **bounded inference contract** (locals, obvious unannotated parameters, analyzable return types, the matching **annotation-required inference boundary** for the rest, and only budgeted local/intra-module constraint solving inside that same bounded fragment), HIR/LIR, simple WASM emission, the shared **Default standalone context (schema v1)** on the **Deno-oriented standalone surface** (including the shared **Web baseline**), the shared **Phase-1 browser-targeted command set** (rather than a standalone browser runtime), runtime sandbox enforcement plus policy-file/config validation, the Phase-1 **base library artifact** via `kali build --lib`, the core CLI workflow, and a library-first internal architecture so the CLI is built on reusable compiler/runtime crates.
...
2. **Phase 2 — Ownership + effects**: MIR, ownership/escape analysis, deterministic memory management, the stable **public effect-report surface** in both of its explicit halves (the reporting half: `kali effects`, `kali package-effects`; and the policy-comparison half: compile/check-time inferred-effect-vs-policy validation), explicit `pure` / effect-annotation checking for the built-in capability model, and the stable **public embedding surface** (Rust API plus the stable public **WIT-first** `--lib` contract, with `--capi` and `--component` as explicit projections/packaging flows over that same exported interface rather than separate embedding semantics).
3. **Phase 3 — Specialization + ecosystem**: specialization, advanced layout selection, broader npm package compatibility beyond the Phase 1 CJS/literal-`require` baseline, broader Node compatibility, broader browser packaging/interoperability beyond the Phase 1 bundle baseline, incremental compilation, stronger optimization, and broader higher-cost constraint solving once those compile-time budgets are explicit and evidence-backed.
...
| `release` | Standard optimization budget over the currently implemented optimizer pipeline; later phases deepen this mode with stronger MIR/layout-aware work without renaming it (selected by `--release`) |
| `release-advanced` | Highest documented compile-budget tier; later phases add deeper specialization, optional user-provided WASM post-pass integration, and LTO-style whole-program work here without inventing a second optimization vocabulary (selected by `--release-advanced`) |
...
- the **mode names themselves are Phase-1-stable**, but the optimization families available behind `release` / `release-advanced` still grow with the rest of the compiler pipeline; selecting a mode early does **not** imply every later MIR/LTO/post-pass optimization already exists in Phase 1
