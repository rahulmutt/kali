...
- `AssignmentExpression`, `ConditionalExpression`, `SequenceExpression` (including compound, nullish, and logical assignment operators such as `+=`, `??=`, `&&=`, and `||=`)
...
- `EffectDeclaration`, `PerformExpression`, `HandleExpression` for algebraic effects/handlers *(reserved later algebraic-effect surface; not part of the Phase 1 MVP and not a separate canonical status label)*
