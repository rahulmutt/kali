...
- under an effective API surface of `browser`, follow the **canonical browser-targeted policy boundary** from [SPEC.md](../SPEC.md): browser-targeted `--sandbox` validation stays inside the documented **Kali-mediated capability subset**, and cross-cutting `resources.*` runtime budgets that would imply Kali-hosted post-deployment enforcement are rejected for that profile
- concretely in schema v1, `resources.maxMemoryMB`, `resources.maxCpuTimeMs`, and `resources.maxOpenFiles` are rejected whenever present for browser-targeted `check` / `build --bundle`, while `resources.maxSpawnedProcesses` and `resources.maxThreads` reject positive values
...
- that subset is still a **global stable capability vocabulary** rather than a guarantee that every surface enables every member; browser-targeted validation admits only the browser-applicable members, while Deno/Node-only capability keys remain unavailable and must stay denied/omitted there
