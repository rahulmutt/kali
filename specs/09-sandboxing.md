...
- in schema v1, the valid browser-targeted capability families are therefore `effects.network.fetch`, `effects.timer.*`, `effects.random`, and `effects.console`, plus later `effects.eval` only when its separate compatibility path exists and is enabled
