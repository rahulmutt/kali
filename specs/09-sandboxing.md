...
- practical schema-v1 browser-targeted budget summary: `resources.maxMemoryMB`, `resources.maxCpuTimeMs`, and `resources.maxOpenFiles` are invalid whenever present; `resources.maxSpawnedProcesses` and `resources.maxThreads` may be omitted or set to `0`, but positive values remain unavailable until the corresponding subprocess/thread capability exists
