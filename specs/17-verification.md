...
- follow the shared **current-repository-state vs target-contract reading** from [SPEC.md](../SPEC.md): the repository now contains a checked-in Lean project tree under `proofs/`, and the published boundary is proof-backed for the widened closed fragment — now including assignment and try/catch in addition to literals, variables, closed functions, application, sequencing, and conditionals — plus a small RC snapshot safety slice (including live-reference ownership/allocation projection, release-update preservation, explicit release-recording, target-cell decrement bookkeeping, last-ref zeroing, zero-count collection, unrelated-heap preservation, disjointness on the decrement path, and a refcount-decrement update helper) and a widened HIR lowering-correctness slice while still remaining narrower than the later Stage 4.2 target
