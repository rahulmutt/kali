...
| Optimization/performance claims | version-pinned benchmark suite coverage, reproducible benchmark harness runs, and like-for-like build-mode comparisons against the claimed baseline; the canonical benchmark lane should include adapted Computer Language Benchmarks Game workloads derived from the Node.js / JavaScript submissions and normalized to Kali's TS/JS pipeline; those adapted CLBG fixtures source from published Node.js / JavaScript CLBG submissions, retain upstream attribution, and respect the CLBG license terms when vendored; and they must stay idiomatic TS ports of the JS submissions (no benchmark-specific intrinsic tuning beyond ordinary Kali compilation) so the comparisons remain honestly like-for-like |
...
| Pure-Rust toolchain invariant | a dependency-source audit / `cargo-deny` source-policy gate / build-graph scan proving no non-Rust compiled translation units enter the default toolchain — **including any build-time-vendored or compiled C/C++/asm units** (a `-sys`, `cc`, or `build.rs` path that compiles a non-Rust unit counts as an embedded C/C++ dependency), checked against the pure-Rust invariant owned by [`SPEC.md`](../SPEC.md); availability is cross-linked from [19 — Feature Maturity](./19-feature-maturity.md) |
...
- when the bootstrap brief names runnable npm binaries, record those as **binary-entrypoint probes** in the same corpus lane: the fixture should name the package, the invoked published bin entrypoint, the command/context under test, and the exact claimed support rung/outcome; to keep the probe reproducible it should also record the concrete Kali invocation path used to drive the bin on the standalone/Deno surface (`kali run --api deno <published-bin-entry> -- <args...>`) together with the canonical argv form (the `[-- args...]` vector, matching the `command: string[]` argv-style resolved command shape from [18 — Schemas](./18-schemas.md))
...
Unlike `@mariozechner/pi-coding-agent`, `semver` is intentionally tracked as **library/package content only**: it is the pure-JS/TS package probe, so its published CLI bin is **not** a required binary-entrypoint probe and the table deliberately omits a "semver published bin entrypoint" row. This resolves the apparent mismatch with the bootstrap brief's runnable-binary framing — the brief's executable-binary expectation is carried by the `@mariozechner/pi-coding-agent` published-bin row, while `semver` stays asserted at its library rung.
