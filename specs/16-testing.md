...
#### Kali-Specific Tests
- **Effect inference tests**: Source → expected effects JSON for the command's **resolved source graph** from [SPEC.md](../SPEC.md) rooted at the chosen analysis/logical root *(Phase 2 target; under the shared **effect-surface split**, Phase 1 may instead test internal effect-bookkeeping units without claiming the stable CLI/JSON surface)*
