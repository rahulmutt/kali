...
| `kali build --lib lib.ts` | Phase 1 MVP | Produce one linked export-oriented WASM artifact without any synthetic executable entry invocation; ordinary top-level module initialization still occurs on instantiation, Phase 1 emits the base `wasm-module` (`role: primary-library`), and Phase 2+ public-library builds add the default `wit` sidecar (`role: interface-wit`) |
