...
| Process identity and process-control/working-directory APIs (`Deno.pid` / `globalThis.Deno.pid`, `process.pid`, `Deno.exit`, `Deno.cwd`, `Deno.chdir`, `process.chdir`, `process.exit`) | Later compatibility | Deferred until a future schema/policy contract makes their sandbox and embedding behavior explicit |
