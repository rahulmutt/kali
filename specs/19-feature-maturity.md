...
| `kali package-audit --output json lodash` | Later compatibility | This is the same later-compatibility command with the canonical **envelope-only JSON support** model: until a dedicated audit payload schema exists, the stable schema-v1 machine contract is the standard command envelope alone (`payload` omitted or `null`), not an ad hoc audit JSON object, undocumented package/version metadata fields, or `stdout` / `stderr` repurposed as hidden result channels. Output-format selection does not create a separate availability path. |
