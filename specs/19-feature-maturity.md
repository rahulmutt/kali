...
| `kali install lodash` in canonical configless project mode | Phase 1 MVP | Follows the shared **configless install split** from [SPEC.md](../SPEC.md): create the minimal canonical manifest `{ "schemaVersion": 1 }` at the effective project root, then record the dependency and materialize/install it |
