...
- stronger **proof-backed** release/support claims for a wider boundary still require that the newly named claims be mechanized plus the corresponding proof jobs/evidence for the covered subset; the current published boundary now also names the pure release helper's ownership/allocation and disjointness corollaries, ownership-envelope preservation on the release-only, decrement, and collection helpers, alongside the local collection helper's positive-count-only final heap theorem and the other RC snapshot safety lemmas
