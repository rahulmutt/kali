...
| Build | `kali build <file>` and `kali build --lib <file>` in the default/inherited Deno-oriented build context (effective `apiSurface = deno`), plus the browser-bundle half of the shared **Phase-1 browser-targeted command set** (`kali build --bundle <file>` when the effective `apiSurface` is `browser`, including equivalent inherited-config forms and supported `--sandbox` variants); shipped static policy validation also covers the supported `kali build --sandbox <policy> <file>` and `kali build --lib --sandbox <policy> <file>` paths | no non-bundle browser build mode; no `--capi`; no `--component`; no Node build path yet |
...
| Effects/sandbox | internal sandbox-oriented effect bookkeeping; policy-schema/config validation for `check --sandbox` / `build --sandbox`; runtime enforcement for `run --sandbox` / `test --sandbox` | no stable public `kali effects`; no stable public `kali package-effects`; no inferred-effect-vs-policy rejection yet; no dry-run `run` / `test` effect-report workflow |
...
| Embedding | Phase-1 **base library artifact** via `kali build --lib` for exact-version/internal consumers in the default/inherited Deno-oriented build context (effective `apiSurface = deno`) | no stable public Rust embedding API; no stable public WIT sidecar for plain `--lib`; no stable public C ABI or Component Model flow; no cross-version host-loading guarantee yet |
...
| `kali check --sandbox kali.policy.json` | Phase 1 MVP | Reuse the same project-discovery behavior as plain `kali check`; Phase 1 validates policy schema/config over the same **resolved source graph** from [SPEC.md](../SPEC.md) selected by that discovered project, and from the Phase 2 target onward the same path also checks inferred effects against the policy |
| `kali check --sandbox kali.policy.json main.ts` | Phase 1 MVP | Same validation path, but over the **resolved source graph** from [SPEC.md](../SPEC.md) rooted at the explicit file set rather than the project-discovery case |
...
| `kali check --api browser --sandbox kali.policy.json` | Phase 1 MVP | Browser-targeted static policy validation over the same **resolved source graph** from [SPEC.md](../SPEC.md) selected by the discovered project, following the **browser-targeted static sandbox contract** from [SPEC.md](../SPEC.md) |
...
| `kali check --api browser --sandbox kali.policy.json main.ts` | Phase 1 MVP | Same browser-targeted static policy validation path, but over the **resolved source graph** from [SPEC.md](../SPEC.md) rooted at the explicit file set rather than the project-discovery case and still following the **browser-targeted static sandbox contract** |
...
- The Lean-backed verification story is phase-correct: the repository reaches the Phase-1 **proof-ready** baseline with one published **proof-boundary manifest** plus the proof-CI trigger policy, and any proof-backed claim stays scoped to the manifest only after it has moved beyond the shared **placeholder proof-boundary manifest** and actually names the modeled subset.
- If a Phase-1 release or support statement advertises formal verification as a shipped capability, the manifest must move beyond the shared **placeholder proof-boundary manifest** and name at least one concrete modeled subsystem plus theorem/property inventory; that placeholder state is acceptable only while Kali remains proof-ready but not yet proof-backed.
