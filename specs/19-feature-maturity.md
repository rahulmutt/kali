...
| `kali package-audit lodash` | Later compatibility | Tooling feature, not a Phase 1-2 compiler/runtime milestone; early `package-audit` is context-free, so inherited `apiSurface` / `runtimeProfiles` / `compat.features` / top-level `sandbox` do not change its semantics |
