...
| `kali build --bundle main.ts` under an inherited browser API surface | Phase 1 MVP | The same plain command spelling becomes the supported browser-bundle shortcut once the effective API surface already resolves to `browser`, matching explicit `kali build --bundle --api browser main.ts` |
...
| `kali build --lib --sandbox kali.policy.json lib.ts` under an inherited Node API surface | Phase 3 target | Same effective request as explicit `kali build --lib --api node --sandbox kali.policy.json lib.ts`; attaching `--sandbox` does not bypass the Node build gate for library-oriented modes. |
...
| `kali package-effects lodash` | Phase 2 target | This is the analysis-context-aware half of the shared **registry-analysis command split** from [SPEC.md](../SPEC.md): follow the shared **registry-analysis availability boundary** until the effect-report pipeline exists, and once it exists the command still uses the shared **effective inherited analysis context** rather than silently falling back when that inherited context selects an unavailable analysis mode. There is intentionally no blanket “all inherited non-default contexts are Phase 2” rule here: inherited-context maturity stays axis-specific under the shared **axis-aligned inherited analysis gating** rule from [SPEC.md](../SPEC.md). |
...
| `kali package-audit lodash` (and `--output json`) | Later compatibility | This is the context-free half of the shared **registry-analysis command split** from [SPEC.md](../SPEC.md): follow the shared **registry-analysis availability boundary** until the command exists. `--output json` follows the schema-owned **Package Audit JSON Output (schema v1)** contract from [18 — Schemas](18-schemas.md) without creating a separate availability path. |
| `kali package-audit --pretty --output json lodash` | Later compatibility | Same command/maturity path as `kali package-audit lodash`; `--pretty` only reformats the standard command envelope and does not create a second audit mode or a separate availability gate. |
...
| `kali run/test --max-spawned-processes N` with `N > 0` before subprocess support exists | Rejected by default | Same rule: positive values stay availability-gated and reject with `E5006` until the selected command/profile/API surface actually supports subprocesses |
...
| `kali run/test --max-threads N` with `N > 0` before thread support exists | Rejected by default | Same rule: positive values stay availability-gated and reject with `E5006` until the threaded runtime profile exists and the selected command/profile supports it |
