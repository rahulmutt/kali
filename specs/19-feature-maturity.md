...
| `kali effects main.ts` (and JSON-formatting forms such as `--pretty`, `--output json`, or both) | Phase 2 target | Base row for the reporting half of the shared **public effect-report surface**: before then the command stays unavailable or explicitly experimental, never a partial bespoke report. JSON-formatting selectors do not create an earlier path; once the command exists they change presentation only while the default API-surface selection still matches `check` (`apiSurface=deno`) unless overridden. |
