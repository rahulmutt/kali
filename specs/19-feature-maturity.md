...
| `kali package-effects lodash` (and JSON-formatting forms such as `--pretty` / `--output json`) | Phase 2 target | This is the analysis-context-aware half of the shared **registry-analysis command split** from [SPEC.md](../SPEC.md): follow the shared **registry-analysis availability boundary** until the effect-report pipeline exists, and once it exists the command still uses the shared **inherited analysis context** rather than silently falling back when that inherited context selects an unavailable analysis mode. JSON-formatting flags only affect presentation/envelope shape and do not create a second availability path. There is intentionally no blanket “all inherited non-default contexts are Phase 2” rule here: inherited-context maturity stays axis-specific under the shared **axis-aligned inherited analysis gating** rule from [SPEC.md](../SPEC.md). |
