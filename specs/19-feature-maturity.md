...
| Runtime object enumeration semantics | `Object.keys`, `Object.entries`, and `Object.values` preserve overwrite ordering in runtime smoke coverage, including mirrored `.js` fixtures for both cases and browser bundle smoke coverage for the string-primitive and overwrite-ordering slices in both `.ts` and `.js` inputs | broader object-model widening still follows later rows; integer-like key ordering remains later compatibility and is still tracked by ignored regression cases until a dedicated lowering fix lands |
