...
| `kali package-effects lodash` under an inherited non-default analysis context | Same maturity as the inherited axis | Follows the shared **axis-aligned inherited analysis gating** rule from [SPEC.md](../SPEC.md): inherited browser context lines up with browser-targeted effect analysis, inherited Node context lines up with the Node analysis gate, inherited `wasm-threads` lines up with the threaded-profile gate, and inherited `compat.features=["eval"]` lines up with the eval compatibility gate. Before the relevant gate opens, fail with `E5006` rather than silently falling back. |
