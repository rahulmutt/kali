...
| `kali package-audit --output json lodash` | Later compatibility | Same command as `kali package-audit lodash`; output formatting follows the schema-owned **Package Audit JSON Output (schema v1)** contract from [18 — Schemas](18-schemas.md). `--output json` does not create a separate availability path. |
