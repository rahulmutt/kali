...
| Build (standalone + base library) | `kali build <file>` in the shared **Deno-oriented build context (schema v1)**, plus `kali build --lib <file>` in that same context for exact-version/internal consumers when Kali can determine a **statically known export surface**; shipped static policy validation also covers the supported `kali build --sandbox <policy> <file>` and `kali build --lib --sandbox <policy> <file>` paths | no non-bundle browser build mode; no `--capi`; no `--component`; no Node build path yet |
