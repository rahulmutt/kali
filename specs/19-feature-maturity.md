...
| `kali package-effects lodash` under an inherited non-default analysis context | Phase 2 target | Once the base command exists, inherited-context maturity follows the shared **axis-aligned inherited analysis gating** rule from [SPEC.md](../SPEC.md); before the relevant inherited gate opens, fail with `E5006` rather than silently falling back. |
