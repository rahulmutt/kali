...
| Runtime object enumeration semantics | `Object.keys`, `Object.entries`, and `Object.values` preserve overwrite ordering and integer-like key ordering in runtime smoke coverage, including mirrored `.js` fixtures for both cases and browser bundle smoke coverage for the integer-like-key-ordering slice; browser bundle smoke also covers string-primitive enumeration on the same trio in both `.ts` and `.js` inputs | broader object-model widening still follows later rows |
