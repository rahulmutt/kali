...
| Runtime built-in math semantics | `Math.max`, `Math.min`, and `Math.abs` are lowered through host imports and covered by codegen/runtime smoke tests | broader built-in/object-model widening still follows later rows |
