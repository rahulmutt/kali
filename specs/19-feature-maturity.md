...
| Read-only `Deno.permissions` facade (query-only over the supported descriptor subset; stable `granted`/`denied` view only; no `request()` / `revoke()` escalation path) | Phase 1 MVP | Exposes Kali sandbox state for compatibility without interactive permission escalation or unsupported-permission drift |
...
| Read-only `Deno.permissions` facade over resolved policy state | Phase 1 MVP | Canonical **observation-only compatibility facade**: query-only over the supported descriptor subset, with stable granted/denied status and no interactive `request()` / `revoke()` escalation flows |
