...
| Threaded runtime profile | `run` / `test` now accept the explicit `--wasm-threads` opt-in on supported execution paths, and positive `--max-threads` values are honored only when that profile is active | analysis/build command widening and actual guest thread-spawn host imports still follow their later rows |
...
| `kali check --wasm-threads main.ts` | Later compatibility (opt-in only) | `check` participates in runtime-profile-sensitive analysis when relevant; reject with `E5506` until the threaded profile exists and the selected analysis mode supports it |
...
| `kali effects --wasm-threads main.ts` | Later compatibility (opt-in only) | `effects` records runtime-profile-sensitive analysis context; reject with `E5506` until the threaded profile exists and effect analysis supports it |
