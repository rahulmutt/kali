...
| `kali run --api browser main.ts` | Later compatibility (opt-in only) | If `KALI_BROWSER_BUNDLE_HARNESS_COMMAND` is configured, the standalone browser-requested run path may reuse the browser bundle harness; otherwise reject with `E5506` until a real browser-execution contract exists. Phase 1 browser-targeted support is still limited to the shared **Phase-1 browser-targeted command set** first |
...
| plain `kali run --sandbox kali.policy.json main.ts` under an inherited browser API surface | Later compatibility (opt-in only) | Same browser-execution gate as explicit `kali run --api browser --sandbox kali.policy.json main.ts`; attaching `--sandbox` does not turn browser into an early standalone runtime |
...
| `kali test --api browser` | Later compatibility (opt-in only) | If `KALI_BROWSER_BUNDLE_HARNESS_COMMAND` is configured, the standalone browser-requested test path may reuse the browser bundle harness; otherwise reject with `E5506` until a real browser-test contract exists. Early browser support is still limited to the shared **Phase-1 browser-targeted command set**, not a standalone test-runtime profile |
...
| plain `kali test --sandbox kali.policy.json` under an inherited browser API surface | Later compatibility (opt-in only) | Same browser-test gate as explicit `kali test --api browser --sandbox kali.policy.json`; attaching `--sandbox` does not turn browser into an early test-runtime profile |
...
| Standalone `run --api browser` | Later compatibility (opt-in only) | If `KALI_BROWSER_BUNDLE_HARNESS_COMMAND` is configured, the standalone browser-requested run path may reuse the browser bundle harness; otherwise reject with `E5506` until a real browser-execution contract exists |
