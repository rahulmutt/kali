...
| `kali check --api browser --sandbox kali.policy.json` | Phase 1 MVP | Browser-targeted static policy validation over the discovered project graph, following the **canonical browser-targeted policy boundary** from [SPEC.md](../SPEC.md) |
| `kali check --api browser --sandbox kali.policy.json main.ts` | Phase 1 MVP | Same browser-targeted static policy validation path, but scoped to the explicit file set rather than the discovered project graph and still following the canonical browser-targeted policy boundary |
...
| `kali build --bundle --api browser --sandbox kali.policy.json main.ts` | Phase 1 MVP | Browser-targeted static policy validation path only, following the **canonical browser-targeted policy boundary** from [SPEC.md](../SPEC.md) |
