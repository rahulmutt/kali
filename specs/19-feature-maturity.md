...
| `kali install --allow-scripts` | Opt-in only | Valid when the invocation has non-empty **effective npm-scriptable install work** (the npm-registry subset of the install work); it permits lifecycle hooks only for that subset during this invocation |
