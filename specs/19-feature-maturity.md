...
| Dynamic loading | literal-string `import()` lowering is live in the current repository snapshot for the linked-graph path; non-literal `import(expr)` still remains gated by the canonical `E5506` availability path | broader dynamic-loading compatibility still follows the later row for non-literal import and other host-mediated loading paths |
