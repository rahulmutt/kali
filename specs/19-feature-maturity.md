...
| Runtime built-in math semantics | `Math.max`, `Math.min`, `Math.abs`, `Math.sign`, `Math.imul`, and `Math.clz32` are lowered through host imports and covered by codegen/runtime smoke tests, including `run` / `test` smoke coverage on both TS and `.js` inputs plus browser bundle smoke coverage for the same math set in both TS and `.js` inputs | broader built-in/object-model widening still follows later rows |
