...
| Build | `kali build <file>` and `kali build --lib <file>` in the default/inherited non-browser build context, `kali build --bundle <file>` only when the effective `apiSurface` is `browser`, and supported `--sandbox` static policy validation on those shipped build paths including `kali build --lib --sandbox <policy> <file>` | no non-bundle browser build mode; no `--capi`; no `--component`; no Node build path yet |
