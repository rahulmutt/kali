...
| Registry analysis | none shipped in Phase 1 | no stable public `kali package-audit`; no separate stable registry-analysis workflow yet |
| Embedding | Phase-1 **base library artifact** via `kali build --lib` for exact-version/internal consumers in the shared **Deno-oriented build context (schema v1)** | no stable public Rust embedding API; no stable public WIT sidecar for plain `--lib`; no stable public C ABI or Component Model flow; no cross-version host-loading guarantee yet |
...
| `kali check --sandbox kali.policy.json` | Phase 1 MVP | Reuse the same project-discovery behavior as plain `kali check`; Phase 1 validates policy schema/config over the same **resolved source graph selected by the command's roots** from [SPEC.md](../SPEC.md), and from the Phase 2 target onward the same path also checks inferred effects against the policy |
...
| `kali check --api browser --sandbox kali.policy.json` | Phase 1 MVP | Browser-targeted static policy validation over the same **resolved source graph selected by the command's roots** from [SPEC.md](../SPEC.md), following the **browser-targeted static sandbox contract** from [SPEC.md](../SPEC.md) |
