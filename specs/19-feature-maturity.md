...
| `package.json#browser` replacement maps and `exports` condition `browser` in browser-targeted analysis/build contexts | Phase 1 MVP | Needed for practical browser-targeted npm compatibility without widening standalone runtime claims; supported browser-targeted commands should share one browser package-selection rule (browser `exports` condition order plus any applicable `package.json#browser` rewrites) rather than inventing per-command ladders |
