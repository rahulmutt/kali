...
| Late host/object-model compatibility gating | `check` / `run` / `test` and standalone `build` now reject late host-control and late object-model compatibility members in `.js` input with canonical gating/error paths, except for the read-only `Deno.pid` / `globalThis.Deno.pid` process-identity query on the default standalone surface; browser-targeted `check` / `build --bundle` carry the browser-specific rejection lanes, the browser-requested `run` / `test` harness path now also carries JSON-output rejection coverage for late object-model members in `.js` input, and standalone `check` / `run` / `test` / `build` now also carry the same text and JSON-output rejection coverage for the late compatibility set in `.js` input | late host/object-model APIs remain phase-gated unless and until their rows are promoted with evidence |
