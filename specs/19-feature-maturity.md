...
| Browser-targeted `check` and `build --bundle` | Phase 1 MVP | Browser-targeted analysis/build use the real browser ambient surface, and emitted browser bundles execute against the real browser host via the browser host adapter, with no standalone browser emulation; support claims require dedicated browser-targeted tests and real-browser bundle smoke coverage, and later browser-targeted analysis commands should reuse the same ambient typing layer and browser package-resolution context once their own rows become available |
