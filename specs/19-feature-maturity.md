...
| Still intentionally gated today | broad `--api node` support, positive `--wasm-threads` execution/profile support, and late host/object-model APIs such as process-control members, `Proxy`, weak/finalization APIs, and broader `Intl` remain phase-gated unless and until their rows are promoted with evidence; the documented Node execution subset is now live for `run` / `test`, and representative Node-package runtime cases now exercise both explicit and inherited `apiSurface=node` paths, but analysis/build breadth remains gated | those rows remain the authoritative future-state plan |
...
| `kali check --api node` | Phase 3 target | Reject with `E5506` until the documented Node typing/global subset exists; `check` keeps its project-discovery no-file form here too rather than inventing a node-specific direct-input requirement |
...
| `kali build --api node main.ts` | Phase 3 target | Reject with `E5506` until the documented Node subset lands for builds too |
