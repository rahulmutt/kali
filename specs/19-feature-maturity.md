...
| Browser package deployability | the browser package corpus now also mirrors the shared exports-map package set onto `.js` input for `check` and `build --bundle`, in addition to the existing browser replacement-map, interop, semver, string-entry/string-export/browser-condition/web-baseline/internal-rewrite, and runtime fixture coverage | broader browser package-support widening still follows the later package rows |
