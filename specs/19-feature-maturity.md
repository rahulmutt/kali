...
| `run --api browser` | Later compatibility | Early standalone runtime does not emulate a browser host; reject with `E5506` until a real browser-execution contract exists |
...
| `kali run --api browser main.ts` | Later compatibility | Reject with `E5506` until a real browser-execution contract exists; Phase 1 browser support is limited to the shared **Phase-1 browser-targeted command set** first |
