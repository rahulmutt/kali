...
| `kali install lodash` in a directory tree with no discovered `kali.json` | Phase 1 MVP | Create the minimal canonical manifest `{ "schemaVersion": 1 }` at the effective project root, then record the dependency and materialize/install it; explicit registry-package adds stay on the manifest path rather than inventing a configless side channel |
| plain `kali install` in a configless directory with no dependency inputs | Phase 1 MVP | Succeed as a no-op and do not create a placeholder `kali.json`; running the command alone is not a request to scaffold a project |
...
| `kali install --allow-scripts` | Opt-in only | Valid when the effective install graph includes at least one **npm** package; it permits lifecycle hooks for that npm portion of the graph during this invocation only |
| `kali install --allow-scripts` on a URL-only / JSR-only / no-npm graph | Rejected by default | If there is no npm-package install work for the flag to affect, fail with `E5008` instead of silently behaving like plain `install` |
...
| library-oriented build with a statically known export surface after frontend lowering | Phase 1 MVP | ESM entrypoints satisfy this directly; CommonJS entrypoints are acceptable only when static CJS lowering can prove one fixed export set for the entry module |
