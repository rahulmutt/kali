...
| `kali check --sandbox kali.policy.json main.ts` | Phase 1 MVP | Same validation path, but over the **resolved source graph selected by the explicit file set** rather than the project-discovery case |
...
| `kali check --api browser --sandbox kali.policy.json main.ts` | Phase 1 MVP | Same browser-targeted static policy validation path, but over the **resolved source graph selected by the explicit file set** rather than the project-discovery case and still following the **browser-targeted static sandbox contract** |
