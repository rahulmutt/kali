...
| `kali package-effects lodash` | Phase 2 target | This is the analysis-context-aware half of the shared **registry-analysis command split** from [SPEC.md](../SPEC.md): reject/mark experimental before the effect-report pipeline exists, and once it exists the command still uses the shared **effective inherited analysis context** rather than silently falling back when that inherited context selects an unavailable analysis mode. |
...
| `kali package-audit lodash` (and `--output json`) | Later compatibility | This is the context-free half of the shared **registry-analysis command split** from [SPEC.md](../SPEC.md): tooling feature, not a Phase 1-2 compiler/runtime milestone. `--output json` follows the schema-owned **Package Audit JSON Output (schema v1)** contract from [18 — Schemas](18-schemas.md) without creating a separate availability path. |
