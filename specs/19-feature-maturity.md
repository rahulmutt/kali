...
| Build | `kali build <file>` in the shared **Deno-oriented build context (schema v1)**, plus `kali build --lib <file>` in that same context for exact-version/internal consumers when Kali can determine a **statically known export surface**, plus the browser-bundle half of the shared **Phase-1 browser-targeted command set** (`kali build --bundle <file>` when the effective `apiSurface` is `browser`, including equivalent inherited-config forms and supported `--sandbox` variants); shipped static policy validation also covers the supported `kali build --sandbox <policy> <file>` and `kali build --lib --sandbox <policy> <file>` paths | no non-bundle browser build mode; no `--capi`; no `--component`; no Node build path yet |
| Browser support | exactly the shared **Phase-1 browser-targeted command set**: browser-targeted `check [files...]` *(including the project-discovery no-file form and explicit file sets)* plus browser-targeted `build --bundle <file>`, including equivalent inherited-config forms and supported `--sandbox` variants | no `run --api browser`; no `test --api browser`; no browser library/embed artifact modes |
...
| Embedding | Phase-1 **base library artifact** via `kali build --lib` for exact-version/internal consumers in the shared **Deno-oriented build context (schema v1)**, only for modules with a **statically known export surface** | no stable public Rust embedding API; no stable public WIT sidecar for plain `--lib`; no stable public C ABI or Component Model flow; no cross-version host-loading guarantee yet |
