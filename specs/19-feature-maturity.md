...
| plain `kali build --lib --sandbox kali.policy.json lib.ts` under an inherited Node API surface | Phase 3 target | Same effective request as explicit `kali build --lib --api node --sandbox kali.policy.json lib.ts`; attaching `--sandbox` does not bypass the Node build gate for library-oriented modes. |
