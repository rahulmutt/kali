...
| Async function / await sequencing | Phase 1 MVP | Basic async/await sequencing is covered by runtime-smoke regression; keep broader async iteration and cancellation semantics on the wider ECMA-262 conformance track. The `for await...of` array-iteration gate now also has `run` / `test` smoke coverage in TS and `.js` input, keeping the async-iteration syntax row separate from the supported-await grammar row. |
