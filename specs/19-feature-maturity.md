...
| Runtime object enumeration semantics | `Object.keys`, `Object.entries`, and `Object.values` preserve overwrite ordering in runtime smoke coverage, including mirrored `.js` fixtures for both cases and browser bundle smoke coverage for the string-primitive and overwrite-ordering slices in both `.ts` and `.js` inputs | broader object-model widening still follows later rows; integer-like key ordering remains later compatibility and is now rejected through a canonical `E5506` gate in the current runtime-smoke path, with the regression cases kept ignored until a dedicated lowering fix lands |
