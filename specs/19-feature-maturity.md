...
- canonical reading example: discovered `compilerOptions.apiSurface = browser` makes `kali build --bundle main.ts` map to the same supported row as explicit `kali build --bundle --api browser main.ts`, while that same inherited browser value still makes plain `kali build main.ts` fail on the browser build-shape contradiction path (`E5008`) until a non-bundle browser build mode exists
