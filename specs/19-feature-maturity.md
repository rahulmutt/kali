...
| Process identity and process-control/working-directory APIs (`process.pid`, `Deno.exit`, `Deno.cwd`, `Deno.chdir`, `process.chdir`, `process.exit`; read-only `Deno.pid` / `globalThis.Deno.pid` stays the default standalone exception) | Later compatibility | Deferred until a future schema/policy contract makes the remaining sandbox and embedding behavior explicit |
