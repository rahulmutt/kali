...
| Build | `kali build <file>`, `kali build --bundle <file>` only when the effective `apiSurface` is `browser`, `kali build --lib <file>`, and supported `--sandbox` static policy validation on shipped build paths including `kali build --lib --sandbox <policy> <file>` | no non-bundle browser build mode; no `--capi`; no `--component`; no Node build path yet |
...
| Registry analysis | none shipped in Phase 1 | no stable public `kali package-effects`; no stable public `kali package-audit`; no stable public registry-analysis workflow yet |
| Embedding | Phase-1 **base library artifact** via `kali build --lib` for exact-version/internal consumers | no stable public Rust embedding API; no stable public WIT sidecar for plain `--lib`; no stable public C ABI or Component Model flow; no cross-version host-loading guarantee yet |
...
| Stable public built-in effect reporting / `kali effects` / `kali package-effects` | Phase 2 target | This is the reporting half of the Phase-2 **public effect-report surface** side of that same split: stable user-facing effect JSON plus the reporting commands that expose it |
...
| Published Lean proof boundary + proof-CI trigger policy for the currently modeled subset | Phase 1 MVP | This is the Phase-1 **proof-ready** baseline: Kali needs an explicit proof boundary at `proofs/BOUNDARY.md` plus the proof-CI trigger policy over whatever subset is already modeled in Lean. During pre-proof iteration that boundary may still be empty; in that state the practical requirement is the published boundary plus its honest trigger policy rather than already-wired hosted proof automation |
...
| Real browser ambient surface for the **Phase-1 browser-targeted command set** (`kali check [files...]`, including the project-discovery no-file form and explicit-file-set forms, and `kali build --bundle <file>` when the effective `apiSurface` is `browser`, including equivalent inherited-config forms and supported `--sandbox` variants), including ambient DOM typings | Phase 1 MVP | Phase 1 exposes the real browser ambient surface only through the shared **Phase-1 browser-targeted command set**, including the DOM typings normally expected in browser programs, without claiming DOM support in Kali's standalone runtime; this status requires its own browser-targeted evidence track rather than inference from standalone runtime tests, and later analysis commands may reuse that same browser context only once their own maturity rows allow it |
...
| Phase-1 **base library artifact** (`kali build --lib`) | Phase 1 MVP | This is the Phase-1 side of the shared **embedding-stability split**: projects can build non-executable exported modules early without waiting for the later **public embedding surface** to freeze, but that early artifact is for exact-version/internal consumers only and is still not a stable public ABI/WIT or cross-version host-loading promise until the Phase-2 contract lands |
...
| `kali build --bundle main.ts` under the default/inherited non-browser API surface | Rejected by default | Invalid command usage (`E5008`): `--bundle` is reserved for browser-targeted output and therefore requires the effective API surface to be `browser` |
...
| `kali build --bundle --sandbox kali.policy.json main.ts` under the default/inherited non-browser API surface | Rejected by default | Invalid command usage (`E5008`): attaching `--sandbox` does not change the browser-only meaning of `--bundle`, so the effective API surface must still be `browser` |
...
| `kali build --lib lib.ts` | Phase 1 MVP | Produce one linked export-oriented **base library artifact** and therefore explicit library compile intent, following the shared **library-oriented instantiation rule** from [SPEC.md](../SPEC.md). Phase 1 emits the base `wasm-module` (`role: primary-library`) only, and support claims for that artifact stop at exact-version/internal consumers; the stable public library/WIT and cross-version host-loading surface is still Phase 2 work and then adds the default `wit` sidecar (`role: interface-wit`) |
| `kali build --lib --sandbox kali.policy.json lib.ts` | Phase 1 MVP | Same Phase-1 base-library build plus the ordinary build-time static policy-validation path. On `build`, `--sandbox` is orthogonal to artifact mode: it does not change compile intent, export-surface rules, or API-surface gating. |
...
| `kali package-effects` with no explicit package | Rejected by default | Violates the shared **single-package registry-analysis command** rule from [SPEC.md](../SPEC.md): omitting the explicit registry target is invalid command usage (`E5008`) |
| `kali package-effects lodash` (and JSON-formatting forms such as `--pretty`, `--output json`, or both) | Phase 2 target | This is the analysis-context-aware half of the shared **registry-analysis command split** from [SPEC.md](../SPEC.md): follow the shared **registry-analysis availability boundary** until the effect-report pipeline exists, and once it exists the command still uses the shared **inherited analysis context** rather than silently falling back when that inherited context selects an unavailable analysis mode. There is intentionally no blanket “all inherited non-default contexts are Phase 2” rule here: inherited-context maturity stays axis-specific under the shared **axis-aligned inherited analysis gating** rule from [SPEC.md](../SPEC.md). |
...
| `kali package-effects lodash` under an inherited browser analysis context | Phase 2 target | Once the base command exists, an inherited `apiSurface = browser` reuses the same browser-targeted analysis context/package-resolution rule as other browser analysis commands, but that later reuse still does **not** widen the exact **Phase-1 browser-targeted command set**. |
| `kali package-effects lodash` under an inherited Node analysis context | Phase 3 target | Once the base command exists, an inherited `apiSurface = node` reuses the same Node analysis gate as other source-graph analysis/effect commands; inherited config must not silently fall back to `deno`. |
| `kali package-effects lodash` under an inherited threaded analysis context (`runtimeProfiles = ["wasm-threads"]`) | Later compatibility (opt-in only) | Once the base command exists, inherited threaded analysis reuses the same runtime-profile gate as other effect-analysis paths; inherited config must not silently drop the threaded profile. |
| `kali package-effects lodash` under an inherited `eval` compatibility context (`compat.features = ["eval"]`) | Phase 4 compatibility | Once the base command exists, inherited compatibility selection reuses the same `eval` gate as other analysis/effect paths; inherited config must not silently remove `eval` to make the command succeed earlier. |
| `kali package-effects` with package-analysis-specific flags (`--api ...`, `--compat ...`, `--wasm-threads`, or `--sandbox ...`) | Rejected by default | Early package analysis inherits semantic context from config/defaults instead of taking its own package-analysis flag family, and `package-effects` is a reporting command rather than a second policy-validation entrypoint; these combinations are therefore invalid command usage (`E5008`) unless a later spec adds them |
...
| `kali package-audit` with no explicit package | Rejected by default | Violates the shared **single-package registry-analysis command** rule from [SPEC.md](../SPEC.md): omitting the explicit registry target is invalid command usage (`E5008`) rather than an implicit whole-project audit mode |
| `kali package-audit lodash` (and envelope-formatting forms such as `--output json` or `--pretty --output json`) | Later compatibility | This is the context-free half of the shared **registry-analysis command split** from [SPEC.md](../SPEC.md): follow the shared **registry-analysis availability boundary** until the command exists. JSON-formatting follows the schema-owned **Package Audit JSON Output (schema v1)** contract from [18 — Schemas](18-schemas.md) without creating a separate availability path. |
| `kali package-audit --pretty lodash` without `--output json` | Rejected by default | Invalid command usage (`E5008`): `--pretty` without `--output json` is not meaningful for schema v1's envelope-only `package-audit` JSON mode, and this command-shape error wins before the later command-availability gate. |
...
| `kali package-audit` with package-analysis-specific flags (`--api ...`, `--compat ...`, `--wasm-threads`, or `--sandbox ...`) | Rejected by default | Early `package-audit` follows **context-free registry analysis (schema v1)** from [SPEC.md](../SPEC.md) and intentionally stays a small single-package registry tool rather than growing host-analysis/runtime/sandbox flags; these combinations are therefore invalid command usage (`E5008`) unless a later spec adds them |
...
- The Lean-backed verification story is phase-correct: the repository reaches the Phase-1 **proof-ready** baseline with one published **proof-boundary manifest** plus the proof-CI trigger policy, and any proof-backed claim stays scoped to the non-placeholder manifest that actually names the modeled subset.
...
| The **Phase-1 browser-targeted command set** (`kali check [files...]`, including the project-discovery no-file form and explicit-file-set forms, and `kali build --bundle <file>` when the effective `apiSurface` is `browser`, including equivalent inherited-config forms and supported `--sandbox` variants) | Phase 1 MVP | These commands use the real browser ambient surface, and emitted browser bundles execute against the real browser host via the browser host adapter, with no standalone browser emulation; support claims require dedicated browser-targeted tests and real-browser bundle smoke coverage, and later browser-targeted analysis commands should reuse the same ambient typing layer and browser package-resolution context only once their own rows become available |
...
| Browser-condition package resolution for the **Phase-1 browser-targeted command set** | Phase 1 MVP | Shared browser package-resolution context for `kali check [files...]` *(including the project-discovery no-file form and explicit-file-set forms)* and `kali build --bundle <file>` when the effective `apiSurface` is `browser`, including equivalent inherited-config forms and their supported `--sandbox` variants: honor `exports` condition `browser` plus applicable `package.json#browser` replacement maps consistently |
