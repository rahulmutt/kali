...
| Real browser ambient surface for the **Phase-1 browser-targeted command set** (`kali check --api browser`, `kali build --bundle --api browser`), including ambient DOM typings | Phase 1 MVP | Phase 1 exposes the real browser ambient surface only through the shared **Phase-1 browser-targeted command set**, including the DOM typings normally expected in browser programs, without claiming DOM support in Kali's standalone runtime; this status requires its own browser-targeted evidence track rather than inference from standalone runtime tests, and later analysis commands may reuse that same browser context only once their own maturity rows allow it |
