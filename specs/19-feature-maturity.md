...
| Read-only `Deno.permissions` facade (query-only over the supported Deno-compatible descriptor subset: `read`, `write`, `net`, `env`, and later `run`; stable `granted`/`denied` view only; no `request()` / `revoke()` escalation path) | Phase 1 MVP | Exposes Kali sandbox state for compatibility without interactive permission escalation, while avoiding synthetic Kali-only permission names inside the Deno facade |
...
| `kali package-audit lodash` | Later compatibility | Tooling feature, not a Phase 1-2 compiler/runtime milestone; early `package-audit` follows **context-free registry analysis (schema v1)** from [SPEC.md](../SPEC.md). |
| `kali package-audit --output json lodash` | Later compatibility | This is the same later-compatibility command with the schema-owned **Package Audit JSON Output (schema v1)** contract from [18 — Schemas](18-schemas.md): until a dedicated audit payload schema exists, the stable machine-readable result is the standard command envelope alone (`payload` omitted or `null`). Output-format selection does not create a separate availability path. |
| `kali package-audit --pretty --output json lodash` | Later compatibility | Same maturity as `package-audit` itself; `--pretty` only reformats the active envelope JSON and must not be treated as a second feature gate or payload shape. |
| `kali package-audit --pretty lodash` | Rejected by default | Invalid command usage (`E5008`): `--pretty` without `--output json` is not meaningful for schema v1's envelope-only `package-audit` JSON mode, and this command-shape error wins before the later command-availability gate. |
...
| Read-only `Deno.permissions` facade over resolved policy state | Phase 1 MVP | Canonical **observation-only compatibility facade**: query-only over the supported Deno-compatible descriptor subset (`read`, `write`, `net`, `env`, and later `run`), with stable granted/denied status and no interactive `request()` / `revoke()` escalation flows |
