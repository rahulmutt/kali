...
| Published Lean proof boundary + CI for the currently modeled subset | Phase 1 MVP | This is the Phase-1 **proof-ready** baseline: Kali needs an explicit proof boundary at `proofs/BOUNDARY.md` plus CI over whatever subset is already modeled in Lean. During pre-proof iteration that boundary may still be empty, but then Kali must avoid proof-backed support claims until at least one concrete modeled theorem family exists and the release becomes genuinely **proof-backed** |
