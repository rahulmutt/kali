...
| Node compatibility breadth | `run` / `test` remain live on the documented Node execution subset, the Node analysis/build subset is now live for `check` / `build` on the documented Node-compatible sources, and `effects` / `package-effects` now also honor the documented Node analysis context; representative Node-package runtime cases now exercise both explicit and inherited `apiSurface=node` paths, the documented Node embedding-build subset now also has explicit and inherited `apiSurface=node` smoke coverage for `build --capi` and `build --component`, and the Node package corpus now also covers the canonical pure-JS `semver` probe on `.js` source-graph input across `check` / `build` / `run` / `test`, plus dedicated `node:buffer`, `node:path`, `node:crypto`, and `node:fs` built-in package slices on `.js` input across the same Node corpus lanes | late host/object-model APIs such as process-control members, `Proxy`, weak/finalization APIs, and broader `Intl` remain phase-gated unless and until their rows are promoted with evidence |
