...
| Threaded runtime profile | `run` / `test` now accept the explicit `--wasm-threads` opt-in on supported execution paths, and `check` / `build` / `effects` now also accept that opt-in on the supported non-browser analysis/build paths; positive `--max-threads` values are honored only when that profile is active | browser-targeted and registry-analysis rows still gate the profile separately, and actual guest thread-spawn host imports remain follow-up work |
