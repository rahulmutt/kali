...
| `kali install --allow-scripts` | Opt-in only | Valid when the invocation has non-empty **effective npm-scriptable install work**; that subset is invocation-scoped and covers the npm package work the install actually reconciles, including directly requested npm targets and transitively touched npm dependencies, while JSR/raw-URL work stays on the normal script-free path |
