...
| `kali check --api browser --sandbox kali.policy.json` | Phase 1 MVP | Browser-targeted static policy validation over the discovered project graph; any cross-cutting `resources.*` budget that would imply post-deployment enforcement is rejected because early browser-targeted commands do not promise Kali-hosted runtime-budget enforcement after deployment |
| `kali check --api browser --sandbox kali.policy.json main.ts` | Phase 1 MVP | Same browser-targeted static policy validation path, but scoped to the explicit file set rather than the discovered project graph |
