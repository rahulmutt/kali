...
- that deterministic `binding-package` sidecar manifest treats `moduleName`, `hostContract`, `runtimeBackend`, and its `artifacts.library` / `artifacts.metadata` / `artifacts.exportsHeader` references as non-empty, non-whitespace machine labels so binding workflows do not accept padded provenance or artifact-reference strings
...
- that export surface must be duplicate-free; repeated exported names must not be merged or shadowed into the emitted artifact surface
