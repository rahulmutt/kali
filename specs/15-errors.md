...
- before the base `kali effects` command is available, plain `kali effects main.ts` still reports the command-family availability gate (`E5006`) even if discovered config already selects an inherited browser/Node/runtime-profile/compatibility context; config inheritance must not silently simplify the request into some smaller ad hoc mode
