...
- well-formed base invocations of later registry-analysis commands, such as `kali package-effects lodash` before Phase 2 or `kali package-audit lodash` before that **Later compatibility** command exists
...
- this applies to registry-analysis commands too: malformed `package-effects` / `package-audit` invocations stay `E5008`, while the corresponding well-formed base invocations fall through to their command-availability gates
