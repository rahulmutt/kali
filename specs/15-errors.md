...
- plain `kali effects main.ts` under an inherited `compilerOptions.apiSurface = browser` or `compilerOptions.apiSurface = node`, or under inherited `compilerOptions.runtimeProfiles = ["wasm-threads"]` / `compat.features = ["eval"]`, because inherited effect-analysis context must hit the same availability gates as the corresponding explicit flags instead of silently simplifying the analysis mode
