...
- The accepted `query(...)` descriptors should be limited to the Deno-compatible subset Kali can answer without inventing Kali-only permission names inside the facade: `read`, `write`, `net`, `env`, and later `run` once subprocess support exists. Descriptors that would require unsupported host contracts or non-modeled Deno capabilities (for example `ffi`, `sys`, or any other non-modeled permission name in early phases) must fail explicitly through the same canonical availability path (`E5006`) instead of returning a misleading synthetic status.
