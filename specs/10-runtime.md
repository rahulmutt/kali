...
- In Phase 1 this is a query-only compatibility surface: the runtime may expose the minimal status-query behavior (`Deno.permissions.query(...)`), but `request()` / `revoke()`-style escalation methods are recognized-but-unavailable rather than implemented as no-op prompts.
...
- Any attempted `Deno.permissions.request(...)` / `revoke(...)` call must therefore fail through the canonical availability path (`E5006`), not through a fake prompt, a silent success path, or an accidental ordinary missing-member failure that would drift from checker behavior.
