...
- The accepted `query(...)` descriptors should be limited to the subset that maps onto Kali's documented capability model for the active phase/API surface. Descriptors that would require unsupported host contracts or non-modeled Deno capabilities (for example `ffi` or `sys` in early phases) must fail explicitly through the same canonical availability path (`E5006`) instead of returning a misleading synthetic status.
