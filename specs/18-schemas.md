...
- build result JSON envelopes may mirror the same provenance fields when the corresponding artifact mode emits them, so result payloads and sidecar metadata stay aligned instead of inventing two different provenance vocabularies; their required `sourceHash` label stays non-empty as well, and their `buildMode` field uses the same non-empty canonical `fast` / `release` / `release-advanced` vocabulary as compiler options and artifact metadata
