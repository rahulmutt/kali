...
- if that explicit registry-package add runs in a directory tree with no discovered `kali.json`, the command first creates the minimal canonical manifest `{ "schemaVersion": 1 }` at the effective project root, then records the dependency there; schema v1 intentionally keeps registry-package ownership in one manifest file rather than inventing a configless dependency sidecar
...
- plain `kali install` in a configless directory with no manifest/import/source dependency inputs is a no-op success and does **not** create a placeholder `kali.json`
