...
- CLI envelopes are fixed-shape in schema v1: top-level unexpected keys are rejected, and optional `artifacts` / `timings` fields are validated when present
...
- `summaryNote: non-empty, non-whitespace string` — stable note that summarizes the later browser runtime contract; validators compare the emitted value after trimming surrounding whitespace, but whitespace-only values still fail. This matches the CLI validator's trim-on-compare rule for the browser runtime contract.
- `contractScopeNote: non-empty, non-whitespace string` — stable note that summarizes the future browser runtime contract scope; validators compare the emitted value after trimming surrounding whitespace, but whitespace-only values still fail. This matches the CLI validator's trim-on-compare rule for the browser runtime contract.
...
Required fields:
- `path: non-empty, non-whitespace string`
- `kind: non-empty, non-whitespace string`
...
- if a producer receives repeated logical-root labels, it should deduplicate them in first-seen order before serialization so repeated invocations remain deterministic without changing the user-facing root spelling
...
- `package.name` therefore carries the registry-native package name only (`lodash`, `@types/node`, `@std/path`); it must not repeat a registry prefix such as `npm:` or `jsr:` inside `package.name`, and it must not contain internal whitespace in its canonical spelling; for JSR packages the `jsr:` identity marker is represented by `package.registry = "jsr"`, not duplicated inside `package.name`
- all three `PackageCoordinate` strings are canonical machine strings, so leading/trailing whitespace is rejected for `package.name`, `package.version`, and `package.registry` even though the field-level requirement is still phrased as a non-empty string; this keeps package-effect payloads deterministic and avoids hidden normalization
- `package.version` is the concrete resolved version actually analyzed. The CLI package argument is versionless in schema v1; for `package-effects`, that resolved version follows the shared **stable-release selection rule (schema v1)** from [SPEC.md](../SPEC.md) unless a later spec adds an explicit version/range selector, and schema v1 records it as a stable SemVer release string.
...
| `Process.Chdir` | `effects.process.chdir` |
...
- `compilerOptions.maxSpecializations` is the project-default specialization cap upper bound; schema v1 models it as a non-negative integer, defaults it to `16`, and CLI `--max-specializations` may override it per invocation
...
- `resources.maxSpawnedProcesses` follows the shared **feature-gated zero-capable execution budgets** rule from [SPEC.md](../SPEC.md): omission means no extra tightening from that source, `0` is a valid explicit deny/tightening value, and positive values are accepted once subprocess support exists; `resources.maxThreads` follows the same zero-capable pattern but remains availability-gated until the threaded profile exists
- schema v1 intentionally has no stable policy keys for process termination (`Deno.exit`) or Node process-control exit (`process.exit`); the working-directory mutation API `Deno.chdir` / `globalThis.Deno.chdir` and the Node `process.chdir` mutation are mediated by the schema-v1 `effects.process.chdir` capability, while the read-only working-directory query `Deno.cwd` / `globalThis.Deno.cwd` and the read-only process-identity query `Deno.pid` / `globalThis.Deno.pid` are available without introducing dedicated schema-v1 policy keys, and the documented Node `process.pid` query stays read-only without introducing a dedicated schema-v1 policy key as well
...
- Under an effective API surface of `browser`, that rejection still applies to Deno/Node-only capabilities, and schema-v1 follows the shared **canonical browser-targeted budget compatibility rule** from [SPEC.md](../SPEC.md). Those `resources.*` fields remain Kali-hosted execution budgets rather than post-deployment browser guarantees.
- In browser-targeted modes, policy/effect reasoning applies only to the browser-applicable part of the **Kali-mediated capability subset** defined by the shared **canonical browser-applicable mediated subset (schema v1)** in [SPEC.md](../SPEC.md). Deno/Node-only capability keys stay unavailable there, and ambient DOM/browser APIs outside that subset do not gain one policy key per operation.
...
- per-file coverage rows are normalized relative to the effective project root when available, emitted in deterministic file order, kept unique by file path, and keep each `file` label as a non-empty, non-whitespace string so the row identity stays deterministic
...
- artifact metadata sidecars may also carry provenance fields such as `runtimeProfiles`, `maxSpecializations`, `profileDataHash`, `hostContract`, and `runtimeBackend`; these describe the build/execution contract that produced the artifact, with `maxSpecializations` modeled as a non-negative integer upper bound, not a separate public embedding surface
- required machine labels such as `kaliVersion` and `sourceHash`, along with the optional provenance strings above, are canonical non-empty strings that must not carry leading or trailing whitespace, and they remain deterministic machine labels rather than free-form prose
...
- build result envelopes keep their emitted `outputPath` and any sidecar path references (`metadataPath`, `witPath`, `headerPath`, `bindingPackagePath`) as canonical non-empty strings that reject leading or trailing whitespace, artifact metadata keeps `entrypoint` / `apiSurface` as canonical non-empty strings that reject leading or trailing whitespace, and artifact entries keep both `path` and `kind` as non-empty, non-whitespace strings so linked artifact references and artifact labels stay deterministic
...
- build result JSON envelopes may mirror the same provenance fields when the corresponding artifact mode emits them, so result payloads and sidecar metadata stay aligned instead of inventing two different provenance vocabularies; their required `sourceHash` label stays canonical as well, and their `artifactKind`, `buildMode`, and where present `bundleFormat` labels use the same canonical exact `fast` / `release` / `release-advanced` or `esm` / `cjs` vocabulary as compiler options and artifact metadata, rather than accepting surrounding whitespace padding
