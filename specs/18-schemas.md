...
- build result envelopes keep their emitted `outputPath` and any sidecar path references (`metadataPath`, `witPath`, `headerPath`, `bindingPackagePath`) as non-empty, non-whitespace strings, and build-result artifact entries keep `path` non-empty as well so linked artifact references stay deterministic
