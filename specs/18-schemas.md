...
- `package.name` therefore carries the registry-native package name only (`lodash`, `@types/node`, `@std/path`); it must not repeat a registry prefix such as `npm:` or `jsr:` inside `package.name`, and for JSR packages the `jsr:` identity marker is represented by `package.registry = "jsr"`, not duplicated inside `package.name`
