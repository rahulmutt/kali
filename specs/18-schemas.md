...
- schema v1 intentionally has no stable policy keys for process termination or working-directory introspection/mutation (`Deno.exit`, `Deno.cwd`, `Deno.chdir`); the read-only process-identity query `Deno.pid` is available without introducing a dedicated schema-v1 policy key, and `process.pid` remains on the later Node-compatibility path until that surface is explicitly enabled
