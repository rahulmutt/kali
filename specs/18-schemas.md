...
- explicit CLI file/path arguments do not relocate that chosen config/root; in schema v1, file-accepting source commands (`run`, `build`, `check`, `effects`, `fmt`, `lint`, `test`) must keep those targets inside the effective project root and must not point into a nested child project that has its own `kali.json`
...
- in the canonical configless project mode, plain `kali install` is a no-op success when the effective project root contributes no manifest/import/source dependency inputs, and it does **not** create a placeholder `kali.json`
...
- `resources.maxSpawnedProcesses` caps concurrently active spawned processes once subprocess APIs exist; because zero concurrent spawned processes is meaningful, `0` is valid here and acts as the deny/disabled value until subprocess support exists. Before subprocess APIs exist, validation should reject values greater than `0` instead of accepting a non-functional budget for an unavailable capability
- `resources.maxThreads` is reserved for the later threaded runtime profile; because zero concurrent extra threads is meaningful, `0` is valid here and acts as the deny/disabled value until that profile exists. Before the threaded runtime profile exists, validation should reject values greater than `0` instead of silently accepting them
...
- Under an effective API surface of `browser`, that rejection still applies to Deno/Node-only capabilities, but it also applies to any cross-cutting `resources.*` budget that would imply post-deployment browser enforcement: `resources.maxMemoryMB`, `resources.maxCpuTimeMs`, and `resources.maxOpenFiles` are rejected whenever present, while `resources.maxSpawnedProcesses` and `resources.maxThreads` are rejected when set to positive values.
- In browser-targeted modes, policy/effect reasoning applies only to the **Kali-mediated capability subset** from [SPEC.md](../SPEC.md). That subset is the global stable capability vocabulary, but browser-targeted validation admits only the browser-applicable members of it. The shared Web-baseline capability keys (`effects.network.fetch`, `effects.timer.*`, `effects.random`, and `effects.console`) remain valid schema-v1 policy targets inside that subset, while Deno/Node-only capability keys stay unavailable and ambient DOM/browser APIs outside the subset do not gain one policy key per operation.
...
| `effects.network.maxConnections` | positive integer | maximum concurrent connections within the modeled network capability subset (for example Kali-mediated `fetch` / `connect` / `listen` paths when supported) |
