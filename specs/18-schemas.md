...
- `compilerOptions.apiSurface` influences command-time API/package selection for `check` / `effects` / `build` / `run` / `test`, but schema v1 does **not** imply separate per-surface lockfiles or install trees for the same manifest/import graph
...
- Under an effective `apiSurface` of `browser`, that rejection still applies to Deno/Node-only capabilities, but it also applies to **all non-deny `resources.*` values** because early browser-targeted `check` / `build --bundle` do not promise post-deployment enforcement of CPU, memory, file-handle, process, or thread budgets in the real browser host.
