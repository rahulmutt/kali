...
- In browser-targeted modes, policy/effect reasoning applies only to the **Kali-mediated capability subset** from [SPEC.md](../SPEC.md). The shared Web-baseline capability keys (`effects.network.fetch`, `effects.timer.*`, `effects.random`, and `effects.console`) remain valid schema-v1 policy targets inside that subset, but ambient DOM/browser APIs outside the subset do not gain one policy key per operation.
