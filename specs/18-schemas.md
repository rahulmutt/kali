...
- artifact metadata sidecars may also carry provenance fields such as `runtimeProfiles`, `maxSpecializations`, `hostContract`, and `runtimeBackend`; these describe the build/execution contract that produced the artifact, not a separate public embedding surface
