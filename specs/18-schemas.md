...
- `resources.maxThreads` is reserved for the later threaded runtime profile; because zero concurrent extra threads is meaningful, `0` is valid here and acts as the deny/disabled value until that profile exists. Before the threaded profile exists, validation should reject values greater than `0` instead of silently accepting them
