...
- `--pretty` changes formatting only: it pretty-prints the active JSON document (native payload by default, outer envelope when `--output json` is selected) without changing any field names or schema semantics
