...
- build result JSON envelopes may mirror the same provenance fields when the corresponding artifact mode emits them, so result payloads and sidecar metadata stay aligned instead of inventing two different provenance vocabularies; their required `sourceHash` label stays non-empty as well, and their `artifactKind`, `buildMode`, and where present `bundleFormat` labels use the same canonical exact `fast` / `release` / `release-advanced` or `esm` / `cjs` vocabulary as compiler options and artifact metadata, rather than accepting surrounding whitespace padding
