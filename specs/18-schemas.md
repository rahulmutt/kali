...
- Under `--api browser`, that rejection still applies to Deno/Node-only capabilities, but it also applies to **all non-deny `resources.*` values** because early browser-targeted `check` / `build --bundle` do not promise post-deployment enforcement of CPU, memory, file-handle, process, or thread budgets in the real browser host.
