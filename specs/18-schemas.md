...
- `primary-library` — the main export-oriented library artifact from `kali build --lib foo.ts` (no synthetic executable entry invocation; hosts instantiate it and use the build's **statically known export surface** as defined in [SPEC.md](../SPEC.md))
...
- In browser-targeted modes, policy/effect reasoning applies only to the browser-applicable part of the **Kali-mediated capability subset** defined by the **canonical browser-applicable mediated subset (schema v1)** in [SPEC.md](../SPEC.md). In schema v1 that means `effects.network.fetch`, `effects.timer.*`, `effects.random`, and `effects.console`, plus later `effects.eval` only when its separate compatibility path exists and is enabled. Deno/Node-only capability keys such as `effects.fileSystem.*`, `effects.process.*`, `effects.network.connect`, and `effects.network.listen` stay unavailable there, and ambient DOM/browser APIs outside that subset do not gain one policy key per operation.
