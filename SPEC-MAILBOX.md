...
- `specs/18-schemas.md`, `schemas/artifact-meta/v1.json`, and the build-side metadata emitters now include the optional `maxSpecializations` provenance field on artifact metadata sidecars so emitted build artifacts can record the effective specialization budget without widening the public build-mode vocabulary.
