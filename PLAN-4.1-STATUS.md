...
- `kali package-audit --preview lodash` is now rejected with the canonical `E5008` invalid-usage diagnostic rather than acting as a compatibility shim ✅
- package-audit smoke coverage now exercises both the no-findings path and a lifecycle-script warning path via a local registry-metadata server ✅
...
- The `package-audit` CLI path no longer advertises `--preview` in help output, and the removed shim now fails with `E5008` instead of being accepted as a hidden no-op
- The package-audit implementation now surfaces registry-metadata warnings for lifecycle scripts and errors for native-addon markers instead of a scaffold placeholder
