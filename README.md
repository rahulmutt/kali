...
- current repository implementation status also matches that split: the Lean proof tree described in [specs/17-verification.md](./specs/17-verification.md) is a target layout for when proofs land, not a claim that those files already exist today
- the same spec-first reading also applies to the workspace/test architecture described in [specs/01-architecture.md](./specs/01-architecture.md) and [specs/16-testing.md](./specs/16-testing.md): those crate/test/CI layouts are target implementation structures, not claims that the current repo already contains every listed crate, fixture tree, or automation lane
- the same target-vs-current distinction also applies to crate references in [specs/11-standard-apis.md](./specs/11-standard-apis.md), [specs/12-cli.md](./specs/12-cli.md), and [specs/13-embedding.md](./specs/13-embedding.md): names such as `kali_fmt`, `kali_lint`, `kali_embed`, and `kali_capi` describe the intended implementation/package structure, not current-repo facts unless the repository actually grows those crates
