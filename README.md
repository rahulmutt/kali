...
- **Registry analysis**: no stable public registry-analysis command ships in Phase 1. Follow the shared **registry-analysis command split** from [`SPEC.md`](./SPEC.md): `kali package-effects <package>` is the Phase-2 single-package registry-analysis/effect-report command, while `kali package-audit <package>` remains **Later compatibility**. Output shape stays split too: `package-effects` is one of the later **native-JSON commands**, while `package-audit` remains the schema-v1 **envelope-only JSON command** when it eventually ships.
