...
- **Sandboxing/effects**: `run/test --sandbox` enforce at runtime; `kali check --sandbox` plus the shared **shipped Phase-1 build-side sandbox paths** do static policy-schema/config validation only in Phase 1; later public effect reporting stays on the explicit `kali effects <file>` / `kali package-effects <package>` commands rather than a `run/test --dry` side path.
