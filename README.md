...
- **Sandboxing/effects**: `run/test --sandbox` enforce at runtime; shipped `check/build --sandbox` paths do static policy-schema/config validation only in Phase 1; later public effect reporting stays on the explicit `kali effects <file>` / `kali package-effects <package>` commands rather than a `run/test --dry` side path.
...
- **Registry analysis**: no stable public `kali package-effects` or `kali package-audit` command ships in Phase 1.
