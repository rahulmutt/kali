...
- **Sandboxing/effects**: `run/test --sandbox` enforce at runtime; the supported Phase-1 `check --sandbox` and `build --sandbox` paths — including browser-targeted `check --sandbox`, browser-targeted `build --bundle --sandbox`, and `build --lib --sandbox` — do static policy-schema/config validation only in Phase 1.
