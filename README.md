...
- Lean-backed verification is also phase-scoped: support claims should read through the published **proof-boundary manifest** at `proofs/BOUNDARY.md` and the shared **proof activation split** in [SPEC.md](./SPEC.md) rather than assuming whole-language or whole-host verification from the start
