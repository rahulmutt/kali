...
- **Sandboxing/effects**: `run/test --sandbox` enforce at runtime; the supported Phase-1 `check --sandbox` paths and shipped `build --sandbox` paths (default executable build, the browser-targeted `build --bundle` path inside the shared **Phase-1 browser-targeted command set**, and `build --lib`) do static policy-schema/config validation only in Phase 1.
