...
- host support follows one small staircase: shared **Web baseline**, **Deno-first** standalone execution, the shared **Phase-1 browser-targeted command set** (`check --api browser`, `build --bundle --api browser`), and broader **Node compatibility** as a later ecosystem phase
...
- registry-analysis commands intentionally stay simpler than source-graph commands: `package-effects` inherits `apiSurface` / `runtimeProfiles` / `compat.features` from config/defaults rather than taking its own `--api` / `--wasm-threads` / `--compat` flag family, while `package-audit` is context-free in schema v1
