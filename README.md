...
- **Sandboxing/effects**: `run/test --sandbox` enforce at runtime; the supported Phase-1 `check --sandbox` paths and shipped `build --sandbox` paths (default executable build, the browser-targeted `build --bundle` path inside the shared **Phase-1 browser-targeted command set**, and `build --lib`) do static policy-schema/config validation only in Phase 1; later public effect reporting stays on the explicit `kali effects <file>` / `kali package-effects <package>` commands rather than a `run/test --dry` side path.
- **Packages**: early support is broad only inside the shared **pure JS/TS package contract** plus the documented raw-URL workflow, and claims still depend on the exact rung being asserted (`installable/materializable`, `checkable`, `buildable`, `executable`, or `deployable-through-host`).
