...
- **“formal verification”** means Kali is expected to be **proof-ready** in Phase 1, but must not be described as **proof-backed** until `proofs/BOUNDARY.md` moves beyond the provisional proof-ready staging state—whether that state is the empty placeholder manifest or a provisional non-empty model—and the named theorem/property claims are fully mechanized
