...
- **Project workflow**: `kali init`, `kali init --lib`, `kali install`, `kali fmt`, `kali lint`, and `kali check [files...]` ship as the main Phase-1 authoring loop. `init` stays intentionally minimal: it creates only the smallest valid scaffold (`kali.json` + `main.ts` by default, or `kali.json` + `lib.ts` with `--lib`) and does not materialize dependencies or write `kali.lock`. Dependency state remains intentionally single-owner: non-install commands fail on missing/stale dependency state instead of auto-repairing it, while plain configless `kali install` with no dependency inputs is a no-op rather than an implicit project scaffold.
