...
- **“formal verification”** means Kali is expected to be **proof-ready** in Phase 1, but the published proof boundary may be described as **proof-backed** once its named theorem/property claims are mechanized; the current boundary in `proofs/BOUNDARY.md` is proof-backed for the widened closed fragment plus a small RC snapshot safety slice, but it does not claim the later Stage 4.2 ownership/memory-safety or lowering-correctness target
