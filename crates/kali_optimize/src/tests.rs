...
fn build_bracketed_global_this_object_string_enumeration_call(
    builder: &mut LirBuilder,
    callee_name: &str,
    string_value: &str,
) -> LirNodeId {
...
fn build_global_this_object_string_enumeration_call(
    builder: &mut LirBuilder,
    callee_name: &str,
    string_value: &str,
) -> LirNodeId {
...
fn build_bracketed_reflect_own_keys_call(builder: &mut LirBuilder, callee_name: &str) -> LirNodeId {
    let call = builder.alloc(LirNodeKind::Call);
    let callee = builder.alloc_text(LirNodeKind::Value, callee_name);
...
fn build_global_this_reflect_own_keys_call(
    builder: &mut LirBuilder,
    callee_name: &str,
) -> LirNodeId {
    let call = builder.alloc(LirNodeKind::Call);
...
fn build_object_has_own_callee(builder: &mut LirBuilder, callee_name: &str) -> LirNodeId {
    let callee = builder.alloc_text(LirNodeKind::Value, callee_name);
    let object_object = builder.alloc_text(LirNodeKind::Value, "Object");
    builder.node_mut(callee).unwrap().children = vec![object_object];
    callee
...
fn assert_global_this_reflect_bracketed_own_keys_folds(level: OptimizationLevel) {
    let mut builder = LirBuilder::new();
    let root = builder.alloc(LirNodeKind::Program);
