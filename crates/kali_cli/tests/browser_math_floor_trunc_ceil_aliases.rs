...
//! (`:109-118`) has no helper: its whole body is a single
//! `assert!(source.contains(expected))` self-check (`:113-116`) run in a `for`
...
//! (`:96-106`), before any command is built and without ever invoking `kali`.
//!
...
//!   RED  comment_coverage.py          exit 1, 57 missing lines -- this
