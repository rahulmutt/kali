...
fn optimization_benchmark_suite_tracks_compile_time_size_and_speed() {
    for (fixture_stem, benchmark_name) in [
        ...
        // no longer do.
        //
        // MEASURED AGAIN 2026-09-10 by the release-tier-allocation-identity project
        // (docs/superpowers/specs/2026-09-10-release-tier-allocation-identity-design.md):
        // the defect the 2026-09-09 re-pin recorded (the release-mode optimizer
        // inlining an allocating `new Array(n).fill(v)` initializer as a pure
        // constant) is fixed, but that did NOT restore these two. `spectral-norm`
        // now builds at every tier but the release tiers emit an UNLOADABLE module
        // (E4201); `nbody` still refuses outright with E5506, from a different,
        // still-open mechanism (its `bodies` array literal has `Call` nodes as
        // elements, so it never enters the spec env this project's fix narrowed).
        // Both remain excluded from this list. Neither is dropped from coverage:
        // both are pinned, per-tier, as `Expectation::KnownBroken` in the standing
        // gate `crates/kali_cli/tests/inprocess/benchmark_execution.rs`, which is
        // where their measured behaviour now lives -- read that before adding
        // either name back here.
        //
    ...
    // spectral-norm-benchmark-v1 and nbody-benchmark-v1: measured 2026-09-10 by
    // the release-tier-allocation-identity project
    // (docs/superpowers/specs/2026-09-10-release-tier-allocation-identity-design.md).
    // spectral-norm builds at every tier but Release/ReleaseAdvanced emit an
    // UNLOADABLE module (E4201); nbody still refuses outright with E5506 from a
