...
//!       `if json_output {` at line 271, 16 dead claim(s) in 2 guarded block(s)
//!   `assert_browser_bundle`
//!       `if json_output {` at line 329, 7 dead claim(s) in 2 guarded block(s)
//!
...
fn standalone_source() -> &'static str {
    r##"const values = [1, 2, 1]; const mapValues = [[1, 2], [1, 3], [4, 5]]; const nullishFullyBracketedSingleQuotedArrayFrom = Object.freeze((null ?? globalThis['Array']['from'])); const andFullyBracketedSingleQuotedArrayFrom = Object.freeze((true && globalThis['Array']['from'])); const orFullyBracketedSingleQuotedArrayFrom = Object.freeze((false || globalThis['Array']['from'])); for (const value of nullishFullyBracketedSingleQuotedArrayFrom(new Set(values))) { console.log(value); } for (const value of andFullyBracketedSingleQuotedArrayFrom(new Set(values))) { console.log(value); } for (const value of orFullyBracketedSingleQuotedArrayFrom(new Set(values))) { console.log(value); } for (const entry of nullishFullyBracketedSingleQuotedArrayFrom(new Map(mapValues))) { console.log(entry[0]); console.log(entry[1]); } for (const entry of andFullyBracketedSingleQuotedArrayFrom(new Map(mapValues))) { console.log(entry[0]); console.log(entry[1]); } for (const entry of orFullyBracketedSingleQuotedArrayFrom(new Map(mapValues))) { console.log(entry[0]); console.log(entry[1]); }"##
}

fn browser_run_source() -> &'static str {
    r##"async function browserArrayFromFullyBracketedSingleQuotedWrappers() {
  const values = [1, 2, 1];
  const mapValues = [[1, 2], [1, 3], [4, 5]];
  const nullishFullyBracketedSingleQuotedArrayFrom = Object.freeze((null ?? globalThis['Array']['from']));
  const andFullyBracketedSingleQuotedArrayFrom = Object.freeze((true && globalThis['Array']['from']));
  const orFullyBracketedSingleQuotedArrayFrom = Object.freeze((false || globalThis['Array']['from']));
  for (const value of nullishFullyBracketedSingleQuotedArrayFrom(new Set(values))) {
    console.log(value);
  }
  for (const value of andFullyBracketedSingleQuotedArrayFrom(new Set(values))) {
    console.log(value);
  }
  for (const value of orFullyBracketedSingleQuotedArrayFrom(new Set(values))) {
    console.log(value);
  }
  for (const entry of nullishFullyBracketedSingleQuotedArrayFrom(new Map(mapValues))) {
    console.log(entry[0]);
    console.log(entry[1]);
  }
  for (const entry of andFullyBracketedSingleQuotedArrayFrom(new Map(mapValues))) {
    console.log(entry[0]);
    console.log(entry[1]);
  }
  for (const entry of orFullyBracketedSingleQuotedArrayFrom(new Map(mapValues))) {
    console.log(entry[0]);
    console.log(entry[1]);
  }
}

browserArrayFromFullyBracketedSingleQuotedWrappers();
"##
}

fn browser_test_source() -> &'static str {
    r##"Kali.test('fully bracketed single-quoted Array.from wrappers', () => {
  async function browserArrayFromFullyBracketedSingleQuotedWrappers() {
    const values = [1, 2, 1];
    const mapValues = [[1, 2], [1, 3], [4, 5]];
    const nullishFullyBracketedSingleQuotedArrayFrom = Object.freeze((null ?? globalThis['Array']['from']));
    const andFullyBracketedSingleQuotedArrayFrom = Object.freeze((true && globalThis['Array']['from']));
  ...
  return browserArrayFromFullyBracketedSingleQuotedWrappers();
});
"##
}

fn browser_bundle_source() -> &'static str {
    r##"// kali-tree-shake: browserArrayFromFullyBracketedSingleQuotedWrappers
export async function browserArrayFromFullyBracketedSingleQuotedWrappers() {
  const values = [1, 2, 1];
  const mapValues = [[1, 2], [1, 3], [4, 5]];
  const nullishFullyBracketedSingleQuotedArrayFrom = Object.freeze((null ?? globalThis['Array']['from']));
  const andFullyBracketedSingleQuotedArrayFrom = Object.freeze((true && globalThis['Array']['from']));
...
fn assert_browser_bundle(filename: &str, json_output: bool) {
    ...
    let harness = browser_bundle_harness_script(
        "app",
        false,
        "const mod = await import(bundleJs.href);\nawait mod.browserArrayFromFullyBracketedSingleQuotedWrappers();\nconsole.log('browser fully bracketed single-quoted Array.from wrapper aliases ok');\n",
    );
...
#[test]
fn run_supports_fully_bracketed_single_quoted_array_from_wrappers_in_js_input() {
    assert_standalone("run", "main.js");
}

#[test]
fn test_supports_fully_bracketed_single_quoted_array_from_wrappers_in_js_input() {
    assert_standalone("test", "smoke.test.js");
}

#[test]
fn run_supports_fully_bracketed_single_quoted_array_from_wrappers_in_js_input_when_browser_harness_is_configured(
) {
    assert_browser_requested("run", "main.js", false);
}

#[test]
fn run_supports_fully_bracketed_single_quoted_array_from_wrappers_in_ts_jsx_and_tsx_input_when_browser_harness_is_configured(
) {
...
#[test]
fn test_supports_fully_bracketed_single_quoted_array_from_wrappers_in_js_input_when_browser_harness_is_configured(
) {
    assert_browser_requested("test", "smoke.test.js", false);
}

#[test]
fn test_supports_fully_bracketed_single_quoted_array_from_wrappers_in_ts_jsx_and_tsx_input_when_browser_harness_is_configured(
) {
...
#[test]
fn build_bundles_fully_bracketed_single_quoted_array_from_wrappers_in_js_input() {
    assert_browser_bundle("app.js", false);
}

#[test]
fn build_bundles_fully_bracketed_single_quoted_array_from_wrappers_in_ts_jsx_and_tsx_input() {
