...
//! (`:140`) has no helper: its whole body is a single `assert!(...)` self-check
//! (`:145`) run in a `for` loop over
...
//! (`:132`)'s OWN TEXT, before any command is built and without ever invoking
//! `kali`.
//!
