...
#[test]
fn json_test_falls_back_to_stdout_when_browser_summary_file_has_unexpected_top_level_keys_when_browser_harness_is_configured_in_jsx_input(
) {
    let dir = tempdir().expect("tempdir");
    let source_path = dir.path().join("unexpected-summary.test.jsx");
    write_jsx_source(&source_path);
