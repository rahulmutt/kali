...
fn standalone_source() -> &'static str {
    r##"const values = [1, 2, 1]; const mapValues = [[1, 2], [1, 3], [4, 5]]; const nullishDotRootArrayFrom = Object.freeze((null ?? globalThis.Array.from)); const andDotRootArrayFrom = Object.freeze((true && globalThis.Array.from)); const orDotRootArrayFrom = Object.freeze((false || globalThis.Array.from)); for (const value of nullishDotRootArrayFrom(new Set(values))) { console.log(value); } for (const value of andDotRootArrayFrom(new Set(values))) { console.log(value); } for (const value of orDotRootArrayFrom(new Set(values))) { console.log(value); } for (const entry of nullishDotRootArrayFrom(new Map(mapValues))) { console.log(entry[0]); console.log(entry[1]); } for (const entry of andDotRootArrayFrom(new Map(mapValues))) { console.log(entry[0]); console.log(entry[1]); } for (const entry of orDotRootArrayFrom(new Map(mapValues))) { console.log(entry[0]); console.log(entry[1]); }"##
}

fn standalone_test_source() -> &'static str {
    r##"Kali.test('nullish/logical globalThis.Array.from wrappers', () => { const values = [1, 2, 1]; const mapValues = [[1, 2], [1, 3], [4, 5]]; const nullishDotRootArrayFrom = Object.freeze((null ?? globalThis.Array.from)); const andDotRootArrayFrom = Object.freeze((true && globalThis.Array.from)); const orDotRootArrayFrom = Object.freeze((false || globalThis.Array.from)); for (const value of nullishDotRootArrayFrom(new Set(values))) { console.log(value); } for (const value of andDotRootArrayFrom(new Set(values))) { console.log(value); } for (const value of orDotRootArrayFrom(new Set(values))) { console.log(value); } for (const entry of nullishDotRootArrayFrom(new Map(mapValues))) { console.log(entry[0]); console.log(entry[1]); } for (const entry of andDotRootArrayFrom(new Map(mapValues))) { console.log(entry[0]); console.log(entry[1]); } for (const entry of orDotRootArrayFrom(new Map(mapValues))) { console.log(entry[0]); console.log(entry[1]); } });"##
}

fn browser_run_source() -> &'static str {
    r##"async function browserArrayFromGlobalThisDotRootWrappers() {
  const values = [1, 2, 1];
  const mapValues = [[1, 2], [1, 3], [4, 5]];
  const nullishDotRootArrayFrom = Object.freeze((null ?? globalThis.Array.from));
  const andDotRootArrayFrom = Object.freeze((true && globalThis.Array.from));
  const orDotRootArrayFrom = Object.freeze((false || globalThis.Array.from));
  for (const value of nullishDotRootArrayFrom(new Set(values))) {
    console.log(value);
  }
  for (const value of andDotRootArrayFrom(new Set(values))) {
    console.log(value);
  }
  for (const value of orDotRootArrayFrom(new Set(values))) {
    console.log(value);
  }
  for (const entry of nullishDotRootArrayFrom(new Map(mapValues))) {
    console.log(entry[0]);
    console.log(entry[1]);
  }
  for (const entry of andDotRootArrayFrom(new Map(mapValues))) {
    console.log(entry[0]);
    console.log(entry[1]);
  }
  for (const entry of orDotRootArrayFrom(new Map(mapValues))) {
    console.log(entry[0]);
    console.log(entry[1]);
  }
}

browserArrayFromGlobalThisDotRootWrappers();
"##
}

fn browser_test_source() -> &'static str {
    r##"Kali.test('nullish/logical globalThis.Array.from wrappers', () => {
  async function browserArrayFromGlobalThisDotRootWrappers() {
    const values = [1, 2, 1];
    const mapValues = [[1, 2], [1, 3], [4, 5]];
    const nullishDotRootArrayFrom = Object.freeze((null ?? globalThis.Array.from));
    const andDotRootArrayFrom = Object.freeze((true && globalThis.Array.from));
  ...
  return browserArrayFromGlobalThisDotRootWrappers();
});
"##
}

fn browser_bundle_source() -> &'static str {
    r##"// kali-tree-shake: browserArrayFromGlobalThisDotRootWrappers
export async function browserArrayFromGlobalThisDotRootWrappers() {
  const values = [1, 2, 1];
  const mapValues = [[1, 2], [1, 3], [4, 5]];
  const nullishDotRootArrayFrom = Object.freeze((null ?? globalThis.Array.from));
  const andDotRootArrayFrom = Object.freeze((true && globalThis.Array.from));
...
fn assert_browser_bundle(filename: &str, json_output: bool) {
    ...
    let harness = browser_bundle_harness_script(
        "app",
        false,
        "const mod = await import(bundleJs.href);\nawait mod.browserArrayFromGlobalThisDotRootWrappers();\nconsole.log('browser globalThis.Array.from wrapper aliases ok');\n",
    );
...
#[test]
fn run_supports_nullish_and_logical_global_this_array_from_wrappers_in_js_input() {
    assert_standalone("run", "main.js");
}

#[test]
fn test_supports_nullish_and_logical_global_this_array_from_wrappers_in_js_input() {
    assert_standalone("test", "smoke.test.js");
}

#[test]
fn run_supports_nullish_and_logical_global_this_array_from_wrappers_in_js_input_when_browser_harness_is_configured(
) {
    assert_browser_requested("run", "main.js", false);
}

#[test]
fn run_supports_nullish_and_logical_global_this_array_from_wrappers_in_ts_jsx_and_tsx_input_when_browser_harness_is_configured(
) {
...
#[test]
fn test_supports_nullish_and_logical_global_this_array_from_wrappers_in_js_input_when_browser_harness_is_configured(
) {
    assert_browser_requested("test", "smoke.test.js", false);
}

#[test]
fn test_supports_nullish_and_logical_global_this_array_from_wrappers_in_ts_jsx_and_tsx_input_when_browser_harness_is_configured(
) {
...
#[test]
fn build_bundles_nullish_and_logical_global_this_array_from_wrappers_in_js_input() {
    assert_browser_bundle("app.js", false);
}

#[test]
fn build_bundles_nullish_and_logical_global_this_array_from_wrappers_in_ts_jsx_and_tsx_input() {
