...
fn map_iteration_run_source() -> &'static str {
    "const values = [[1, 2], [1, 3], [4, 5]]; const mapAlias = Map; const wrappedMapAlias = (mapAlias); const frozenMap = Object.freeze(Map); for (const entry of new Map(values)) { console.log('map constructor iteration ok'); } for (const entry of new mapAlias(values)) { console.log('map constructor iteration ok'); } for await (const entry of new (wrappedMapAlias)(values)) { console.log('map constructor iteration ok'); } const frozenMapValues = Object.freeze(values); for (const entry of new Map(frozenMapValues)) { console.log('map constructor iteration ok'); } for (const entry of new (frozenMap)(values)) { console.log('frozen map constructor iteration ok'); }\n"
}
