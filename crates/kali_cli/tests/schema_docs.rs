...
fn phase_six_conformance_dashboard_is_present_and_deterministic() {
    ...
    for expected in [
        ...
        "Browser-requested `run` / `test` `Math.max()` / `Math.min()` / `Math.abs()` / `Math.sign()` / `Math.imul()` semantics in `.ts` and `.js` input",
...
fn package_corpus_matrix_tracks_current_browser_and_default_rows() {
    ...
    for (row, message) in [
        ...
        (
            "| npm-style package corpus | Node built-in packages (`node:buffer`, `node:assert`, `node:events`, `node:stream`, `node:fs/promises`, `node:path`, `node:os`, `node:crypto`, `node:fs`, `node:url`, `node:util`) on `.js` input | Node | `check`, `build`, `run`, `test` | checkable / buildable / executable / testable | `crates/kali_cli/tests/package_corpus.rs` |",
