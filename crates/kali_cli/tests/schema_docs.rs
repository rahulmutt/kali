...
fn phase_six_conformance_dashboard_is_present_and_deterministic() {
    ...
    for expected in [
        ...
        "Basic function call return semantics",
        "Basic relational comparison semantics",
        "Basic strict equality / inequality semantics",
        "Basic boolean conjunction / disjunction semantics",
        "Basic async/await sequencing",
        "Basic try/catch exception handling and try/finally sequencing",
        "Console error / warn / info / debug routing plus `console.assert()` false-branch reporting",
        ...
        "Basic `Reflect.ownKeys()` enumeration semantics in `.js` input, including integer-like key ordering",
        "Browser-requested `run` / `test` `Reflect.ownKeys()` enumeration semantics when the browser API surface is inherited in `.ts` and `.js` input, including JSON-output coverage",
        ...
        "Browser-requested `run` / `test` object-enumeration semantics in `.ts` and `.js` input, including overwrite-delete-reinsert ordering plus string-primitive enumeration and array-literal-arguments in `.js` input and JSON-output coverage",
        ...
        "Browser bundle integer-like key ordering semantics in `.ts` and `.js` input, including JSON-output coverage",
        ...
        "Browser-requested `run` / `test` zero-capable budget pair (`--max-threads 0` / `--max-spawned-processes 0`) in `.ts` input",
        ...
        "Browser-requested `run` / `test` object-enumeration semantics in `.ts` and `.js` input, including overwrite-delete-reinsert ordering plus string-primitive enumeration and array-literal-arguments in `.js` input and JSON-output coverage",
        "Browser-requested `run` / `test` `Reflect.ownKeys()` enumeration semantics in `.ts` and `.js` input, including JSON-output coverage",
        ...
        "Browser bundle `Math.clz32()` semantics in `.ts` and `.js` input, including JSON-output coverage",
        ...
        "Browser bundle `Reflect.ownKeys()` enumeration semantics in `.ts` and `.js` input, including JSON-output coverage",
...
fn package_corpus_matrix_tracks_current_browser_and_default_rows() {
    ...
    for (row, message) in [
        ...
        (
            "| npm-style package corpus | scoped packages with exports maps with `.js` input and inherited browser `apiSurface` | browser-targeted | `check`, `build --bundle` | checkable / buildable / deployable-through-host | `crates/kali_cli/tests/package_corpus.rs` |",
            "browser inherited scoped package corpus row should be recorded in the package corpus matrix",
        ),
        (
        ...
        (
            ...
            "browser runtime string/web-baseline corpus row should be recorded in the package corpus matrix",
        ),
        (
            "| browser runtime corpus | browser string/web-baseline packages with `.js` input and inherited browser `apiSurface` | browser-targeted execution harness | `run`, `test` | executable / testable-through-host | `crates/kali_cli/tests/package_corpus.rs` |",
            "browser runtime inherited-browser string/web-baseline corpus row should be recorded in the package corpus matrix",
        ),
        (
        ...
        (
            "| npm-style package corpus | browser internal browser-rewrite packages with `.js` input and inherited browser `apiSurface` | browser-targeted | `check`, `build --bundle` | checkable / buildable / deployable-through-host | `crates/kali_cli/tests/package_corpus.rs` |",
            "browser inherited internal browser-rewrite corpus row should be recorded in the package corpus matrix",
        ),
        (
        ...
        (
            ...
            "node inherited net rejection row should be recorded in the package corpus matrix",
        ),
        (
    ...
    let browser_string_web_baseline_row =
        "| browser runtime corpus | browser string/web-baseline packages with `.js` input | browser-targeted execution harness | `run`, `test` | executable / testable-through-host | `crates/kali_cli/tests/package_corpus.rs` |";
    assert_row_once(
        browser_string_web_baseline_row,
        "browser runtime string/web-baseline corpus row should be recorded exactly once in the package corpus matrix",
    );
    let browser_string_web_baseline_inherited_row =
        "| browser runtime corpus | browser string/web-baseline packages with `.js` input and inherited browser `apiSurface` | browser-targeted execution harness | `run`, `test` | executable / testable-through-host | `crates/kali_cli/tests/package_corpus.rs` |";
    assert_row_once(
        browser_string_web_baseline_inherited_row,
        "browser runtime inherited-browser string/web-baseline corpus row should be recorded exactly once in the package corpus matrix",
    );
    ...
    let browser_internal_browser_rewrite_inherited_row =
        "| npm-style package corpus | browser internal browser-rewrite packages with `.js` input and inherited browser `apiSurface` | browser-targeted | `check`, `build --bundle` | checkable / buildable / deployable-through-host | `crates/kali_cli/tests/package_corpus.rs` |";
    assert_row_once(
        browser_internal_browser_rewrite_inherited_row,
        "browser inherited internal-browser-rewrite corpus row should be recorded exactly once in the package corpus matrix",
    );
