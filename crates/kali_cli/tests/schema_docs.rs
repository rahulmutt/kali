...
fn phase_7_readme_tracks_browser_harness_summary_fallback_coverage() {
    ...
    assert!(
        readme.contains(
            "The browser bundle `--format cjs` path now also has first-class `.js` input coverage in both explicit and inherited browser API-surface modes, including JSON-output coverage and sandbox-policy acceptance in those JS-input lanes, and the explicit and inherited browser `apiSurface` paths now also have human-output coverage in `.js` input, keeping the alternate browser wrapper format aligned with the same JS-input evidence lane."
        ),
