...
fn phase_six_conformance_dashboard_is_present_and_deterministic() {
    ...
    for expected in [
        ...
        "Basic `Object.keys()` enumeration semantics, including overwrite ordering",
        "Basic `Object.entries()` enumeration semantics, including overwrite ordering",
        "Basic `Object.values()` enumeration semantics, including overwrite ordering",
        ...
        "Browser-requested `run` / `test` object-enumeration semantics in `.ts` and `.js` input, including string-primitive enumeration in `.js` input and JSON-output coverage",
        ...
        "Shared web-baseline primitives (`structuredClone`, `AbortController`, `AbortSignal`, `EventTarget`, `CustomEvent`, `URL`, `URLSearchParams`, `TextEncoder`, `TextDecoder`) in `.ts` and `.js` input",
        "Browser-requested `run` / `test` shared web-baseline primitives (`structuredClone`, `AbortController`, `AbortSignal`, `EventTarget`, `CustomEvent`, `URL`, `URLSearchParams`, `TextEncoder`, `TextDecoder`) in `.ts` and `.js` input, including inherited-browser-api-surface coverage and JSON-output coverage",
        ...
        "Browser bundle `Object.keys()` / `Object.entries()` / `Object.values()` enumeration semantics in `.ts` and `.js` input, including string-primitive enumeration and overwrite ordering, plus JSON-output coverage for string-primitive enumeration and overwrite ordering",
...
fn package_corpus_matrix_tracks_current_browser_and_default_rows() {
    ...
    for (row, message) in [
        ...
        (
            "| npm-style package corpus | Node built-in packages (`node:buffer`, `node:assert`, `node:child_process`, `node:events`, `node:stream`, `node:fs/promises`, `node:path`, `node:os`, `node:crypto`, `node:http`, `node:timers`, `node:fs`, `node:url`, `node:util`) on `.js` input | Node | `check`, `build`, `run`, `test` | checkable / buildable / executable / testable | `crates/kali_cli/tests/package_corpus.rs` |",
