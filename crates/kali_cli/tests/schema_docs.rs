...
fn phase_six_conformance_dashboard_is_present_and_deterministic() {
    ...
    for expected in [
        ...
        "Browser-requested `run` / `test` shared web-baseline primitives (`structuredClone`, `AbortController`, `AbortSignal`, `EventTarget`, `CustomEvent`, `URL`, `URLSearchParams`, `TextEncoder`, `TextDecoder`) when the browser API surface is inherited in `.ts` and `.js` input, including JSON-output coverage",
        ...
        "Browser-requested `run` / `test` `Math.ceil()` semantics in `.js` input, including JSON-output coverage",
        "Browser-requested `run` / `test` object-enumeration semantics in `.ts` and `.js` input, including overwrite/delete-reinsert ordering plus string-primitive enumeration in `.js` input and JSON-output coverage",
