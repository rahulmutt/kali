...
fn browser_late_process_control_source_includes_zero_probe_invocation_forms() {
    ...
    assert_eq!(
        ...
        1,
        "browser TS late-compat source should embed the shared zero-probe inventory exactly once"
    );
    assert!(
    ...
    assert_eq!(
        ...
        1,
        "browser TS late-compat source should embed the shared parenthesized receiver-freeze source exactly once"
    );
    for expected in
...
#[test]
fn check_and_build_reject_generator_and_async_generator_class_expressions_in_browser_api_surface_ts_input(
) {
