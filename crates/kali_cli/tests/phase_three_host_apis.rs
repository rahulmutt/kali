...
#[test]
fn phase_three_host_api_source_includes_mixed_bracket_subprocess_alias() {
    let source = phase_three_host_api_source();
    assert!(
        source.contains(r#"new globalThis.Deno["Command"]('sh').spawn()"#),
        "source: {source}"
    );
}
