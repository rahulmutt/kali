...
fn supported_source() -> &'static str {
    r#"console.log(String.fromCodePoint());
console.log(Object.freeze(globalThis.String.fromCodePoint)());
console.log(String.fromCodePoint(72, 105));
console.log(globalThis.String.fromCodePoint(79, 75));
console.log(globalThis["String"]["fromCodePoint"](65));
console.log(globalThis["String"]['fromCodePoint'](67));
console.log(Object.freeze(globalThis['String'])["fromCodePoint"](68));
...
#[test]
fn browser_bundle_accepts_static_ascii_string_from_code_point_across_source_classes() {
    for extension in ["js", "ts", "jsx", "tsx"] {
        let dir = tempdir().expect("tempdir");
        let source_path = dir
            .path()
            .join(format!("string-from-code-point.{extension}"));
        fs::write(&source_path, supported_source()).expect("write source");
...
#[test]
fn json_browser_bundle_accepts_static_ascii_string_from_code_point_in_tsx_input() {
    let dir = tempdir().expect("tempdir");
    let source_path = dir.path().join("string-from-code-point.tsx");
    fs::write(&source_path, supported_source()).expect("write source");
