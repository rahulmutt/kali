...
fn browser_bundle_math_sin_cos_tan_source() -> &'static str {
    r##"// kali-tree-shake: mathSinCosTanZeroIdentities
function mathSinCosTanZeroIdentities() {
  const zero = 0;
  console.log(Math.sin(zero));
  console.log(Math.cos(zero));
  console.log(Math.tan(zero));
  return [Math.sin(zero), Math.cos(zero), Math.tan(zero)];
}
"##
}

fn assert_browser_bundle_math_sin_cos_tan(filename: &str, json_output: bool) {
    let dir = tempdir().expect("tempdir");
    let source_path = dir.path().join(filename);
    fs::write(&source_path, browser_bundle_math_sin_cos_tan_source()).expect("write source");
...
await mod.mathSinCosTanZeroIdentities();
"#,
    );
...
#[test]
fn build_emits_math_sin_cos_tan_zero_identities_in_js_input() {
    assert_browser_bundle_math_sin_cos_tan("app.js", false);
}

#[test]
fn build_emits_math_sin_cos_tan_zero_identities_in_ts_input() {
    assert_browser_bundle_math_sin_cos_tan("app.ts", false);
}
