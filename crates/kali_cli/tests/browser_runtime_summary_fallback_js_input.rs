...
#[test]
fn json_test_falls_back_to_stdout_when_browser_summary_file_is_missing_when_browser_harness_is_configured_in_js_input(
) {
    let dir = tempdir().expect("tempdir");
    let source_path = dir.path().join("missing-summary.test.js");
    ...
    let output = Command::new(kali_bin())
        ...
        .env(
            ...
            r#"node -e 'process.stdout.write("{\"args\":[\"stdout\"],\"tests\":[\"browser missing summary\"],\"testsFailed\":0,\"hostContract\":\"browser-requested\",\"runtimeBackend\":\"browser-harness\"}\n");'"#,
        )
        .arg("--output")
        .arg("json")
        .arg("test")
...
#[test]
fn run_falls_back_to_stdout_when_browser_summary_file_is_missing_when_browser_harness_is_configured_in_js_input(
) {
    let dir = tempdir().expect("tempdir");
    let source_path = dir.path().join("missing-summary.js");
    write_js_source(&source_path);
...
#[test]
fn run_falls_back_to_stdout_when_browser_summary_file_is_unparseable_when_browser_harness_is_configured_in_js_input(
) {
    let dir = tempdir().expect("tempdir");
...
#[test]
fn json_test_canonicalizes_whitespace_padded_host_labels_when_browser_harness_is_configured_in_js_input(
) {
    let dir = tempdir().expect("tempdir");
...
#[test]
fn json_run_canonicalizes_whitespace_padded_host_labels_when_browser_harness_is_configured_in_js_input(
) {
    let dir = tempdir().expect("tempdir");
    let source_path = dir.path().join("padded-summary.js");
    write_js_source(&source_path);
...
#[test]
fn json_test_falls_back_to_stdout_when_browser_summary_file_has_whitespace_only_host_labels_when_browser_harness_is_configured_in_js_input(
) {
    let dir = tempdir().expect("tempdir");
    let source_path = dir.path().join("whitespace-labels-summary.test.js");
    write_js_source(&source_path);
...
#[test]
fn json_run_falls_back_to_stdout_when_browser_summary_file_has_whitespace_only_host_labels_when_browser_harness_is_configured_in_js_input(
) {
    let dir = tempdir().expect("tempdir");
    let source_path = dir.path().join("whitespace-labels-summary.js");
    write_js_source(&source_path);
...
#[test]
fn run_falls_back_to_stdout_when_browser_summary_file_is_whitespace_only_when_browser_harness_is_configured_in_js_input(
) {
    let dir = tempdir().expect("tempdir");
    let source_path = dir.path().join("whitespace-summary.js");
    write_js_source(&source_path);
...
#[test]
fn json_test_falls_back_to_stdout_when_browser_summary_file_thread_topology_script_url_is_whitespace_padded_when_browser_harness_is_configured_in_js_input(
) {
    let dir = tempdir().expect("tempdir");
    let source_path = dir.path().join("thread-topology-summary.test.js");
    write_js_source(&source_path);
...
#[test]
fn json_test_falls_back_to_stdout_when_browser_summary_file_thread_topology_is_invalid_when_browser_harness_is_configured_in_js_input(
) {
    let dir = tempdir().expect("tempdir");
    let source_path = dir.path().join("thread-topology-summary.test.js");
    write_js_source(&source_path);
    ...
    let output = Command::new(kali_bin())
        ...
        .env(
            "KALI_BROWSER_BUNDLE_HARNESS_COMMAND",
            r#"node -e 'const fs = require("fs"); fs.writeFileSync(process.env.KALI_BROWSER_HARNESS_SUMMARY_FILE, "{\"args\":[\"alpha\"],\"tests\":[\"browser merge\"],\"testsFailed\":2,\"hostContract\":\"browser-requested\",\"runtimeBackend\":\"browser-harness\",\"threadTopology\":{\"totalInstances\":1,\"terminatedInstances\":0,\"liveInstances\":[{\"instanceId\":0,\"scriptUrl\":\"https://example.com/thread.js\",\"postedMessages\":[],\"postedSharedBuffers\":[],\"wasTerminated\":false}]}}\n"); process.stdout.write("{\"args\":[\"stdout\"],\"tests\":[\"browser merge\"],\"testsFailed\":1,\"hostContract\":\"browser-requested\",\"runtimeBackend\":\"browser-harness\"}\n");'"#,
        )
        .arg("--output")
        .arg("json")
        .arg("test")
...
fn run_falls_back_to_stdout_when_browser_summary_file_thread_topology_script_url_is_whitespace_padded_when_browser_harness_is_configured_in_js_input(
    ...
    let dir = tempdir().expect("tempdir");
    let source_path = dir.path().join("thread-topology-summary.js");
    write_js_source(&source_path);
...
#[test]
fn run_falls_back_to_stdout_when_browser_summary_file_is_empty_when_browser_harness_is_configured_in_js_input(
) {
    let dir = tempdir().expect("tempdir");
...
#[test]
fn json_run_falls_back_to_stdout_when_browser_summary_file_is_missing_when_browser_harness_is_configured_in_js_input(
) {
    let dir = tempdir().expect("tempdir");
    let source_path = dir.path().join("missing-summary.js");
    write_js_source(&source_path);
...
#[test]
fn run_falls_back_to_stdout_when_browser_summary_file_has_invalid_tests_failed_type_when_browser_harness_is_configured_in_js_input(
) {
    let dir = tempdir().expect("tempdir");
    let source_path = dir.path().join("invalid-summary.js");
    write_js_source(&source_path);
...
#[test]
fn json_test_falls_back_to_stdout_when_browser_summary_file_has_null_args_and_tests_when_browser_harness_is_configured_in_js_input(
) {
    let dir = tempdir().expect("tempdir");
    let source_path = dir.path().join("null-summary.test.js");
    write_js_source(&source_path);
...
#[test]
fn json_test_falls_back_to_stdout_when_browser_summary_file_has_unexpected_keys_when_browser_harness_is_configured_in_js_input(
) {
    let dir = tempdir().expect("tempdir");
    let source_path = dir.path().join("unexpected-keys-summary.test.js");
    write_js_source(&source_path);
...
#[test]
fn run_falls_back_to_stdout_when_browser_summary_file_has_null_args_and_tests_when_browser_harness_is_configured_in_js_input(
) {
    let dir = tempdir().expect("tempdir");
    let source_path = dir.path().join("null-summary.js");
    write_js_source(&source_path);
...
#[test]
fn json_test_falls_back_to_stdout_when_browser_summary_file_has_invalid_numeric_tests_failed_value_when_browser_harness_is_configured_in_js_input(
) {
    let dir = tempdir().expect("tempdir");
    let source_path = dir.path().join("invalid-numeric-summary.test.js");
    write_js_source(&source_path);
    ...
    let output = Command::new(kali_bin())
        ...
        .env(
            "KALI_BROWSER_BUNDLE_HARNESS_COMMAND",
            r#"node -e 'const fs = require("fs"); fs.writeFileSync(process.env.KALI_BROWSER_HARNESS_SUMMARY_FILE, "{\"args\":[\"alpha\"],\"tests\":[\"browser invalid numeric summary\"],\"testsFailed\":1.5,\"hostContract\":\"browser-requested\",\"runtimeBackend\":\"browser-harness\"}\n"); process.stdout.write("{\"args\":[\"stdout\"],\"tests\":[\"browser invalid numeric summary\"],\"testsFailed\":7,\"hostContract\":\"browser-requested\",\"runtimeBackend\":\"browser-harness\"}\n");'"#,
        )
        .arg("--output")
        .arg("json")
        .arg("test")
...
#[test]
fn run_falls_back_to_stdout_when_browser_summary_file_has_invalid_numeric_tests_failed_value_when_browser_harness_is_configured_in_js_input(
) {
    let dir = tempdir().expect("tempdir");
    let source_path = dir.path().join("invalid-numeric-summary.js");
    write_js_source(&source_path);
...
#[test]
fn json_test_uses_stdout_metadata_when_browser_summary_file_is_missing_tests_failed_when_browser_harness_is_configured_in_js_input(
) {
    let dir = tempdir().expect("tempdir");
...
#[test]
fn run_uses_stdout_metadata_when_browser_summary_file_is_missing_tests_failed_when_browser_harness_is_configured_in_js_input(
) {
    let dir = tempdir().expect("tempdir");
    let source_path = dir.path().join("incomplete-summary.js");
    write_js_source(&source_path);
    ...
    let output = Command::new(kali_bin())
        ...
        .env(
            "KALI_BROWSER_BUNDLE_HARNESS_COMMAND",
            r#"node -e 'const fs = require("fs"); fs.writeFileSync(process.env.KALI_BROWSER_HARNESS_SUMMARY_FILE, "{\"args\":[\"alpha\"],\"tests\":[\"browser incomplete summary\"],\"hostContract\":\"browser-requested\",\"runtimeBackend\":\"browser-harness\"}\n"); process.stdout.write("browser incomplete summary\n");'"#,
        )
        .arg("run")
        .arg("--api")
        .arg("browser")
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("browser incomplete summary"),
        "stdout: {stdout}"
    );
    assert_eq!(String::from_utf8_lossy(&output.stderr), "");
}
...
#[test]
fn run_falls_back_to_stdout_when_browser_summary_file_has_invalid_tests_array_items_when_browser_harness_is_configured_in_js_input(
) {
    let dir = tempdir().expect("tempdir");
    let source_path = dir.path().join("invalid-tests-array-items.js");
    write_js_source(&source_path);
