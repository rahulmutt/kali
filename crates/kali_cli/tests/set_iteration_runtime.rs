...
fn set_iteration_run_source() -> &'static str {
    "const values = [1, 2, 1]; const setAlias = Set; const wrappedSetAlias = (setAlias); const frozenSet = Object.freeze(Set); const frozenGlobalThisSet = Object.freeze(globalThis.Set); const frozenGlobalThisBracketedSet = Object.freeze(globalThis[\"Set\"]); for (const value of new Set(values)) { console.log(value); } for (const value of new setAlias(values)) { console.log(value); } for await (const value of new (wrappedSetAlias)(values)) { console.log(value); } const frozenValues = Object.freeze(values); for (const value of new Set(frozenValues)) { console.log(value); } for (const value of new (frozenSet)(values)) { console.log('frozen set constructor iteration ok'); } for (const value of new (frozenGlobalThisSet)(values)) { console.log('frozen globalThis.Set constructor iteration ok'); } for (const value of new (frozenGlobalThisBracketedSet)(values)) { console.log('frozen globalThis[\"Set\"] constructor iteration ok'); }\n"
}
