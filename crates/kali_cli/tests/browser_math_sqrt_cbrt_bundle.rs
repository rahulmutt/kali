...
fn browser_bundle_math_sqrt_cbrt_source() -> &'static str {
    r##"// kali-tree-shake: mathSqrtCbrtZeroIdentities
async function mathSqrtCbrtZeroIdentities(left, right) {
...
fn assert_browser_bundle_math_sqrt_cbrt(filename: &str, json_output: bool) {
    let dir = tempdir().expect("tempdir");
    let source_path = dir.path().join(filename);
    fs::write(&source_path, browser_bundle_math_sqrt_cbrt_source()).expect("write source");
...
const result = await mod.mathSqrtCbrtZeroIdentities(1n, 2n);
if (result !== 0n) {
...
#[test]
fn build_emits_math_sqrt_and_cbrt_zero_identities_in_js_input() {
    assert_browser_bundle_math_sqrt_cbrt("app.js", false);
}

#[test]
fn build_emits_math_sqrt_and_cbrt_zero_identities_in_ts_input() {
    assert_browser_bundle_math_sqrt_cbrt("app.ts", false);
}

#[test]
fn json_build_emits_math_sqrt_and_cbrt_zero_identities_in_js_input() {
    assert_browser_bundle_math_sqrt_cbrt("app.js", true);
}
