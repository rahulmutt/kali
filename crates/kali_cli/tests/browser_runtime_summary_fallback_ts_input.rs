...
#[test]
fn json_test_canonicalizes_whitespace_padded_host_labels_when_browser_harness_is_configured_in_ts_input(
) {
    let dir = tempdir().expect("tempdir");
    let source_path = dir.path().join("padded-summary.test.ts");
    write_ts_source(&source_path);
...
#[test]
fn json_run_canonicalizes_whitespace_padded_host_labels_when_browser_harness_is_configured_in_ts_input(
) {
    let dir = tempdir().expect("tempdir");
    let source_path = dir.path().join("padded-summary.ts");
    write_ts_source(&source_path);
...
#[test]
fn json_test_falls_back_to_stdout_when_browser_summary_file_has_whitespace_only_host_labels_when_browser_harness_is_configured_in_ts_input(
) {
    let dir = tempdir().expect("tempdir");
    let source_path = dir.path().join("whitespace-labels-summary.test.ts");
    write_ts_source(&source_path);
...
#[test]
fn json_run_falls_back_to_stdout_when_browser_summary_file_has_whitespace_only_host_labels_when_browser_harness_is_configured_in_ts_input(
) {
    let dir = tempdir().expect("tempdir");
    let source_path = dir.path().join("whitespace-labels-summary.ts");
    write_ts_source(&source_path);
...
#[test]
fn json_test_falls_back_to_stdout_when_browser_summary_file_thread_topology_script_url_is_whitespace_padded_when_browser_harness_is_configured_in_ts_input(
) {
    let dir = tempdir().expect("tempdir");
    let source_path = dir.path().join("thread-topology-summary.test.ts");
    write_ts_source(&source_path);
...
#[test]
fn run_falls_back_to_stdout_when_browser_summary_file_thread_topology_script_url_is_whitespace_padded_when_browser_harness_is_configured_in_ts_input(
) {
    let dir = tempdir().expect("tempdir");
    let source_path = dir.path().join("thread-topology-summary.ts");
    write_ts_source(&source_path);
...
#[test]
fn run_falls_back_to_stdout_when_browser_summary_file_is_empty_when_browser_harness_is_configured_in_ts_input(
) {
    let dir = tempdir().expect("tempdir");
    let source_path = dir.path().join("empty-summary.ts");
    write_ts_source(&source_path);
...
#[test]
fn json_test_falls_back_to_stdout_when_browser_summary_file_has_null_args_and_tests_when_browser_harness_is_configured_in_ts_input(
) {
    let dir = tempdir().expect("tempdir");
    let source_path = dir.path().join("null-summary.test.ts");
    write_ts_source(&source_path);
...
#[test]
fn json_test_falls_back_to_stdout_when_browser_summary_file_has_unexpected_keys_when_browser_harness_is_configured_in_ts_input(
) {
    let dir = tempdir().expect("tempdir");
    let source_path = dir.path().join("unexpected-keys-summary.test.ts");
    write_ts_source(&source_path);
...
#[test]
fn run_falls_back_to_stdout_when_browser_summary_file_has_null_args_and_tests_when_browser_harness_is_configured_in_ts_input(
) {
    let dir = tempdir().expect("tempdir");
    let source_path = dir.path().join("null-summary.ts");
    write_ts_source(&source_path);
...
#[test]
fn json_test_falls_back_to_stdout_when_browser_summary_file_has_invalid_numeric_tests_failed_value_when_browser_harness_is_configured_in_ts_input(
) {
    let dir = tempdir().expect("tempdir");
    let source_path = dir.path().join("invalid-numeric-summary.test.ts");
    write_ts_source(&source_path);
...
#[test]
fn run_falls_back_to_stdout_when_browser_summary_file_has_invalid_numeric_tests_failed_value_when_browser_harness_is_configured_in_ts_input(
) {
    let dir = tempdir().expect("tempdir");
    let source_path = dir.path().join("invalid-numeric-summary.ts");
    write_ts_source(&source_path);
...
#[test]
fn json_run_falls_back_to_stdout_when_browser_summary_file_is_missing_when_browser_harness_is_configured_in_ts_input(
) {
    let dir = tempdir().expect("tempdir");
    let source_path = dir.path().join("missing-summary.ts");
    write_ts_source(&source_path);
...
#[test]
fn json_run_falls_back_to_stdout_when_browser_summary_file_is_empty_when_browser_harness_is_configured_in_ts_input(
) {
    let dir = tempdir().expect("tempdir");
    let source_path = dir.path().join("empty-summary.ts");
    write_ts_source(&source_path);
...
#[test]
fn json_test_falls_back_to_stdout_when_browser_summary_file_has_unexpected_top_level_keys_when_browser_harness_is_configured_in_ts_input(
) {
    let dir = tempdir().expect("tempdir");
    let source_path = dir.path().join("unexpected-summary.test.ts");
    write_ts_source(&source_path);
