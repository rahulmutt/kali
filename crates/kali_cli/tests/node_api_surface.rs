...
fn node_api_surface_supports_process_kill_zero_probe_in_js_jsx_and_tsx_input_on_check_build_run_and_test_commands(
    ...
    for extension in ["js", "jsx", "tsx"] {
        for inherited in [false, true] {
            ...
            fs::write(
                &run_file,
                "console.log(process.kill(0)); globalThis.process.kill(0); globalThis.process[\"kill\"](0); globalThis[\"process\"].kill(0); globalThis[\"process\"][\"kill\"](0); process[\"kill\"](0);\n",
            )
            .expect("write run file");
            fs::write(
                &test_file,
                "Kali.test('process kill', () => { if (!process.kill(0) || !globalThis.process.kill(0) || !globalThis.process[\"kill\"](0) || !globalThis[\"process\"].kill(0)) { throw new Error('expected zero probe'); } });\n",
            )
            .expect("write test file");
