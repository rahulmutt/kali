...
fn node_api_surface_supports_process_kill_zero_probe_through_static_zero_aliases_in_js_ts_jsx_and_tsx_input_on_check_build_run_and_test_commands(
    ...
    for extension in ["js", "ts", "jsx", "tsx"] {
        for inherited in [false, true] {
            ...
            fs::write(
                &run_file,
                "const zero = 0; const zeroAlias = zero; console.log(process.kill(zeroAlias)); console.log(globalThis.process.kill(+zero)); console.log(globalThis[\"process\"][\"kill\"](zero)); console.log(process[\"kill\"](zero)); console.log(globalThis[\"process\"][\"kill\"](zero));\n",
            )
            .expect("write run file");
            fs::write(
                &test_file,
                "const zero = 0; const zeroAlias = zero; Kali.test('process kill alias', () => { if (!process.kill(zeroAlias) || !globalThis.process.kill(+zero) || !globalThis[\"process\"][\"kill\"](zero) || !process[\"kill\"](zero)) { throw new Error('expected zero probe'); } });\n",
            )
            .expect("write test file");
