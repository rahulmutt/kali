...
//! (`:138-138`) has no helper: its whole body is a single
//! `assert!(source.contains(expected))` self-check (`:142-145`) run in a `for`
...
//! (`:114-139`), before any command is built and without ever invoking `kali`.
//!
