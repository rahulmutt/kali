...
fn browser_bundle_math_imul_clz32_aliases_source() -> &'static str {
    r##"// kali-tree-shake: browserMathImulClz32Aliases
function browserMathImulClz32Aliases() {
  const imulLeft = 2147483647;
  const imulRight = 2;
  const clz32Value = 1;
...
fn assert_browser_bundle_math_imul_clz32_aliases(filename: &str, json_output: bool) {
    let dir = tempdir().expect("tempdir");
    let source_path = dir.path().join(filename);
    fs::write(
        &source_path,
        browser_bundle_math_imul_clz32_aliases_source(),
    )
    .expect("write source");
...
await mod.browserMathImulClz32Aliases();
"#,
    );
...
#[test]
fn build_emits_math_imul_clz32_aliases_in_js_input() {
    assert_browser_bundle_math_imul_clz32_aliases("app.js", false);
}

#[test]
fn build_emits_math_imul_clz32_aliases_in_ts_input() {
    assert_browser_bundle_math_imul_clz32_aliases("app.ts", false);
}

#[test]
fn json_build_emits_math_imul_clz32_aliases_in_js_input() {
    assert_browser_bundle_math_imul_clz32_aliases("app.js", true);
}

#[test]
fn json_build_emits_math_imul_clz32_aliases_in_ts_input() {
    assert_browser_bundle_math_imul_clz32_aliases("app.ts", true);
}
