...
//!
//! Every figure in those sections is emitted from here, from three committed
//! inputs: the register (tiers via `parse_register`, verdicts via §0.2's
//! generated table), `tools/blast-radius/counts.json` (the two frequency axes)
//! and `tools/blast-radius/clusters.json` (the assignment). A hand-typed number
//! in a ranking is the rot this whole project exists to end, so there is no
//! path here by which one can appear.
//!
...
use kali_blast_radius::{aggregate, band, parse_register, Cluster, ScoredEntry};
...
fn main() {
    ...
    let _ = writeln!(
        out,
        "`FIXED`, `FAIL_CLOSED`, `ACCEPTS_INVALID` and `FL_INTERNAL` are not silent damage \
         (spec §8.1), so these {} entries do not enter. Their counts are printed because the \
         removal is not cosmetic: it takes the largest reachable count in the whole \
         measurement out of the ranking.\n",
        excluded.len()
    );
    ...
    let _ = writeln!(
        ...
        "### 3.4 A lane result is not an entry result\n\n{} of the {} ranked entries measure \
         ...
         retired: {}. §0.2 records why in each case — R-30's, R-47's and R-53's `const` lanes \
         are the entries' own declared controls, R-08's `===` half fails closed while its `??` \
         half is untouched, and R-49 (not in the ranking at all) fails closed by **R-35's** \
         switch allowlist rather than by its own gate. An entry is retired when every lane \
         moves, which is a claim no single lane can make.\n",
        mixed.len(),
        silent.len(),
        mixed.join("; ")
    );
