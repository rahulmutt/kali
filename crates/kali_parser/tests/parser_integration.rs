...
mod member_expressions {
    ...
    fn test_parse_bracketed_string_literal_permissions_query_member_expression() {
        ...
        let assert_permissions_query_chain = |statement: &kali_ast::Statement, inherited: bool| {
            match statement {
                kali_ast::Statement::ExpressionStatement(es) => {
                    match es.expression.as_ref() {
                        kali_ast::Expression::MemberExpression(me) => {
                                ...
                                kali_ast::Expression::MemberExpression(inner) => {
                                    ...
                                    if inherited {
                                        ...
                                        match &inner.object {
                                            ...
                                            other => {
                                                panic!("Expected Deno identifier, got {other:?}")
                                            }
                                        }
                                    }
                                }
        ...
        assert_permissions_query_chain(&output.statements[0], false);
        assert_permissions_query_chain(&output.statements[1], true);
    }

    #[test]
    fn test_parse_bracketed_string_literal_permission_escalation_member_expressions() {
        let output = parse(
        ...
        let assert_permission_chain = |statement: &kali_ast::Statement,
                                       inherited: bool,
                                       property: &str| {
            match statement {
                kali_ast::Statement::ExpressionStatement(es) => match es.expression.as_ref() {
        ...
        assert_permission_chain(&output.statements[0], false, "request");
        assert_permission_chain(&output.statements[1], false, "revoke");
        assert_permission_chain(&output.statements[2], true, "request");
        assert_permission_chain(&output.statements[3], true, "revoke");
    }

    #[test]
    fn test_parse_mixed_bracket_dot_permission_escalation_member_expressions() {
        let output = parse(
            "globalThis[\"Deno\"].permissions[\"request\"]; globalThis[\"Deno\"].permissions[\"revoke\"]; globalThis.Deno[\"permissions\"][\"request\"]; globalThis.Deno[\"permissions\"][\"revoke\"];",
        );
        assert_eq!(output.statements.len(), 4);
        ...
        assert_permission_chain(&output.statements[2], "request");
        assert_permission_chain(&output.statements[3], "revoke");
    }
