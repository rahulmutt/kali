...
fn supported_static_string_prefix_suffix_lowers_ascii_literals() {
    ...
    // `n - 1` separator-count term all use `i64.const 1`. + 4 from `__streq`
    // (throw-fallout Stage 1, also present in every module): the identical-
    // handles return, the len==0 return, the loop-increment `i += 1`, and the
    // all-bytes-equal result.
    assert_eq!(
