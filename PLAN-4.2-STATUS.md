...
- `KaliCore/Safety.lean` now proves the current no-dangling-reference statement for the RC snapshot model, its release-path liveness split, live/released disjointness, a release-update preservation theorem for the remaining live set, and the new refcount-decrement update helper theorem
...
- The memory-safety theorem is still a bounded RC snapshot model, not the full ownership / RC safety proof target from Stage 4.2, even though the current slice now explicitly records the live-to-released transition preservation plus the refcount-decrement helper's release bookkeeping and live/released disjointness
