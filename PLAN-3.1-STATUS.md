...
- The richer generic-instantiation planner is still ahead; the current work now covers small-function call-site specialization plus MIR-annotated layout-stable clones, const-bound object and array layout folding, and pruning, but not the broader cross-module instantiation model.
...
Continue Stage 3.1 by broadening MIR-aware specialization from the current layout-stable clone path toward the fuller generic-instantiation planner and cross-module specialization model, so the remaining phase-3 breadth targets can be closed.
