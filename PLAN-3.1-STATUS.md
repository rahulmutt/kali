...
- MIR-aware layout signatures now also collapse shared closure-valued bindings to a single specialization when the call-site layout is otherwise identical ✅
...
- MIR-aware specialization now has a shared closure-layout regression test so identical higher-order call-site layouts reuse one specialized clone
...
Continue Stage 3.1 by broadening MIR-aware specialization from the current layout-stable clone path toward the fuller generic-instantiation planner and cross-module specialization model, especially around richer closure/layout sharing, so the remaining phase-3 breadth targets can be closed.
